...
class YLB_Db_Pdo
{
     ...
     * @access    private
     */
    public function __construct($id, $cfg)
    {
        try
        {
            $options = array(
    ...
    public function query($sql = null)//执行SQL
    {
        if(is_object($this->dbHandle))
        {
            ...
            if (DB_DEBUG)
            {
                YLB_Log::log($sql, YLB_Log::LOG, 'db_query');
            }
            ...
            if (false === $rs)
            {
                ...
                if (DB_DEBUG)
                {
                    ...
                    YLB_Log::log(encode_json($error_info), YLB_Log::ERROR, 'db_error');
                }
            }
        }
        else
    ...
    public function exec($sql = null)//执行SQL
    {
        if(is_object($this->dbHandle))
        {
            ...
            if (DB_DEBUG)
            {
                YLB_Log::log($sql, YLB_Log::LOG, 'db_exec');
                YLB_Log::log('effect_rows=' . $rs, YLB_Log::LOG, 'db_exec');
            }
        ...
        if ($sql)
        {
            ...
            else
            {
                ...
                if (DB_DEBUG)
                {
                    ...
                    YLB_Log::log($error_info, FirePHP::ERROR, 'db_error');
                }
                */
     ...
     * @access  Db Object
     */
    public function getErrorCode(&$error_info = null)
    {
        $error_info = $this->dbHandle->errorInfo();
        return $error_info[1];
    }
	...
	public function getStructure()
	{
		...
		foreach($rows as $value)
		{
			...
			$create_sql = sprintf('SHOW CREATE TABLE %s', $table_name);
			
			$create_row = $this->getRow($create_sql);
			
			
			//$structure_row[] = $table_name;
			$structure_row[$table_name] = $create_row['Create Table'];
		}
	...
	public function version() {
		...
		return preg_replace( '/[^0-9.].*/', '', $row['version'] );
	}
}
?>