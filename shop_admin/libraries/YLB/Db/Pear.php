...
class YLB_Db_Pear
{
    ...
    private function __construct($id, $cfg)
    {
        ...
        $this->dbHandle = DB::connect($dsn);

        if (DB::isError($this->dbHandle))
        {
            if ('cli' == SAPI)
            {
                echo $this->dbHandle;
    ...
    public function query($sql = null)//执行SQL
    {
        ...
        if (DB::isError($rs))
        {
            ...
            if (DB_DEBUG)
            {
                ...
                YLB_Log::log($error_info, YLB_Log::ERROR, 'db');
            }
    ...
    public function exec($sql = null)//执行SQL
    {
        ...
        {
            $rs = intval($this->dbHandle->affectedRows());
        }
    ...
    public function insertId()
    {
        ...
        $rs = $this->dbHandle->getRow('SELECT LAST_INSERT_ID() as last_insert_id;');
