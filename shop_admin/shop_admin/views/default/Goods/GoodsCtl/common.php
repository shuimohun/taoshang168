...
<div class="wrapper page">
  <div class="fixed-bar">
    <div class="item-title">
      ...
      ?>
         <!--  <li><a  href="<?= YLB_Registry::get('url') ?>?ctl=Goods_Goods&met=common" <?=((-1 == request_int('common_state', '-1') && -1 == request_int('common_verify', '-1')) ? 'class="current"' : '')?>><span>所有商品</span></a></li>
          <li><a href="<?= YLB_Registry::get('url') ?>?ctl=Goods_Goods&met=common&common_state=10" <?=(10 == request_int('common_state', '-1') ? 'class="current"' : '')?> ><span>违规下架</span></a></li>
          <li><a href="<?= YLB_Registry::get('url') ?>?ctl=Goods_Goods&met=common&common_verify=10" <?=(10 == request_int('common_verify', '-1') ? 'class="current"' : '')?>><span>等待审核</span></a></li>
          <li><a href="<?= YLB_Registry::get('url') ?>?ctl=Config&met=goods&config_type%5B%5D=goods" <?=('Config' == request_string('ctl') ? 'class="current"' : '')?>><span>商品设置</span></a></li> -->
      </ul>
    </div>
  </div>
			...
			<ul class="ul-inline fix">
				...
				<li id="brand" style="display: list-item;">
                    <span class="mod-choose-input" id="filter-brand">
                        <input type="text" class="ui-input" id="brand_id" autocomplete="off" placeholder="输入品牌名称" >
