    ...
    <div class="ncap-form-default">
        ...
        <dl class="row">
            ...
            <dd class="opt">
                ...
                <p class="notic">上传分类图片时，请上传宽度不低于100像素，高度不低于100像素的图片</p>
       ...
       <!-- <dl class="row">
        ...
        <dl class="row">
            ...
            <dd class="opt">
                ...
                <link rel="stylesheet" href="<?= $this->view->css ?>/combo.select.css">

                <div class="dowebok">
                    <div class="combo-select combo-open">
                        <select tabindex="-1">
                        </select>
                        <div class="combo-arrow"></div>
                        <ul class="combo-dropdown">
                        </ul>
                        <input type="text" placeholder="" name="keyword" class="combo-input" >
                    </div>
                </div>
                ...
                <script>
                    $(function () {
                        if(rowData.type_id)
                        {
                            $.ajax({
                                type: 'GET',
                                url: '?ctl=Goods_Type&met=getTypeById&typ=json',
                                cache:false,//不从浏览器缓存中加载请求信息
                                data: {'type_id':rowData.type_id},//向服务器端发送的数据
                                dataType: 'json',//服务器返回数据的类型为json
                                success: function (json) {
                                    if (json.length != 0) {//检测返回的结果是否为空
                                        $('#type_id').val(rowData.type_id);
                                        $('.combo-input').val(json.data['type_name']);
                                    }
                                }
                            });
                        }
                        ...
                        $.ajax({
                            type: 'GET',
                            url: '?ctl=Goods_Type&met=lists&typ=json',
                            cache:false,//不从浏览器缓存中加载请求信息
                            data: {'rows':20},//向服务器端发送的数据
                            dataType: 'json',//服务器返回数据的类型为json
                            success: function (json) {
                                console.log(json)
                                if (json.length != 0) {//检测返回的结果是否为空
                                    $('.combo-dropdown').html('');
                                    var lists = "";
                                    $.each(json.data.items, function (index,array) {
                                        //遍历出每一条返回的数据
                                        lists +="<li class='option-item' data-value='"+array['type_id']+"'>"+array['type_name']+"</li>";
                                    });
                                    $('.combo-dropdown').html(lists);//将搜索到的结果展示出来
                                } else {
                                    $('.combo-dropdown').html('');
                                }
                            }
                        ...
                        //var nDivHight = $(".combo-dropdown").height();
                        $(".combo-dropdown").scroll(function(){
                            nScrollHight = $(this)[0].scrollHeight;
                            nScrollTop = $(this)[0].scrollTop;
                            if(nScrollTop + 300 >= nScrollHight){
                                $.getJSON("?ctl=Goods_Type&met=lists&typ=json",{page:i},function(json){
                                    if(json){
                                        var lists = "";
                                        $.each(json.data.items, function (index,array) {
                                            //遍历出每一条返回的数据
                                        ...
                                        $(".combo-dropdown").append(lists);//将搜索到的结果展示出来
                                        i++;
                                    }else{
                                        return false;
                                    }
                        ...
                        $('input[name="keyword"]').keyup(function() {
                            ...
                            if(inputText!=""){//检测键盘输入的内容是否为空，为空就不发出请求
                                xhr=$.ajax({
                                    ...
                                    url: '?ctl=Goods_Type&met=lists&typ=json',
                                    cache:false,//不从浏览器缓存中加载请求信息
                                    data: "skey=" + inputText,//向服务器端发送的数据
                                    dataType: 'json',//服务器返回数据的类型为json
                                    success: function (json) {
                                        $(".combo-dropdown").html("").show();
                                        if (json.length != 0) {//检测返回的结果是否为空
                                            var lists = "";
                                            $.each(json.data.items, function (index,array) {
                                                //遍历出每一条返回的数据
                                            ...
                                            $(".combo-dropdown").html(lists).show();//将搜索到的结果展示出来
                                        } else {
                                            $(".combo-dropdown").hide();
                                        }
                                    }
                                ...
                                $(".combo-dropdown").hide();//没有查询结果就隐藏搜索框
                            }
                        });

                        //绑定事件
                        $('.combo-input,.combo-arrow').on('click',function () {
                             $('.combo-dropdown').show();
                             $('.combo-select').css('border','1px solid #2CA1B3');
                        }).on('blur',function () {
                            $('.combo-dropdown').hide();
                            $('.combo-select').css('border','');
                        });
                        $('.combo-dropdown').delegate('.option-item','mousedown',function () {
                            $('#type_id').val($(this).data('value'));
                            $('.combo-input').val($(this).text());
                            $('.combo-dropdown').hide();
                        });
                    });
                </script>
