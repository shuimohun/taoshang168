...
<div class="wrapper page">
 ...
 -->
	...
	<?php if ($client_version !== $version_row['current'] && version_compare( $version_row['current'], $client_version, 'gt')):?>
        <h1 class="tc">有新的BBC版本(<?=$version_row['current']?>)可供升级。</h1><br />
...
<script>
    ...
    $('#reinstall').on("click", function (e)
    {
        $.dialog.confirm('确认重新安装,是否继续？', function () {
            window.location.href = "<?= YLB_Registry::get('url') ?>?ctl=Config&met=upgradeShopContainer&upgrade=1&force-upgrade=1";
        });
    });

    $('#force-check').on("click", function (e)
    {
        window.location.href = "<?= YLB_Registry::get('url') ?>?ctl=Config&met=updateShop&force-check=1";
    });

    $('#upgrade').on("click", function (e)
    {
        $.dialog.confirm('确认更新,是否继续？', function () {
            window.location.href = "<?= YLB_Registry::get('url') ?>?ctl=Config&met=upgradeShopContainer&upgrade=1";
        });
    });
    ...
    $('#force-upgrade').on("click", function (e)
    {
        $.dialog.confirm('确认强制重新安装,是否继续？', function () {
            window.location.href = "<?= YLB_Registry::get('url') ?>?ctl=Config&met=upgradeShopContainer&force-upgrade=1";
        });
    });
