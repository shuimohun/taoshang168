    ...
    if ( agent.indexOf("msie") > -1 && (version = agent.match(/msie [\d]/), ( version == "msie 8" || version == "msie 9" )) ) {
        ...
        $('#slider1_upload').on('click', function () {
            $.dialog({
                ...
                data: {SHOP_URL:SHOP_URL,width:1920,height:400 , callback: callback1 },    // 需要截取图片的宽高比例
                width: '800px',
                lock: true
            })
        });
        ...
        $('#slider2_upload').on('click', function () {
            $.dialog({
                title: '图片裁剪',
                content: "url: <?= YLB_Registry::get('url') ?>?ctl=Index&met=cropperImage&typ=e",
                data: {SHOP_URL:SHOP_URL,width:1920,height:400 , callback: callback2 },    // 需要截取图片的宽高比例
                width: '800px',
                lock: true
            })
        });
        ...
        $('#slider3_upload').on('click', function () {
            $.dialog({
                title: '图片裁剪',
                content: "url: <?= YLB_Registry::get('url') ?>?ctl=Index&met=cropperImage&typ=e",
                data: {SHOP_URL:SHOP_URL,width:1920,height:400 , callback: callback3 },    // 需要截取图片的宽高比例
                width: '800px',
                lock: true
            })
        });
        ...
        $('#slider4_upload').on('click', function () {
            $.dialog({
                title: '图片裁剪',
                content: "url: <?= YLB_Registry::get('url') ?>?ctl=Index&met=cropperImage&typ=e",
                data: {SHOP_URL:SHOP_URL,width:1920,height:400 , callback: callback4 },    // 需要截取图片的宽高比例
                width: '800px',
                lock: true
            })
        });
