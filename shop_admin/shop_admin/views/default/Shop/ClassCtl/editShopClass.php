...
<body>
    <form method="post" enctype="multipart/form-data" id="shop_edit_class" name="form1">
        <?php foreach ($data as $key => $value) { ?>
            ...
            <div class="ncap-form-default">
                ...
                <dl class="row">
                    <dt class="tit">
                        <label for="shop_class_displayorder">*关联分类</label>
                    </dt>
                    <dd class="opt">
                        <select id="goods_cat" name="goods_cat_id" data-id="<?=$value["goods_cat_id"]?>">
                        </select>
                    </dd>
                </dl>
            </div>
    ...
    <script>
        ...
        function postData(t, e) {
            $_form.validator({
                ...
                fields: {
                    ...
                    'shop_class_deposit': 'required;digital+;',
                ...
                valid: function(form) {
                    var shop_class_name = $.trim($("#shop_class_name").val()),
                        ...
                        goods_cat_id = $("#goods_cat").val(),
                        goods_cat_name = $("#goods_cat").find("option:selected").text(),
                        n = "add" == t ? "新增分类" : "修改分类";
        ...
        $.get(SITE_URL + '?ctl=Goods_Cat&met=getCatNew&typ=json', {cat_id: 0}, function(data) {
            if (data.status == 200) {
                var option_rows = [], cat_data = data.data;
                if(cat_data.length > 0){
                    option_rows.push("<option value=>" + '请选择...' + "</option>");
                    for (var i = 0; i < cat_data.length; i++) {
                        if(Number(cat_data[i].cat_id) == Number($('#goods_cat').data('id'))){
                            option_rows.push("<option selected='selected' value=" + cat_data[i].cat_id + ">" + cat_data[i].cat_name + "</option>");
                        }else{
                            option_rows.push("<option value=" + cat_data[i].cat_id + ">" + cat_data[i].cat_name + "</option>");
                        }
                    }
                    $('#goods_cat').html(option_rows.join(""));
                }
            }
        });
