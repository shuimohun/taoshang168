            ...
            <dd class="opt">
                <ul class="promotion-ms">
                    ...
                    <li>
                        ...
                        <?php if($rule['rule_discount']){?>
                            <span> 立减现金 <strong><?=$rule['rule_discount']?></strong> 元 </span>
                        <?php }?>
                        <?php if($rule['goods_id']){?>
                            <span> 赠送礼品 <a href="<?=YLB_Registry::get('shop_api_url')?>?ctl=Goods_Goods&met=goods&gid=<?=$rule['goods_id']?>" title="<?=$rule['goods_name']?>" target="_blank"><img style="width:40px;height:40px;vertical-align: baseline;" src="<?=$rule['goods_image']?>"></a> </span>
                        <?php }?>
                    </li>
                    <?php } ?>
                </ul>
