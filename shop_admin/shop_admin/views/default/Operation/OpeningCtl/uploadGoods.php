...
<script type="text/javascript">
    ...
    var now_num = <?=count($data['goods_list']) ?>;
    ...
    $(document).ready(function(){
        ...
        var api = frameElement.api,
            ...
            opening_data = <?=json_encode($data['goods_list'])?>,
        ...
        if (opening_data) {
            ...
            for (var i=0; i<opening_data.length; i++) {
                goods_html = template.render('item_goods_template', opening_data[i]);
                $('[nctype="item_content"]').append(goods_html);
                goods_ids.push(opening_data[i].goods_id);
            }
        } else {
            /*var opening_data = new Array();*/
        }
        ...
        function postData() {
          $.post(SITE_URL + '?ctl=Operation_Opening&met=editGoods&typ=json',{opening_id:opening_id,opening_data:goods_ids},function(data){
              if (data.status == 200) {
