...
function initGrid()
{
    ...
    $("#grid").jqGrid({
        url: SITE_URL + '?ctl=Promotion_ScareBuy&met=getPriceRangeList&typ=json',
        datatype: "json",
...
var handle = {
    operate: function (t, e)
    {
        if ("add" == t)
        {
            var i = "惠抢购管理 - 新增惠抢购价格区间", a = {oper: t, callback: this.callback};
        }
        else
        {
            var i = "惠抢购管理 - 编辑惠抢购价格区间", a = {oper: t, rowData: $("#grid").data("gridData")[e], callback: this.callback};
        }
        $.dialog({
            title: i,
            content: "url:"+SITE_URL + '?ctl=Promotion_ScareBuy&met=priceRangeManage',
            data: a,
            width: 588,
            height: 300,
            max: !1,
    ...
    }, del: function (t)
    {
        ...
        {
            Public.ajaxPost(SITE_URL + '?ctl=Promotion_ScareBuy&met=removePriceRange&typ=json', {range_id: t}, function (e)
            {
