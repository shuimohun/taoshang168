...
var THISPAGE = {
    ...
    loadGrid: function(){
        ...
        var colModel = [
            ...
            {name:'fu_sort', label:'排序',  width:100, align:"center"},
            ...
            {name:'fu_t_price', label:'价格2',  width:100, align:"center",formatter:num_format2},
            ...
            {name:'fu_total_times', label:'总次数',  width:100, align:"center"},
            {name:'fu_state_label', label:'状态',  width:50, align:"center"},
        ];
        this.mod_PageConfig.gridReg('grid', colModel);
        colModel = this.mod_PageConfig.conf.grids['grid'].colModel;

        $("#grid").jqGrid({
            url: SITE_URL + '?ctl=Promotion_Fu&met=getFuList&typ=json',
        ...
        function operFmatter (val, opt, row) {
            ...
            if(row.fu_state == 1)
            {
                html_con = '<div class="operating" data-id="' + row.fu_id + '"><span class="ui-icon ui-icon-trash" title="删除"></span><span class="ui-icon ui-icon-pencil" title="编辑"></span><span class="ui-icon ui-icon-close" title="取消活动"></span></div>';
            }
            else
            {
                html_con = '<div class="operating" data-dis="1" data-id="' + row.fu_id + '"><span class="ui-icon ui-icon-trash" title="删除"></span><span class="ui-icon ui-icon-pencil" title="编辑"></span><span class="ui-icon ui-icon-disabled ui-icon-close" title="取消活动"></span></div>';
            }
    ...
    addEvent: function(){
        ...
        $('#search').click(function(){
            queryConditions.fu_state = $source.getValue();
            queryConditions.fu_name = _self.$_fu_name.val();
...
var handle = {
    ...
    operate: function (t, e){
        if ("edit" == t){
            var i = "修改", a = {oper: t, rowData: $("#grid").jqGrid('getRowData',e), callback: this.callback};
            $.dialog({
                title: i,
