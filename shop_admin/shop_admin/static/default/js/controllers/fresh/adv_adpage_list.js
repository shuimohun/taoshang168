function initEvent()
{
	...
	$('.grid-wrap').on('click', '.ui-icon-search', function(e){
        ...
        var page_id = $(this).parent().data("id");
         $.dialog({
            title: '查看模板',
            content: "url:"+SITE_URL + '?ctl=Fresh_Adposition&met=position&page_id=' + page_id,
            //width:1236,
...
function initGrid()
{
   ...
   var t = ["操作","排序", "板块名称", "色彩风格","更新时间","显示"], e = [{
	...
	$("#grid").jqGrid({
        url: SITE_URL + "?ctl=Fresh_Adpage&met=advIndex&typ=json",
        datatype: "json",
...
var handle = {
    ...
    {         
        ...
        $.dialog({
            title: i,
            content: "url:"+ SITE_URL + "?ctl=Fresh_Adpage&met="+t+"Pagelist&page_id="+e,
            data: a,
            width: 676,
                // height: $(window).height() * 0.9,
                height:435,
    ...
    },del: function (t)
    {
        ...
        {
            Public.ajaxPost(SITE_URL + "?ctl=Fresh_Adpage&met=delPagelist&typ=json", {page_id: t}, function (e)
            {
                if (e && 200 == e.status)
                {
