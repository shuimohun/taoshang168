    ...
    if(rowData.id){
        $.get(SITE_URL + '?ctl=Goods_Goods&met=getGoodsRecommendById&typ=json&id='+rowData.id, function(data){
            if(data.status == 200)
            {
                ...
                var cat_id = data.data['goods_cat_id'];
                ...
                $.get(SITE_URL + '?ctl=Goods_Cat&met=getCat&typ=json&cat_id='+cat_id, function(d){
                    if(d.status == 200) {
                        $.each(d.data,function (k,v) {
                            ...
                            //var option = ("<option value=" + v['cat_id'] + ">" + v['cat_name'] + "</option>");
                            //$("span[nctype=gc" + k + "]").append("<select disabled='true' data-deep=" + k + ">" + option + "</select>");
                            $("span[nctype=gc" + k + "]").append(v['cat_name']);
                            $("#gc_id").val(v['cat_id']), $("#cat_name").val(v['cat_name']);
                        })
                    }
                });
                ...
                $.each(data.data['items'],function(key,val){
                    ...
                    text_append += '<i></i>';
                    text_append += '<img width="72" height="72" goods_id="'+goods_id+'" title="'+goods_name+'" goods_name="'+goods_name+'" src="'+goods_pic+'" />';
