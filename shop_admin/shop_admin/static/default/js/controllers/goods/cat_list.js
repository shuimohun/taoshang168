var handle = {
    ...
    imageFmatter: function (val, opt, row) {
        if (row.cat_pic) {
            val = '<img src="' + row.cat_pic + '" style="width:40px;height:40px;">';
        } else {
            val = '&#160;';
        }
...
jQuery(document).ready(function ($) {
    ...
    function initEvent() {
        $_matchCon = $("#matchCon"),
            ...
            $("#search").on("click", function (a) {
                a.preventDefault();
                var b = "输入分类名称查询" === $_matchCon.val() ? "" : $.trim($_matchCon.val());
                //$("#grid").GridUnload();
    ...
    function initGrid() {
        ...
        $("#grid").jqGrid({
            url: SITE_URL + '?ctl=Category&met=lists&typ=json&type_number=goods_cat&is_delete=2&step=1',
            datatype: "json",
            autowidth: true,
            shrinkToFit: false,
            forceFit: true,
