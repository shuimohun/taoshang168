...
var handle = {
    ...
    is_display:function(val, opt, row) {
        if(row.property_is_display == 1){
            return '√';
        }else {
            return '×';
        }
...
function initBrand()
{
    //?ctl=Goods_Brand&met=listBrand&typ=json&isDelete=2&page=1&rows=100
    /*$.get('./index.php?ctl=Goods_Brand&met=getBrands&typ=json', function (b)
        {
            if (b.status == 200)
            {
                ...
                for (var i = 0; i < b.data.length; i++)
                {
                    ...
                    if (urlParam.type_id)
                    {
                        if (rowData.brand && rowData.brand[b_id])
                        {
                            b_str = "checked='checked'";
                            c_str = "";
                        }
...
function initGrid()
{
    var t = ["操作", "排序","是否显示", "属性名称", '属性内容', '数据格式'], e = [{
        name: "operate",
        width: 100,
        fixed: !0,
        align: "center",
        ...
        {name: "property_is_display", index: "property_is_display","formatter": handle.is_display, align: "center", width: 100},
            ...
            Public.ajaxPost('./index.php?ctl=Goods_Type&typ=json&met=' + (oper == 'addType' ? 'addType' : 'editType'), params, function (data)
            {
                if (data.status == 200)
                {
                    ...
                    if (oper == 'addType')
                    {
                        ...
                        window.location.href = SITE_URL +  '?ctl=Goods_Type&met=manage&typ=e&type_id=' + id;
                    }
