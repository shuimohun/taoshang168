...
function BBSPop() {
	var a = $("#yswb-tab"),
		b = ['<ul id="yswbPop">', '<li class="yswbPop-title">请选择您要进入的论坛</li>', '<li><strong>产品服务论坛</strong><p>在线会计，在线进销存操作问题咨询</p><a href="" target="_blank">进入论坛>></a></li>', '<li><strong>会计交流论坛</strong><p>会计实操，会计学习，会计资讯</p><a href="" target="_blank">进入论坛>></a></li>', "</ul>"].join("");
...
function getStores() {
	SYSTEM.isAdmin || SYSTEM.rights.CLOUDSTORE_QUERY ? Public.ajaxGet("http://www.taoshang168.com/wd/cloudStore.do?action=list", {}, function(a) {
...
function getLogistics() {
	SYSTEM.isAdmin || SYSTEM.rights.EXPRESS_QUERY ? Public.ajaxGet("http://www.taoshang168.com/wd/express.do?action=list", {}, function(a) {
...
$("#col-side").on("click", "[rel=pageTab]", function(a) {
	...
	if (2 === SYSTEM.siVersion && SYSTEM.isOpen) {
		var b, c = location.protocol + "//" + location.host + "/update_info.jsp",
			d = '您的单据分录已经录入达到300条，继续使用选择<a href="http://www.taoshang168.com/buy/invoicing/" target="_blank">购买产品</a>或者完善个人信息赠送1000条免费容量。';
		SYSTEM.isshortUser ? SYSTEM.isshortUser && (b = "http://www.taoshang168.com/service/user/set_password.jsp?updateUrl=" + encodeURIComponent(c) + "&warning=" + encodeURIComponent(d) + "&loginPage=http://www.taoshang168.com/buy/invoicing/", a.dialog({
			min: !1,
			max: !1,
			cancle: !1,
			lock: !0,
			...
			title: "完善个人信息",
			content: "url:" + b
		})) : (b = "http://www.taoshang168.com/service/user/phone_validate.jsp?updateUrl=" + encodeURIComponent(c) + "&warning=" + encodeURIComponent(d), a.dialog({
			min: !1,
			max: !1,
			cancle: !1,
			lock: !0,
	...
	a(), $("#skin-" + SYSTEM.skin).addClass("select").append("<i></i>"),
        $("#sysSkin").powerFloat({
		reverseSharp: !0,
		target: function() {
			return $("#selectSkin")
	...
	if ($("#scollUp").click(function() {
		...
		var c = [{
			...
			focus: !0,
			callback: function() {
				window.open("http://www.taoshang168.com/service/fee/renew.do?langOption=zh-CHS&accIds=" + SYSTEM.DBID)
			}
		}, {
			name: "下次再说"
		}],
