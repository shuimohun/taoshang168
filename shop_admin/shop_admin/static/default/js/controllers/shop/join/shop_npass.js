...
var THISPAGE = {
    ...
    loadGrid: function(){
        ...
        var colModel = [
            ...
            {name:'shop_status_name', label:'店铺状态',  width:100, align:"center"},
            {name:'shop_npass_content', label:'不通过原因',  width:150, align:"center"},
            {name:'shop_npass_image', label:'不通过图片',  width:300, align:"center",classes:'img_flied' },
            {name:'shop_npass_ctime', label:'添加时间',  width:250, align:"center"}
        ];
        this.mod_PageConfig.gridReg('grid', colModel);
        colModel = this.mod_PageConfig.conf.grids['grid'].colModel;
        $("#grid").jqGrid({
            url: SITE_URL + "?ctl=Shop_Npass&met=shop_npass&typ=json",
    ...
    var html_con = '<div class="operating" data-id="' + row.shop_id + '"><span class="ui-icon ui-icon-search" title="查看"></div>';
    return html_con;
};
...
var handle = {
      ...
      imgFmt: function (val, opt, row)
    {
        ...
        {
            if (row.shop_npass_image)
            {
                val = '<img src="' + row.shop_npass_image + '">';
            }
            else
            {
                val = '&#160;';
        ...
        $.dialog.confirm("此审核是审核用户的开店信息，只有审核通过，用户才能进行下一步付款，", function ()
        {
            Public.ajaxPost(SITE_URL + "?ctl=Shop_Manage&met=editShopStatus&typ=json", {shop_id: t}, function (e)
            {
                if (e && 200 == e.status)
                {
                    ...
                    location.href= SITE_URL + "?ctl=Shop_Manage&met=complement_order";
                }
                else
                {
