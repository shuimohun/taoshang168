...
var THISPAGE = {
    ...
    loadGrid: function(){
        ...
        var colModel = [
            ...
            {name:'renew_time', label:'续费时长（年）', width:100, align:"center"},
        ...
        function operFmattershop(val, opt, row) {
            if(row.status==0){
            var html_con = '<div class="operating" data-id="' + row.id + '"><span class="ui-icon ui-icon-search" title="查看"></span><span class="ui-icon ui-icon-trash" title="删除"></span><span class="ui-icon ui-icon-pencil" title="审核"></span></div>';
            }else{
               var html_con = '<div class="operating" data-id="' + row.id + '"><span class="ui-icon ui-icon-search" title="查看"></span><span class="ui-icon ui-icon-trash" title="删除"></span><span class="ui-icon ui-icon-pencil ui-icon-disabled" data-dis="1" title="审核"></span></div></div>';
            }
             return html_con;
        };
        ...
        $("#grid").on("click", ".operating .ui-icon-pencil", function (e)
        {
            ...
            if (Business.verifyRight("INVLOCTION_DELETE"))
            {
                ...
                handle.status(e)
                  ...
                  //跳转到店铺认证信息页面
        $('#grid').on('click', '.ui-icon-search', function(e) {
            e.stopPropagation();
            e.preventDefault();
            var id = $(this).parent().data("id");
            $.dialog({
                title: '查看店铺续签信息',
                content: "url:"+SITE_URL + '?ctl=Shop_Manage&met=getreopen&id=' + id,
                width: 1000,
