...
function initGrid()
{
    ...
    $("#grid").jqGrid({
        url: SITE_URL + "?ctl=Help_Base&met=helpBaseList&typ=json&isDelete=2",
        datatype: "json",
...
var handle = {
    operate: function (t, e)
    {
        if ("add" == t)
        {
            var i = "新增帮助", a = {oper: t, callback: this.callback};
        }
        else
        {
            var i = "修改帮助", a = {oper: t, rowData: $("#grid").data("gridData")[e], callback: this.callback};
        }
        $.dialog({
            title: i,
            content: 'url:'+SITE_URL + "?ctl=Help_Base&met=manage",
    ...
    }, del: function (t)
    {
        ...
        {
            Public.ajaxPost(SITE_URL + "?ctl=Help_Base&met=removeBase&typ=json", {help_id: t}, function (e)
            {
                if (e && 200 == e.status)
                {
...
$(function(){
    ...
    $source = $("#source").combo({
        data: "./index.php?ctl=Help_Base&met=helpGroupSon&typ=json",
        value: "id",
        text: "name",
        width: 110,
        ajaxOptions: {
        ...
        callback:{onChange:function (e) {
            if(e.id !== 0){
                ...
                $source.loadData("./index.php?ctl=Help_Base&met=helpGroupSon&typ=json&pid=" + e.id);
            }
        }}
    }).getCombo();
