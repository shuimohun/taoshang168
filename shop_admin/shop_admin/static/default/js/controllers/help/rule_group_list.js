var handle = {
    ...
    operate: function(oper, row_id) {
        if (oper == 'add' && !row_id) {
            ...
            var title = sprintf(_('修改分类 [%s]'), $("#grid").data('gridData')[row_id]['help_group_title']);
            var data = {
                oper: oper,
                rowId: row_id,
                rowData: $("#grid").data('gridData')[row_id],
        ...
        $.dialog({
            title: title,
            content: 'url:'+SITE_URL + "?ctl=Help_RuleGroup&met=manage",
            data: data,
    ...
    del: function(row_ids) {
        $.dialog.confirm('删除的将不能恢复，请确认是否删除？', function() {
            Public.ajaxPost(SITE_URL + '?ctl=Help_Group&met=removeGroup&typ=json', {
                id: row_ids
            }, function(data) {
...
var colModel = [{
    ...
    "name": "help_group_sort",
    "index": "help_group_sort",
    "label": "排序",
    "classes": "ui-ellipsis",
    "align": "center",
    "title": false,
    ...
    "name": "help_group_title",
    "index": "help_group_title",
    "label": " 分类名称",
    "classes": "ui-ellipsis",
    "align": "left",
    "title": false,
    ...
    "name": "help_group_parent_id",
    "index": "help_group_parent_id",
    "label": "父类id",
    "classes": "ui-ellipsis",
    "align": "center",
    "title": false,
...
jQuery(document).ready(function($) {
    ...
    function initGrid() {
        $("#grid").jqGrid({
            url: SITE_URL + '?ctl=Help_Group&met=helpGroupCat&typ=json&type=rule',
            datatype: "json",
            autowidth: true,
            shrinkToFit: false,
            forceFit: true,
            ...
            loadComplete: function(response) {
                if (response && response.status == 200) {
                    ...
                    for (var i = 0; i < data.items.length; i++) {
                        ...
                        item['id'] = item.help_group_id;
                        gridData[item.help_group_id] = item;
                    }
            ...
            "treeReader": {
                "parent_id_field": "help_group_parent_id",
