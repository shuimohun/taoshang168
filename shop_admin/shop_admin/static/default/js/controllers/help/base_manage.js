...
function initEvent(){
    ...
    group = $("#type").combo({
        data: SITE_URL + "?ctl=Help_Group&met=queryAllGroup&typ=json",
        value: "help_group_id",
...
function postData(oper, id){
    ...
    Public.ajaxPost( SITE_URL + '?ctl=Help_Base&typ=json&met=' + (oper == 'add' ? 'addHelpBase' : 'editHelpBase&help_id='+help_id) + '&help_group_id='+help_group_id, params, function(data){
        if (data.status == 200) {
            rowData = data.data;
