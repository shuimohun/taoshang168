 ...
 *
 */
class Plugin_Log implements YLB_Plugin_Interface
{
	function __construct()
	{
		...
		//第三个是插件所执行的方法
		YLB_Plugin_Manager::getInstance()->register('log', $this, 'addLog');
	}
	...
	public function addLog()
	{
		...
		if (true || isset($ccmd_rows[$_REQUEST['ctl']][$_REQUEST['met']]['log']))
		{
			...
			if ($rights_id)
			{
				$right = current($Rights_BaseModel->getBase($rights_id));
			}
			...
			if (Perm::$login)
			{
				...
				$data['log_date'] = date('Y-m-d'); // 日期


				$data['action_id']      = $rights_id; // 行为id == protocal_id -> rights_id
				$data['action_type_id'] = @$right['rights_parent_id'];
