...
(function() {
    window.IM = window.IM || {
            _appid: window.appID,
            _onUnitAccount: 'KF10089',
            // 多渠道客服帐号，目前只支持1个
            _3rdServer: 'http://123.57.230.158:8886/authen/',
            // 3rdServer，主要用来虚拟用户服务器获取SIG
            ...
            // pre的光标监控对象
            _pre_range_num: 0,
            // 计数，记录pre中当前光标位置，以childNodes为单位
            ...
            init: function() {
                ...
                if (200 == resp.code) { // 初始化成功
                    ...
                    $(".content_you").html(getQueryString('contact_yname'));
            ...
            initEmoji: function() {
                ...
                for (var i in emoji.show_data) {
                    ...
                    var content_emoji = '<li ' + 'onclick="IM.DO_chooseEmoji(\'' + i + '\', \'' + c[0][0] + '\')" ' + 'imtype="content_emoji">' + out + '</li>';
             ...
             * @private
             */
            _login: function(user_id) {
                var timestamp = IM._getTimeStamp();

                var flag = false; //是否从第三方服务器获取sig
                if (flag) {
                    IM._privateLogin(user_id, timestamp, function(obj) {
                    ...
                    var sig = hex_md5(IM._appid + user_id + timestamp + appToken);
                    IM.EV_login(user_id, sig, timestamp);
                }
            },
             ...
             * @constructor
             */
            EV_login: function(user_id, sig, timestamp) {
                //console.log("EV_login----" + user_id);
                var loginBuilder = new RL_YTX.LoginBuilder();
                loginBuilder.setType(IM._loginType);
                loginBuilder.setUserName(user_id);
                ...
                RL_YTX.login(loginBuilder, function(obj) {
                    ...
                    IM._user_account = user_id;
                    IM._username = user_id;
                    ...
                    // 添加客服号到列表中
                    IM.HTML_addContactToList(IM._onUnitAccount, IM._onUnitAccount, IM._contact_type_m, false, false, false, null, null, null);
                    ...
                    if (200 == obj.code) {
                        if (obj.state == 1) { //收到振铃消息
                            ...
                            if (im_contact.length <= 0) {
                                ...
                                IM.HTML_addContactToList(obj.caller, obj.caller, IM._contact_type_c, true, true, false, null, null, null);
            ...
            EV_resendMsg: function(obj) {
                ...
                if (1 == msgtype) { // 文本消息
                    ...
                    if ( !! oFile) {
                        ...
                        if (IM._contact_type_m == contact_type) {
                            IM.EV_sendToDeskAttachMsg(oldMsgid, oFile, msgtype, receiver, true);
                        } else {
            ...
            EV_sendAttachMsg: function(oldMsgid, file, type, receiver, isresend) {
                ...
                var msgid = RL_YTX.sendMsg(obj, function(obj) {
                    setTimeout(function() {
                        ...
                        msg.find('p[imtype="msg_attach"]').css('display', 'block');
                    ...
                    setTimeout(function() {
                        var msg = $(document.getElementById(receiver + "_" + obj.msgClientNo));
                        msg.find('span[imtype="resend"]').css('display', '');
                        msg.find('div[class="bar"]').parent().css('display', 'none');
                        msg.find('p[imtype="msg_attach"]').css('display', '');
                        Public.tips.warning("错误码： " + obj.code + "; 错误描述：" + obj.msg);
                    }, 100)
            ...
            EV_getMyInfo: function() {
                RL_YTX.getMyInfo(function(obj) {
                    ...
                    user_logo = '';
                    receiver_logo = '';
                    content_you = getQueryString('contact_yname');

                    $.post(ImApiUrl + "?ctl=UserApi&met=getUserInfo&typ=json", {
                        "k": k,
                        "u": u
                    }, function(data) {
                        if (data.status == 200) {
                            user_logo = data.data.user_avatar;
                            /*receiver_name = data.data.nickname ? data.data.nickname : data.data.user_account;*/
                        }
                    });
            ...
            EV_createGroup: function(groupName, permission) {
                ...
                RL_YTX.createGroup(obj, function(obj) {
                    ...
                    if (permission == 4) { // 如果是讨论组，需要在讨论组创建成功之后随即添加账号
                        // 左侧名称列表
                        IM.HTML_addContactToList(groupId, groupName, IM._contact_type_g, true, true, false, IM._user_account, 1, 1);
                        IM.EV_inviteGroupMember(groupId, permission, true);
                        $('#im_add').find('input[imtype="im_add_group"]').val('');
                        IM.HTML_pop_hide();
                    } else {
                        // 左侧名称列表
                        IM.HTML_addContactToList(groupId, groupName, IM._contact_type_g, true, true, false, IM._user_account, 1, 2);
                        IM.HTML_pop_hide();
                    }
                }, function(obj) {
                    $("#createGroup_bt").removeAttr("disabled");
            ...
            EV_getGroupDetail: function(groupId, isowner, target) {
                ...
                RL_YTX.getGroupDetail(objBuilder, function(obj) {
                    ...
                    if (target == null) { // 推送的target参数为空
                        ...
                        // 展示群组的详细信息
                        IM.DO_pop_show_help_GroupDetail(obj, groupId, getTarget);
                        // 获取成员列表并展示
            ...
            EV_getGroupList: function() {
                ...
                RL_YTX.getGroupList(obj, function(obj) {
                    for (var i in obj) {
                        ...
                        IM.HTML_addContactToList(groupId, groupName, IM._contact_type_g, false, false, true, owner, isNotice, target);
                    }
                }, function(obj) {
                    Public.tips.warning("错误码： " + obj.code + "; 错误描述：" + obj.msg);
                });
            ...
            EV_updateGroupInfo: function(groupId) {
                ...
                RL_YTX.modifyGroup(builder, function() {
                    ...
                    IM.HTML_addContactToList(groupId, groupName, IM._contact_type_g, false, true, true, null, null, null);
                    IM.HTML_pop_hide();
                }, function(obj) {
                    Public.tips.warning("错误码： " + obj.code + "; 错误描述：" + obj.msg);
                });
            ...
            EV_confirmInviteJoinGroup: function(you_sender, groupName, version, invitor, groupId, confirm) {
                ...
                RL_YTX.confirmInviteJoinGroup(obj, function() {
                    ...
                    if (2 == confirm) {
                        ...
                        IM.HTML_addContactToList(groupId, groupName, IM._contact_type_g, false, isShow, false, null, null, null);
                    }
                }, function(obj) {
                    Public.tips.warning("错误码： " + obj.code + "; 错误描述：" + obj.msg);
                });
            ...
            DO_pop_show_help_GroupDetail: function(obj, groupId, target) {
                ...
                if (isowner || target == 1) {
                    ...
                    str = '<span class="pull-left" maxlength="128">' + obj.declared + '</span>';
                    $('#pop').find('span[imtype="im_pop_group_declared"]').html(str);
                }
                if (obj.isNotice == 1) {
            ...
            DO_push_createMsgDiv: function(obj) {
                ...
                var b_current_contact_you = IM.DO_createMsgDiv_Help(you_sender, name, b_isGroupMsg); //false:不是当前联系人  true:当前联系人
                ...
                if (0 == obj.mcmEvent) { // 0普通im消息
                    ...
                    if (1 == msgType) {
                        ...
                        if (isFireMsg) {
                            str = '<p style="display:inline"><img fireMsg="yes" onclick="IM.DO_pop_phone(\'' + you_sender + '\', \'' + version + '\')" videourl="' + urlReal + '" src="' + urlShow + '" style="max-width:' + imgWid + 'px;max-height:' + imgHei + 'px;display:none;cursor:pointer" />' + '<span style="font-size: small;margin-left:15px;">' + size + '</span></p>';
                        } else {
                            str = '<p style="display:inline"><img onclick="IM.DO_pop_phone(\'' + you_sender + '\', \'' + version + '\')" videourl="' + urlReal + '" src="' + urlShow + '" style="cursor:pointer;max-width:' + imgWid + 'px;max-height:' + imgHei + 'px;" />' + '<span style="font-size: small;margin-left:15px;">' + size + '</span></p>';
                        }
                        ...
                        if (isFireMsg) {
                            ...
                            var str = '<p><img src="' + url + '" id="img" style="cursor:pointer;max-width:' + imgWid + 'px; max-height:' + imgHei + 'px;" onclick="IM.DO_pop_phone(\'' + you_sender + '\', \'' + version + '\')"/></p>';
                        }
                        ...
                        if (isFireMsg) {
                            str = '<p style="display:inline"><a fireMsg="yes" href="' + url + '" target="_blank">' + '<span>' + '<img style="width:32px; height:32px; margin-right:5px; margin-left:5px;" src="../css/img/im/attachment_icon.png" />' + '</span>' + '<span>' + fileName + '</span>' //+ '<span style="font-size: small;margin-left:15px;">'+size+'</span>'
                                + '</a>' + '<span style="font-size: small;margin-left:15px;">' + size + '</span></p>';
                        } else {
                            str = '<p style="display:inline"><a href="' + url + '" target="_blank">' + '<span>' + '<img style="width:32px; height:32px; margin-right:5px; margin-left:5px;" src="../css/img/im/attachment_icon.png" />' + '</span>' + '<span>' + fileName + '</span>' //+ '<span style="font-size: small;margin-left:15px;">'+size+'</span>'
                                + '</a>' + '<span style="font-size: small;margin-left:15px;">' + size + '</span></p>';
                        }
                    }
                    ...
                    if (1 == msgType) {
                        ...
                        var fileName = obj.msgFileName;
                        if (isFireMsg) {
                            str = '<a fireMsg="yes" style="display:none" href="' + url + '" target="_blank">' + '<span>' + '<img style="width:32px; height:32px; margin-right:5px; margin-left:5px;" src="../css/img/im/attachment_icon.png" />' + '</span>' + '<span>' + fileName + '</span>' + '<span style="font-size: small;margin-left:15px;">' + size + '</span>' + '</a>';
                        } else {
                            str = '<a href="' + url + '" target="_blank">' + '<span>' + '<img style="width:32px; height:32px; margin-right:5px; margin-left:5px;" src="../css/img/im/attachment_icon.png" />' + '</span>' + '<span>' + fileName + '</span>' + '<span style="font-size: small;margin-left:15px;">' + size + '</span>' + '</a>';
                        }
                    }
            ...
            DO_pop_phone: function(you_sender, version, obj) {
                ...
                if ( !! videoUrl) {
                    ...
                    str = '<video controls="controls" preload="auto" height="' + showHei + 'px" style="position:relative;top:-20px;left:0px;">' + '<source src="' + videoUrl + '" type="video/' + type + '" /></video>';
            ...
            DO_pop_photo_up: function() {
                ...
                if ($("#carousels").find("img")) {
                    $("#carousels").find("img").css('max-height', (showHei - 30) + "px").css('max-width', ($(window).width() - 50) + "px");
                };
                var q = 1;
            ...
            DO_notice_createMsgDiv: function(obj) {
                ...
                if (1 == auditType) {
                    ...
                    noticeContent = you_msgContent;
                    if ( !! obj.groupName) {
                        IM.HTML_addContactToList(groupId, obj.groupName, IM._contact_type_g, false, isShow, true, null, null, null);
                    }
            ...
            DO_procesGroupNotice: function(type, groupId, memberId, memberName, groupName, ext) {
                ...
                if (type == 4) {
                    ...
                    IM.HTML_popAddMember(groupId, memberId, memberName, isowner, target);
                } else if (type == 10) {
                    var obj = $('#pop').find('div[im_isowner]');
                    var isowner = obj.attr('im_isowner');
                    if ( !! groupName) {
            ...
            HTML_pushMsg_addHTML: function(msgtype, you_sender, version, content_type, b, name, you_msgContent) {
                ...
                if (you_msgContent) {
                    $.post(ImApiUrl + "?ctl=UserApi&met=getUserInfosByName&typ=json", {
                        "user_name": getQueryString('contact_yname'),
                        "k": k,
                        "u": u
                    }, function(data) {
                        if (data.status == 200) {
                            ...
                            if (data.data.user_avatar) {
                                ...
                                var str = '<div  msg="msg" im_carousel="' + carou + '" im_msgtype="' + msgtype + '" id="' + you_sender + '_' + version + '" content_type="' + content_type + '" content_you="' + you_sender + '" class="chatinterfacelist-left clearfix alert-left" style="display:' + ((b) ? 'block' : 'none') + '">'
                                ...
                                receiver_logo_str = '';
                                var str = '<div msg="msg" im_carousel="' + carou + '" im_msgtype="' + msgtype + '" id="' + you_sender + '_' + version + '" content_type="' + content_type + '" content_you="' + you_sender + '" class="alert-left" style="display:' + ((b) ? 'block' : 'none') + '">'
                    ...
                    if (you_msgContent.indexOf("fireMsg") > -1) { //fireMsg="yes"
                        ...
                        var fireMsgStr = '<img style="cursor:pointer;max-width:' + imgWid + 'px; max-height:' + imgHei + 'px; margin-right:5px; margin-left:5px;" ' + 'src="assets/img/fireMessageImg.png" onclick="IM.DO_showFireMsg(\'' + id + '\',\'' + msgtype + '\')" />';
            ...
            HTML_pushCall_addHTML: function(you_sender, callId, you_msgContent) {
                ...
                var str = '<div msg="msg" id="' + you_sender + '_' + callId + '" content_you="' + you_sender + '" class="alert-left " style="display:' + ((b) ? 'block' : 'none') + '">' + '<code style="max-width:70%;word-break:keep-all;text-overflow:ellipsis;overflow: hidden;">' + you_sender + ':</code>&nbsp;<p class="name"> ' + name + '</p>' + you_msgContent + '</div>';
                $('#im_content_list').append(str);
            ...
            HTML_pushMsg_addPreHTML: function(msgtype, you_sender, version, content_type, b, name, you_msgContent) {
                var carou = '';
                if (msgtype == 4 || msgtype == 3) {
                    carou = "real";
                };
                var str = '<div msg="msg" im_carousel="' + carou + '" im_msgtype="' + msgtype + '" id="' + you_sender + '_' + version + '" content_type="' + content_type + '" content_you="' + you_sender + '" class=" alert-left " style="display:' + ((b) ? 'block' : 'none') + '">' + '<code style="max-width:70%;word-break:keep-all;text-overflow:ellipsis;overflow: hidden;">' + name + ':</code>&nbsp;<pre style="font-size:12px;"> ' + name + '</pre>' + you_msgContent + '</div>';
            ...
            HTML_sendMsg_addHTML: function(msg, msgtype, msgid, content_type, content_you, im_send_content) {
                ...
                if (msgtype != 6) { //附件的要等上传结束才能处理，需单独处理
                    _user_account = getCookie('user_account');
                    IM.send_to_server_bysun(_user_account, content_you, msgid, im_send_content, msgtype);
                }
                if (user_logo) {
                ...
                var str = '<div contactor="sender" im_carousel="' + carou + '" msg="' + msg + '" im_msgtype="' + msgtype + '" id="' + content_you + '_' + msgid + '" content_type="' + content_type + '" content_you="' + content_you + '" class="chatinterfacelist-right clearfix alert-right" style="display:' + display + '">' + '<span imtype="resend" class="add-on" onclick="IM.EV_resendMsg(this)"' + ' style="display:none; cursor:pointer; position: relative; left: -40px; top: 0px;"><i class="icon-repeat"></i></span>' + '<div class="hd-portrait"><img ' + user_logo_str + '></div><div class="chatcontent"><div class="triangle"></div>' + im_send_content + '</div></div>';
                $('#im_content_list').append(str);
            ...
            HTML_sendMsg_addPreHTML: function(msg, msgtype, msgid, content_type, content_you, im_send_content) {
                ...
                var str = '<div contactor="sender" im_carousel="' + carou + '" msg="' + msg + '" im_msgtype="' + msgtype + '" id="' + content_you + '_' + msgid + '" content_type="' + content_type + '" content_you="' + content_you + '" class="alert-right" style="display:' + display + '">' + '<span imtype="resend" class="add-on" onclick="IM.EV_resendMsg(this)"' + ' style="display:none; cursor:pointer; position: relative; left: -40px; top: 0px;"><i class="icon-repeat"></i></span>' + '<code class="pull-right" style="max-width:70%;word-break:keep-all;text-overflow:ellipsis;overflow: hidden;">&nbsp;:' + IM._username + '</code>' + im_send_content + '</div>';
            ...
            /*
             聊天记录发送到数据库
             */
            send_to_server_bysun: function(_user_account, content_you, msgid, im_send_content, content_type) {
                $.post(ImApiUrl + "?ctl=Api_Chatlog&met=add&typ=json", {
                    u: _user_account,
                    to: content_you,
                    msgid: msgid,
                    content: im_send_content,
                    type: content_type
                }, function() {
            ...
            DO_checkContact: function(contactVal) {
                ...
                if (contactVal.indexOf("@") > -1) {
                    ...
                    if (regx1.exec(contactVal) == null) {
                        IM.HTML_showAlert('alert-error', '联系人只能使用数字、_、-、大小写英文组成; 如果是跨应用则应用id长度为32位由数字或大小写英文组成');
                        return false;
                    }
                }
                return true;
             ...
             * @constructor
             */
            HTML_addContactToList: function(contactVal, contactName, content_type, b, bb, bbb, owner, isNotice, target) {
                ...
                var str = '<li onclick="IM.DO_chooseContactList(\'' + content_type + '\', \'' + contactVal + '\')" id="im_contact_' + contactVal + '" im_isnotice="' + isNotice + '" contact_type="' + content_type + '" contact_you="' + contactVal + '" class="' + active + '"  style="display:' + dis + '">' + '<a href="javascript:void(0);">' + '<span contact_style_type="name">' + contactName + '</span>';
                ...
                str += '<span contact_style_type="warn" class="badge badge-warning pull-right" style="margin-top:3px; margin-right:10px; display:none;">0</span>' + '<span contact_style_type="onlineState" class="badge pull-right" style="margin-top:6px; margin-right:10px; display:none;"></span>' + '</a>' + '</li>';
                $('#im_contact_list').prepend(str);
            ...
            DO_html_create: function(groupName, permission) {
                ...
                str += '</table></div></td></tr></table></div>' + '<div class="modal-footer">' + '<button href="#" id= "createGroup_bt" class="btn btn-primary" onclick="IM.EV_createGroup(\'' + groupName + '\',' + permission + ');" > 确定 </button>' + '<a href="javascript:void(0);" class="btn" onclick="IM.HTML_pop_hide()">取消</a>' + '</div>' + '</div>';
            ...
            DO_pop_show_help_GroupMemberList: function(obj, groupId, target) {
                ...
                for (var i in obj) {
                    ...
                    if (member.role == 1 || member.role == 2) {
                        ...
                        str += '<tr contact_you="' + member.member + '">' + '<td><span class="pull-left"><span style="color: #b94a48">[管理员]</span>&nbsp;&nbsp;' + '<a href="javascript:void(0);" onclick="IM.DO_editNickName(this)" ' + ' style="max-width:70%;word-break:keep-all;text-overflow:ellipsis;overflow: hidden;text-decoration:none">' + member.nickName + '</a><input style="display:none;padding:0px;margin-bottom:0px" type="text" onblur="IM.DO_checkNick(this);" /></span></td>' + '</tr>';

                    } else {
                        str += '<tr contact_you="' + member.member + '">' + '<td>' + '<span class="pull-left"><span style="color: #006dcc">[成员]</span>&nbsp;&nbsp;' + '<a href="javascript:void(0);"  onclick="IM.DO_editNickName(this)" ' + 'style="max-width:50%;word-break:keep-all;text-overflow:ellipsis;overflow: hidden;text-decoration:none">' + member.nickName + '</a><input style="display:none;padding:0px;margin-bottom:0px" type="text" onblur="IM.DO_checkNick(this);" /></span>';
                        if (isowner) {
             ...
             * @constructor
             */
            HTML_popAddMember: function(groupId, memberId, memberName, isowner, target) {
                if ($('#pop').find('tr[contact_you=' + memberId + ']').length > 0) {
                    return;
                }
                var str = '<tr contact_you="' + memberId + '">' + '<td>' + '<span class="pull-left"><span style="color: #006dcc">[成员]</span>&nbsp;&nbsp;' + '<a href="javascript:void(0);"  onclick="IM.DO_editNickName(this)" ' + 'style="max-width:50%;word-break:keep-all;text-overflow:ellipsis;overflow: hidden;text-decoration:none">' + memberName + '</a><input style="display:none;padding:0px;margin-bottom:0px" type="text" onblur="IM.DO_checkNick(this);" /></span>';
                if (isowner && isowner == 'true') {
            ...
            DO_pop_show: function(groupId, isowner, target) {
                ...
                str += '</div>' + '<div class="modal-body">' + '<table class="table table-bordered">' + '<tr>' + '<td>' + '<div style="height:auto;">' + '<table imtype="im_pop_members" class="table table-striped">' + '</table>' + '</div>' + '<div style="height:auto; padding-bottom: 0px;">' + '<table imtype="im_pop_members_add" style="display:none;" class="table table-striped">' + '</table>' + '</div>' + '<a href="javascript:void(0);" class="btn pull-right" style="margin-left:10px;" onclick="IM.DO_cleanChatHis(\'' + groupId + '\')">清除聊天记录</a>' + '</td>' + '</tr>' + '<tr>' + '<td>' + '<span class="pull-left">消息免打扰：</span>' + '<span class="pull-right" imtype="im_pop_group_notice">' + '<a href="javascript:void(0);" class="btn btn-primary" style="margin-left:10px;">开启</a>' + '<a href="javascript:void(0);" class="btn" style="margin-left:10px;">关闭</a>' + '</span>' + '</td>' + '</tr>';
                ...
                str += '<div class="pull-right" style="width: 75%;" imtype="im_pop_group_name">' + '<input class="pull-right" type="text" style="width:95%;" />' + '</div>' + '</td>' + '</tr>' + '<tr>' + '<td>' + '<span class="pull-left" style="width: 25%;">公告：</span>' + '<span class="pull-right" style="width: 75%;" imtype="im_pop_group_declared">' + '<textarea class="pull-left" style="width:95%;"></textarea>' + '</span>' + '</td>' + '</tr>' + '</table>';

                if (target == 2) {
                    if (!isowner) {
                        str += '<div style="text-align: center;">' + '<button class="btn btn-primary" type="button" onclick="IM.EV_quitGroup(\'' + groupId + '\')"> 退出群组 </button>' + '</div>';
                    }
                    str += '</div>';
                    if (isowner) {
                        str += '<div class="modal-footer">' + '<a href="javascript:void(0);" class="btn" onclick="IM.EV_dismissGroup(\'' + groupId + '\')"> 解散群组 </a>' + '<a href="javascript:void(0);" class="btn btn-primary" onclick="IM.EV_updateGroupInfo(\'' + groupId + '\')">保存修改</a>' + '</div>';
                    }
                }
                if (target == 1) {

                    str += '</div><div class="modal-footer">' + '<a href="javascript:void(0);" class="btn" onclick="IM.EV_quitGroup(\'' + groupId + '\')">退出讨论组</a>' + '<a href="javascript:void(0);" class="btn btn-primary" onclick="IM.EV_updateGroupInfo(\'' + groupId + '\')">保存修改</a>' + '</div>';
                }
                str += '</div>';
            ...
            DO_sendMsg: function() {
                ...
                content_yname = content_yname ? content_yname : getQueryString('contact_yname');

                if (content_yname && content_you && content_type) {
                    b = true;
                }
            ...
            DO_im_image_file: function() {
                ...
                var str = '<div class="progress progress-striped active">' + '<div class="bar" style="width: 20%;"></div>' + '</div>' + '<p imtype="msg_attach">' + '<img imtype="msg_attach_src" src="#" style="max-width:' + imgWid + 'px; max-height:' + imgHei + 'px;" onclick="IM.DO_pop_phone(\'' + content_you + '\',\'' + '' + '\',this)" />' /*+ 'px;"  />'*/
                    + '<input imtype="msg_attach_resend" type="file" accept="image/*" style="display:none;margin: 0 auto;" onchange="IM.DO_im_image_file_up(\'' + content_you + '_' + msgid + '\', \'' + msgid + '\',null)">' + '</p>';
            ...
            DO_im_attachment_file: function() {
                ...
                var str = '<div class="progress progress-striped active">' + '<div class="bar" style="width: 40%;"></div>' + '</div>' + '<p imtype="msg_attach">' + '<a imtype="msg_attach_href" href="javascript:void(0);" target="_blank">' + '<span>' + '<img style="width:32px; height:32px; margin-right:5px; margin-left:5px;" src="../css/img/im/attachment_icon.png" />' + '</span>' + '<span imtype="msg_attach_name"></span>' + '</a>' + '<span style="font-size: small;margin-left:15px;"></span>' + '<input imtype="msg_attach_resend" type="file" accept="" style="display:none;margin: 0 auto;" onchange="IM.DO_im_attachment_file_up(\'' + content_you + '_' + msgid + '\', \'' + msgid + '\')">' + '</p>';
                // 添加右侧消息
            ...
            DO_im_attachment_file_up: function(id, oldMsgid) {
                ...
                if ($("#camera_button").find("i").hasClass("icon-picture")) {
                    ...
                    if ("image" == oFile.type.substring(0, oFile.type.indexOf("/"))) {
                        ...
                        var str = '<img imtype="msg_attach_src" src="' + url + '" style="max-width:' + imgWid + 'px; max-height:' + imgHei + 'px;" ' + '  onclick="IM.DO_spop_phone(\'' + url + '\',this)"/>';
            ...
            _snapshot: function() {
                ...
                if ("174010" == resultObj.code) { //没有调用applay方法
                    ...
                    // 初始化右侧对话框消息
                    var str1 = '<div class="progress progress-striped active">' + '<div class="bar" style="width: 20%;"></div>' + '</div>' + '<p imtype="msg_attach">' + '<img imtype="msg_attach_src" src="' + url + '" onclick="IM.DO_pop_phone(\'' + content_you + '\', \'' + '' + '\',this)" style="cursor:pointer;max-width:' + imgWid + 'px;max-height:' + imgHei + 'px;" />' + '<object style="display:none"></object>' + '</p>';
		 ...
		 *            0普通im消息 1 start消息 2 end消息 53发送mcm消息 }
            ...
            EV_getHistoryMessage: function(firstMsg) {
                ...
                RL_YTX.getHistoryMessage(getHistoryMessageBuilder, function(obj) {
                    var windowWid = $(window).width();
                    var imgWid = 0;
                    var imgHei = 0;
                    if (windowWid < 666) {
                    ...
                    for (var i = 0; i < obj.length; i++) {
                        ...
                        if (msg.msgType == 3) {
                            str = '<img onclick="IM.DO_pop_phone(\'' + content_you + '\', \'' + version + '\')" ' + 'videourl="' + msg.msgFileUrl + '" src="' + msg.msgFileUrlThum + '" ' + 'style="max-width:' + imgWid + 'px;max-height:' + imgHei + 'px;" />';
                        };
                        if (msg.msgType == 4) {
                            str = '<p imtype="msg_attach">' + '<img imtype="msg_attach_src" src="' + msg.msgFileUrl + '" style="max-width:' + imgWid + 'px;max-height:' + imgHei + 'px;" />' + '</p>';
                        };
                        if (msg.msgType == 6) {
                            str = '<p imtype="msg_attach">' + '<a imtype="msg_attach_href" href="' + msg.msgFileUrl + '" target="_blank">' + '<span>' + '<img style="width:32px; height:32px; margin-right:5px; margin-left:5px;" src="../css/img/im/attachment_icon.png" />' + '</span>' + '<span imtype="msg_attach_name">' + msg.msgFileName + '</span>' + '</a>' + '</p>';
                        };
            ...
            DO_inviteCall: function(callType) {
                ...
                var callId = RL_YTX.makeCall(makeCallBuilder, function() {
                ...
                if (windowWidth < 666) {
                    ...
                    if (type == 1) {
                        ...
                        if (!document.getElementById("cancelVoipCall")) {
                            var str = '<div style="width:100%;height:30px;background-color:black;margin-top:-7px;' + 'padding-top:5px;padding-bottom:5px;"><a href="javascript:void(0);" id="cancelVoipCall" ' + 'class="btn" >取消</a><a href="javascript:void(0);" id="acceptVoipCall" ' + 'class="btn btn-primary" >接受视频</a><a href="javascript:void(0);" id="refuseVoipCall" ' + 'class="btn" >拒绝视频</a></div>';
                        ...
                        if (!document.getElementById("cancelVoiceCall")) {
                            var audiostr = '<div style="width:100%;height:30px;background-color:black;margin-top:-7px;' + 'padding-top:10px;background:transparent;"><a href="javascript:void(0);" id="cancelVoiceCall" ' + 'class="btn " >取消</a><a href="javascript:void(0);" id="acceptVoiceCall" ' + 'class="btn btn-primary" >接受音频</a><a href="javascript:void(0);" id="refuseVoiceCall" ' + 'class="btn " >拒绝音频</a></div>';
            ...
            DO_answerVoipCall: function(callId, caller, called, acceptType) {
                ...
                RL_YTX.accetpCall(acceptCallBuilder, function() {
            ...
            _sendRecorder: function(blob) {
                // 初始化右侧对话框消息
                var str1 = '<div class="progress progress-striped active">' + '<div class="bar" style="width: 20%;"></div>' + '</div>' + '<p imtype="msg_attach">' + '<audio controls="controls" src="' + blob.url + '" ></audio>' + '<object style="display:none"></object>' + '</p>';
                var msgid = new Date().getTime();
                var content_type = '';
                var content_you = '';
