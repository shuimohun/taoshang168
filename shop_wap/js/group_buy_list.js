...
function get_list()
{
    ...
    if(groupbuy_type == 1){
        $.getJSON(ApiUrl + "?ctl=GroupBuy&met=groupBuyList&typ=json"+ window.location.search.replace("?", "&"), param, function (e)
        {
            if (!e)
            {
                e = [];
        ...
        $.getJSON(ApiUrl + "?ctl=GroupBuy&met=vrGroupBuyList&typ=json"+ window.location.search.replace("?", "&"), param, function (e)
        {
            if (!e)
            {
                e = [];
