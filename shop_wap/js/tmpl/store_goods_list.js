...
$(function () {
    ...
    $("#sort_salesnum").click(function () {
        ...
        $("#sort_inner").addClass("hide").find("a").removeClass("cur");
        $("#search_adv").attr('class');
        if($("#search_adv").attr('class') == 'current')
        {
    ...
    $("#product_list").on("click", '[nc_type="goods_more_link"]', function () {
        ...
        $.ajax({
            type: "post",
            url: ApiUrl + "?ctl=Buyer_Favorites&met=getGoodsFI&typ=json",
...
function get_list()
{
    ...
    $.getJSON(ApiUrl + "?ctl=Shop&met=goodsList&typ=json" + window.location.search.replace("?", "&"), param, function (e)
    {
        if (!e)
        {
            e = [];
