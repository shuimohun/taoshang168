...
$(function () {
    ...
    if(tab)
    {
        ...
        $(tab).parent().parent().addClass("selected").siblings().removeClass("selected");
        $("#storeindex_con,#allgoods_con,#newgoods_con,#storeactivity_con").hide();
        window.scrollTo(0, 0);
        var type = $(tab).parent().attr("data-type");
        switch (type)
        {
            case"allgoods":
    ...
    $("#goods_cla").attr("href","store_search.html?shop_id=" + e);
    ...
    $.ajax({
        type: "post",
        url: ApiUrl + "?ctl=Shop&met=getStoreInfo&typ=json",
    ...
    $("#goods_rank_tab").find("a").click(function () {
        ...
        $("[nc_type='goodsranklist']").addClass('visibility');

      // $("#goodsrank_" + t).show();
        $("#goodsrank_" + t).removeClass('visibility');
      ...
      if ($("#goodsrank_" + t).html()) {
        ...
        $.ajax({
            type: "post",
            url: ApiUrl + "?ctl=Shop&met=goodsList&typ=json",
            data: {id: e, order: o, pagesize: s, sort: 'desc'},
            dataType: "json",
            success: function (e) {
                if (e.status == 200) {
                    console.log(e.data);
                    ...
                    var swiper = new Swiper('.top-list .swiper-container', {
                        pagination: '.top-list .swiper-pagination',
                        slidesPerView: 3,
    ...
    function s() {
        ...
        o.loadInit({
            url: ApiUrl + "?ctl=Shop&met=goodsList&order=common_sell_time&sort=desc&typ=json",
            getparam: t,
            tmplid: "newgoods_tpl",
    ...
    function r() {
        $.ajax({
            type: "post",
            url: ApiUrl + "?ctl=Shop&met=getShopPromotion&typ=json",
            data: {shop_id: e},
            dataType: "json",
            success: function (t) {
                t.data.shop_id = e;
...
function LikeAndHotGoods(e)
{
    $.ajax({
        url:ApiUrl+'?ctl=Shop&met=getLikeAndHot&typ=json',
        data:{shop_id:e},
        type:'post',
        dataType:'json',
        success:function(r)
        {
            if(r.status == 200)
            {
                var data = r.data;
                console.log(data);
                var hotHtml = template.render('hot_goods',data);
                $('.hot_goods').html(hotHtml);
                var likeHtml = template.render('like_shop_goods',data);
                $('.like_shop_goods').html(likeHtml);
            }
        }
    })
}
