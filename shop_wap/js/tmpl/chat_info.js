...
$(function ()
{
    $.getJSON(ApiUrl + "?ctl=Buyer_Message&met=getNodeInfo&typ=json", {k: key, u: getCookie('id'), u_id: t_id, chat_goods_id: chat_goods_id}, function (t)
    {
        checkLogin(t.login);
    ...
    var init_chat_data = function (e)
    {
        ...
        userInfo = e.userInfo;
        // $("h1").html(userInfo.store_name != "" ? userInfo.store_name : userInfo.user_name);
        $("h1").html(userInfo.user_name);
        ...
        */
        ...
        function s()
        {
            ...
            */
            ...
            $.ajax({
                type: "post", url: ApiUrl + "?ctl=Buyer_Message&met=message&op=get_chat_msg&typ=json", data: {user_id:t_id, k: key, u: getCookie('id')}, dataType: "json", success: function (e)
                {
                    if (e.status == 200)
                    {
                        get_msg(e.data);
            ...
            function s(e)
            {
                if (true || t === 1)
                {
                    var send_msg_obj = e;
                    $.ajax({
                        type: "post", url: ApiUrl + "?ctl=Buyer_Message&met=addMessageDetail&typ=json", data: e, dataType: "json", success: function (e)
                        {
                            if (e.status == 200)
                            {
            ...
            $("#submit").click(function ()
            {
                ...
                s({k: key, u: getCookie('id'), t_id: t_id, user_message_receive: userInfo.user_name, user_message_content: e, chat_goods_id: chat_goods_id});
        ...
        $("#chat_msg_log").click(function ()
        {
            $.ajax({
                type: "post",
                url: ApiUrl + "?ctl=Buyer_Message&met=message&op=get_chat_msg&typ=json&page=50",
        ...
        function n(e, user)
        {
            ...
            var result  = reg.test(e.t_msg);
            if( result )
            {
                var content = e.t_msg.split("%_%");
                var title;
                if(content[0].length > 23 )
                {
                    title  = content[0].substr(0,23)+'...';
                }
                else
                {
                    title = content[0];
                ...
                t = '<dl class="' + e.class + '">' +
                        '<dt>' +
                            '<img onerror="this.src=\'../img/robot-ss.png\'" src="' + user[e.user_message_send_id].user_logo + '" alt=""/><i></i>' +
                        '</dt>' +
                        '<dd>' +
                            '<a class="share_a" href="'+content[3]+'">' +
                                '<h4 style="font-size: 16px;">'+title+'</h4>' +
                                '<img onerror="this.src=\'../img/robot-ss.png\'" src="'+content[1]+'"  />' +
                                '<p>'+content[2]+'</p>' +
                            '</a>' +
                        '</dd>' +
                    '</dl>';
            ...
            {
                 t = '<dl class="' + e.class + '"><dt><img onerror="this.src=\'../img/robot-ss.png\'" src="' + user[e.user_message_send_id].user_logo + '" alt=""/><i></i></dt><dd>' + e.t_msg + '</dd></dl>';
            }
            if( e.class == "msg-other" )
            {
                $("#chat_msg_html").prepend(t);
            }
            else
            {
        ...
        function c(e)
        {
            if (typeof smilies_array !== "undefined")
            {
                ...
                for (var t in smilies_array[1])
                {
                    ...
                    var i = '<img title="' + a[6] + '" alt="' + a[6] + '" src="' + resourceSiteUrl + "/images/smilies/images/" + a[2] + '">';
                    e = e.replace(s, i)
                }
            }
            return e
