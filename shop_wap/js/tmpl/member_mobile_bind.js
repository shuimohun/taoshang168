$(function ()
{
    ...
    $.ajax({
        type: "get", url: ApiUrl + "?ctl=Buyer_User&met=getUserInfo&typ=json", data: {k: e, u: getCookie('id')}, dataType: "json", success: function (e)
        {
            console.info(e);
            if (e.status = 200)
            {
    ...
    $("#send").click(function ()
    {
        if ($.sValid())
        {
            ...
            $.ajax({
                type: "post",
                url: ApiUrl + "?ctl=Buyer_User&met=getMobileYzm&typ=json",
                    ...
                    if (e.status != 250)
                    {
                        ...
                        var a = setInterval(function ()
                        {
                            ...
                            if (t == 0)
                            {
                                ...
                                $(".code-countdown").hide();
                                clearInterval(a);
                                //$("#codeimage").attr("src", ApiUrl + "?act=seccode&op=makecode&k=" + $("#codekey").val() + "&t=" + Math.random());
                                //$("#captcha").val("")
                            }
                            else
                            {
    ...
    $("#nextform").click(function ()
    {
        ...
        {
            $.ajax({
                type: "post", url: ApiUrl + "?ctl=Buyer_User&met=editMobileInfo&typ=json", data: {k: e, u: getCookie('id'), yzm: a, user_mobile:mobile}, dataType: "json", success: function (e)
                {
                    if (e.status == 200)
                    {
