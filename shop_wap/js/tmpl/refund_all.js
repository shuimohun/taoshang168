...
$(function() {
    ...
    $.getJSON(ApiUrl + "?ctl=Buyer_Service_Return&met=index&act=add&typ=json", {
        ...
        if(a.status == 250){
            $('.nctouch-main-layout').hide();
            $.sDialog({
                skin: "red",
                content: a.msg?a.msg:'申请失败!',
                okBtn: false,
                cancelBtn: false
            });
            ...
            $('.nctouch-main-layout').show();
            a.data.WapSiteUrl = WapSiteUrl;

            $("#order-info-container").html(template.render("order-info-tmpl", a.data));

            order_id = a.data.order_id;
            allow_refund_amount = a.data.return_cash;
            order_payment_amount = a.data.order.order_payment_amount;
            order_shipping_fee  = a.data.order.order_shipping_fee;
            ...
            $("#order_payment_amount").html("￥" + order_payment_amount);
            $("#order_shipping_fee").html("￥" + order_shipping_fee);
            $("#return_cash").val(allow_refund_amount.toFixed(2));
            ...
            $(".btn-l").click(function() {
                ...
                if (r.return_reason_id == '0') {
                    ...
                    r.return_reason = $('#res_content').find('option:selected').text();
                }
                if (!r.return_cash) {
                    $.sDialog({
                        skin: "red",
                ...
                if (!isNaN(r.return_cash) && r.return_cash > allow_refund_amount) {
                    $.sDialog({
                        skin: "red",
                        content: "最大退款金额为" + allow_refund_amount.toFixed(2),
                        okBtn: false,
                        cancelBtn: false
                    });
                    return false
