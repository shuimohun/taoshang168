...
$(function ()
{
    ...
    e.loadInit({
        url: ApiUrl + "?ctl=Buyer_Favorites&met=footprint&typ=json",
    ...
    {
        $.ajax({
            type: "post", url: ApiUrl + "?ctl=Buyer_Favorites&met=delFootPrint&typ=json", data: {k: key, u:getCookie('id')}, dataType: "json", async: false, success: function (e)
            {
                if (e.status == 200)
                {
