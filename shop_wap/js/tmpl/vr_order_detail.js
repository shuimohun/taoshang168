...
$(function(){
    ...
    $.getJSON(ApiUrl + '?ctl=Buyer_Order&met=virtual&act=detail&typ=json',{k: getCookie("key"),u: getCookie("id"),order_id:order_id}, function(result) {
        ...
        $.getJSON(ApiUrl + '?ctl=Buyer_Order&met=getEntityList&typ=json', {shop_id:result.data.shop_id},function(result){
            if (result.status == 250) {
                return ;
            }
    ...
    function cancelOrderId(order_id) {
        $.ajax({
            type:"post",
            url: ApiUrl + "?ctl=Buyer_Order&met=orderCancel&typ=json",
