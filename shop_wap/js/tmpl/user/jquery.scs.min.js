...
function(a, b) {
    ...
    SCS.init = function(a) {
        ...
        c = "",
        d = a.navArr;
        c += '<div class="cascade_scroll_selects ' + a.skinCss + '" id="cascade_scroll_selects"><div class="scs_container"><div class="scs_cover"></div><div class="scs_dialog"><div class="scs_selects">',
        ...
        c += '</div><div class="scs_btns"><div class="scs_btn_span"></div><div class="scs_btn scs_btn_ok">确定</div><div class="scs_btn scs_btn_cancel">取消</div></div></div></div></div>',
        b("#cascade_scroll_selects").remove(),
        b("body").append(c);
        var e = b("#cascade_scroll_selects");
        e.find(".scs_dialog").slideDown(300),
        e.find(".scs_cover").click(function() {
            e.find(".scs_dialog").slideUp(200,
            function() {
                e.remove()
            })
        }),
    ...
    SCS.scrollCascadeSelect = function(a) {
        function c(a) {
            ...
            p = o = a.pageY,
            s = (new Date).getTime(),
            q = parseInt(h.css("webkitTransform").split(",").pop().replace(" ", "").replace(")", "")),
        ...
        function e(b) {
            if (t = (new Date).getTime(), Math.abs(p - o) > 5 && t - s > 20) {
                ...
                r += g;
                var k = r % 40;
                Math.abs(k) >= 20 ? r += 0 > k ? -1 * (40 + k) : 40 - k: r -= k,
                r > 80 ? r = 80 : r < -1 * (h.attr("data-height") - 120) && (r = -1 * (h.attr("data-height") - 120)),
                h.css({
                    "-webkit-transition": "-webkit-transform " + j / 1e3 + "s ease-out",
                    "-webkit-transform": "translate3d(0, " + r + "px, 0)"
                }),
                l.selectedIndex = Math.abs((r - 80) / -40),
                setTimeout(function() {
                    h.find(".scs_item").removeClass("scs_selected").eq(l.selectedIndex).addClass("scs_selected"),
                    a.onChange(l.getSelectedItem(), l.selectedIndex)
                },
                j)
            } else n = !0;
            m = !1,
        ...
        function f(b) {
            if (m) if (t = (new Date).getTime(), Math.abs(p - o) > 5 && t - s > 20) {
                ...
                r += g,
                residue = r % 40,
                Math.abs(residue) >= 20 ? r += residue < 0 ? -1 * (40 + residue) : 40 - residue: r -= residue,
                r > 80 ? r = 80 : r < -1 * (h.attr("data-height") - 120) && (r = -1 * (h.attr("data-height") - 120)),
                h.css({
                    "-webkit-transition": "-webkit-transform " + j / 1e3 + "s ease-out",
                    "-webkit-transform": "translate3d(0, " + r + "px, 0)"
        ...
        l.selectedIndex = k,
        l.getSelectedItem = function() {
            return j[this.selectedIndex]
        },
        x ? (i.addEventListener("touchstart",
        function(a) {
            p = o = a.changedTouches[0].pageY,
            s = (new Date).getTime(),
            q = parseInt(h.css("webkitTransform").split(",").pop().replace(" ", "").replace(")", "")),
            w = 0
        },
        !1), i.addEventListener("touchmove",
        function(a) {
            event.preventDefault(),
            w = (a.changedTouches[0].pageY - v) / ((new Date).getTime() - u),
            p = a.changedTouches[0].pageY,
            r = q + p - o,
            h.css({
                "-webkit-transition": "-webkit-transform 0s ease-out",
                "-webkit-transform": "translate3d(0, " + r + "px, 0)"
        ...
        function(b) {
            if (t = (new Date).getTime(), Math.abs(p - o) > 5 && t - s > 50) {
                ...
                r += d;
                var f = r % 40;
                Math.abs(f) >= 20 ? r += 0 > f ? -1 * (40 + f) : 40 - f: r -= f,
                r > 80 ? r = 80 : r < -1 * (h.attr("data-height") - 120) && (r = -1 * (h.attr("data-height") - 120)),
                h.css({
        ...
        !1)) : i.addEventListener("mousedown", c),
        h.on("click", ".scs_item",
        function(c) {
            if (n) {
                var d = b(this),
                ...
                h.find(".scs_item").removeClass("scs_selected"),
                d.addClass("scs_selected"),
                l.selectedIndex = Math.abs((f - 80) / -40),
                a.onChange(l.getSelectedItem(), l.selectedIndex),
                havedClick = !1
            }
        }),
        ...
        var z = function() {
            l.render(j, k),
            a.onInit(l.getSelectedItem(), l.selectedIndex)
        };
        z()
    }
} (this, jQuery);