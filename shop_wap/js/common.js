...
function buildUrl(type, data) {
    switch (type) {
        ...
        case 'url':
            return data;
        case 'self_keyword':
            return WapSiteUrl + '/tmpl/product_list.html?own_shop=1&keyword=' + encodeURIComponent(data);
    }
    return WapSiteUrl;
}
...
function getSearchName() {
    ...
    if (keyword == '') {
        if (getCookie('deft_key_value') == null) {
            $.getJSON(ApiUrl + '?ctl=Index&met=getSearchWords&typ=json', function(result) {
    ...
    $.extend($, {
        ...
        animationLeft: function(options) {
            var defaults = {
                ...
                // 动作块
                scroll: '', // 滚动块，为空不触发滚动
                start: function() { $('body').addClass('ovfHiden');
                },// 开始动作触发事件
                close: function() { $('body').removeClass('ovfHiden');
                } // 关闭动作触发事件
            }
            var options = $.extend({}, defaults, options);
        ...
        animationUp: function(options) {
            var defaults = {
                ...
                // 滚动块，为空不触发滚动
                start: function() { /*$('html,body').addClass('ovfHiden');*/
                },
                // 开始动作触发事件
                close: function() { /*$('html,body').removeClass('ovfHiden');*/
                } // 关闭动作触发事件
            }
            var options = $.extend({}, defaults, options);
            ...
            return this.each(function() {
                ...
                $(options.wrapper).on('click', '.nctouch-bottom-mask-tip,.nctouch-bottom-mask-bg,.nctouch-bottom-mask-close', function() {
...
function favoriteStore(shop_id) {
    ...
    $.ajax({
        type: 'post',
        url: ApiUrl + '?ctl=Shop&met=addCollectShop&typ=json',
        data: {
            k: key,
            u: getCookie('id'),
            shop_id: shop_id
...
function dropFavoriteStore(shop_id) {
    ...
    $.ajax({
        type: 'post',
        url: ApiUrl + '?ctl=Buyer_Favorites&met=delFavoritesShop&typ=json',
        data: {
            k: key,
            u: getCookie('id'),
            id: shop_id
        ...
        success: function(data) {
            ...
            if (200 == data.status) {
                ...
                if (u && key && u == data.data.us) {
                    ...
                    $.ajax({
                        type: "get",
                        url: ApiUrl + "?ctl=Login&met=check&typ=json",
                        data: {
                            ks: data.data.ks,
                            us: data.data.id
                        },
