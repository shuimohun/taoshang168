...
UploadImage = function (uploadArguments) {
    ...
    if (typeof(uploadConfig) === 'undefined'){
        $.ajax({
            type: "get",
            url: UCenterApiUrl + "?ctl=Upload&action=config",
            dataType: "jsonp",
...
UploadImage.prototype = {
    ...
    initUploader: function () {
        var ratio = window.devicePixelRatio || 1,
            ...
            //如果没有图片添加默认图片
            if ( $image.attr('src') == '' ) {
                $image.attr('src', WapSiteUrl + '/images/image.png');
            }
            ...
            uploader = WebUploader.create({
                ...
                swf: UCenterWapUrl + '/static/default/js/Uploader.swf',

                server: UCenterApiUrl + "?ctl=Upload&action=" + UploadImage.uploadConfig.imageActionName,
