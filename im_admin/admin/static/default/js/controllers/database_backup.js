function initEvent() {
	...
	$("#btn-refresh").click(function(a) {
        ...
        $("#grid").jqGrid("setGridParam", {
            url: "./admin.php?ctl=Database_Backup&met=getBackupList&typ=json",
            datatype: "json"
        }).trigger("reloadGrid")
    }
    ),
...
function do_backup()
{
	...
	$.ajax({
		url:"./admin.php?ctl=Database_Backup&typ=json&met=backup", 
		type:'post',
		data:'action=backup',
		timeout:0,//兼容火狐
		success:function(b) {
...
var r_callback=function(file,db_name){
	var a = parent.$.dialog.tips("恢复中，请稍候...", 1e3, "loading.gif", !0);
	$.ajax({
		url:"./admin.php?ctl=Database_Backup&typ=json&met=restore", 
		type:'post',
		data:'action=restore&file='+file+'&db_name='+db_name,
		timeout:0,//兼容火狐
		success:function(b) {
			a.close();
			if(b && 200 == b.status)
			{
				parent.Public.tips({content: "恢复成功！"});
			}
			else
			{
				parent.Public.tips({type: 1,content: "恢复失败！"+b.msg});
			}
		}
	})
}
function do_action(s,i)
{
	if(s=='restore')
	{
		var a = {file: i, callback: r_callback};
		$.dialog({
            title: '选择要恢复的数据库',
            content: "url:./admin.php?ctl=Database_Backup&met=manage",
            data: a,
            width: 350,
            height: 150,
            max: !1,
	...
	else
	{
		...
		$.ajax({
			url:"./admin.php?ctl=Database_Backup&typ=json&met="+s, 
			type:'post',
			data:'action='+s+'&file='+i,
			timeout:0,//兼容火狐
			success:function(b) {
				a.close();
				if(b && 200 == b.status)
