	...
	<body>
			...
			<div class="wrap clearfix bbc_bg">
				<div class="bbuyer_head_fl">
					<div class="bbuyer_logo">
						<a href="<?= YLB_Registry::get('url') ?>?ctl=Index&met=index"><img src="<?= $this->web['site_logo'] ?>"/></a>
					</div>
					<div class="bbuyer_others">
						<p class="mine_shopmall"><?=_('我的个人中心')?></p>
						<p class="back_mall_index"><a href="<?= YLB_Registry::get('url') ?>"><?=_('返回个人中心首页')?></a></p>
					</div>
							...
							<?php foreach ($app_rows as $app_row): ?>
								<?php if (104 != $app_row['app_id']):?>
									...
									<?php endif; ?>
										<?php endforeach; ?>
											<li>
												<a href="<?= YLB_Registry::get('url') ?>?ctl=Login&met=logout">
													<?= _('退出') ?>
												</a>
											</li>
						</ul>
						...
						<div class="right  ">
							<div class="_left">
								<ul>
									<li>
										<div class="Divradius">
											<a href="<?= YLB_Registry::get('url') ?>?ctl=Order&met=physical&status=wait_pay"><img src="<?= $this->view->img ?>/ico_dai1.png"></a></div>
										<p class="_p">
									...
									<li>
										<div class="Divradius">
											<a href="<?= YLB_Registry::get('url') ?>?ctl=Order&met=physical&status=wait_confirm_goods"><img src="<?= $this->view->img ?>/ico_dai2.png"></a></div>
										<p class="_p">
									...
									<li>
										<div class="Divradius">
											<a href="<?= YLB_Registry::get('url') ?>?ctl=Order&met=physical&status=finish"><img src="<?= $this->view->img ?>/ico_dai3.png"></a></div>
										<p class="_p">
											<?= _('已完成') ?>
									...
									<li>
										<div class="Divradius">
											<a href="<?= YLB_Registry::get('url') ?>?ctl=Order&met=physical&status=cancel"><img src="<?= $this->view->img ?>/ico_dai4.png"></a></div>
										<p class="_p">
											<?= _('已取消') ?>
							...
							<div class="b_rol">
								<div class=" same">
									<div class="same_1">
										...
										<a href="<?= YLB_Registry::get('url') ?>?ctl=Property&met=cash">
											<?= _('余额') ?>
										</a>
									</div>
								...
								<div class="same">
									<div class="same_1">
										...
										<a href="<?= YLB_Registry::get('url') ?>?ctl=Points&met=points">
											<?= _('金蛋') ?>
										</a>
									</div>
									<div class="same_2">
								...
								<div class="same">
									<div class="same_1">
										...
										<a href="<?= YLB_Registry::get('url') ?>?ctl=Voucher&met=voucher">
											<?= _('代金卷') ?>
										</a>
									</div>
									<div class="same_2">
     ...
     <div  class="shangcheng  " >
      ...
      <ul class="nav-menu clearfix">
        ...
        <li class="set"><a href="<?= YLB_Registry::get('url') ?>?ctl=User&met=getUserInfo" class="current"><?= _('用户设置') ?><i class="iconfont">&#xe632;</i></a>
          <div class="sub-menu">
            <dl>
              <dt><a href="<?= YLB_Registry::get('url') ?>?ctl=User&met=security" style="color: #3AAC8A;"><?= _('安全设置') ?></a></dt>
              <dd><a href="<?= YLB_Registry::get('url') ?>?ctl=User&met=security&op=mobile"><?= _('手机绑定') ?></a></dd>
              <dd><a href="<?= YLB_Registry::get('url') ?>?ctl=User&met=security&op=email"><?= _('邮件绑定') ?></a></dd>
            </dl>
            <dl>
              <dt><a href="<?= YLB_Registry::get('url') ?>?ctl=User&met=getUserInfo" style="color: #EA746B"><?= _('个人资料') ?></a></dt>
              <dd><a href="<?= YLB_Registry::get('url') ?>?ctl=User&met=tag"><?= _('兴趣标签') ?></a></dd>
              <dd><a href="<?= YLB_Registry::get('url') ?>?ctl=User&met=getUserImg"><?= _('修改头像') ?></a></dd>
              <dd><a href="<?= YLB_Registry::get('url') ?>?ctl=Message&met=message&op=manage"><?= _('消息接受设置') ?></a></dd>

            </dl>
            <dl>
              <dt><a href="<?= YLB_Registry::get('url') ?>?ctl=Property&met=cash" style="color: #FF7F00"><?= _('账户财产') ?></a></dt>
              <dd><a href="<?= YLB_Registry::get('url') ?>?ctl=Voucher&met=voucher"><?= _('我的代金券') ?></a></dd>
			  <dd><a href="<?= YLB_Registry::get('url') ?>?ctl=Points&met=Points"><?= _('我的金蛋') ?></a></dd>

            </dl>
            <dl>
              <dt><a href="<?= YLB_Registry::get('url') ?>?ctl=Favorites&met=favoritesGoods"  style="color: #398EE8"><?= _('我的收藏') ?></a></dt>
              <dd><a href="<?= YLB_Registry::get('url') ?>?ctl=Favorites&met=favoritesShop"><?= _('店铺收藏') ?></a></dd>
            </dl>
          </div>
        </li>
      </ul>
				...
				foreach ($buyer_menu[$level_row[1]]['sub'] as $menu_row)
				{
					...
					if (!empty($menu_row['sub']))
					{
						foreach ($menu_row['sub'] as $menus_row)
						{
							...
							?>
													<li>
														<a href="<?= sprintf('%s?ctl=%s&met=%s', YLB_Registry::get('url'), $menus_row['menu_url_ctl'], $menus_row['menu_url_met'], $menus_row['menu_url_parem']); ?>" class="_Color" style="<?= ($menus_row['menu_id'] == $level_row[3]) ? 'color:red;' : '' ?>">
															<?= $menus_row['menu_name'] ?>
														</a>
													</li>
													<?php
							...
							foreach ($buyer_menu[$level_row[1]]['sub'][$level_row[2]]['sub'][$level_row[3]]['sub'] as $menu_row)
							{
								?>
															<li class="<?= ($menu_row['menu_id'] == $level_row[4]) ? 'active' : 'normal' ?>">
																<a href="<?= sprintf('%s?ctl=%s&met=%s', YLB_Registry::get('url'), $menu_row['menu_url_ctl'], $menu_row['menu_url_met'], $menu_row['menu_url_parem']); ?>">
																	<?= $menu_row['menu_name'] ?>
																</a>
															</li>
															<?php
