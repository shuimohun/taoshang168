...
<div id="form-header" class="header">
    <div class="logo-con w clearfix">
        <a href="<?=YLB_Registry::get('shop_api_url')?>" class="index_logo">
         <img src="<?= $web['site_logo'] ?>" alt="logo" height="60">
        </a>
        <div class="logo-title">欢 迎 注 册</div>

        <div class="have-account">已有账号 <a href="<?=sprintf('%s?ctl=Login&met=index&t=%s&from=%s&callback=%s', YLB_Registry::get('url'), request_string('t'), request_string('from'), urlencode(request_string('callback')))?>" target="_blank">请登录</a></div>
    </div>
		...
		<div class="main clearfix" id="form-main">
			<div class="reg-form fl">
				<form action="" id="register-form" method="post" novalidate="novalidate" onsubmit="return false;">
					...
					<input type="hidden" name="from" class="from" value="<?=$from?>">
					...
					<div class="form-item form-item-account disb" id="form-item-account">
						<label>用　户　名</label>
						<input type="text" id="re_user_account"  class="field re_user_account" autocomplete="off" maxlength="20" placeholder="您的账户名和登录名" default="<i class=&quot;i-def&quot;></i>6-20个字符，支持英文、数字，不能为纯数字，不能以g或者G开头" onfocus="checkAcount()" onblur="userCallback()">
						<i class="i-status"></i>
					</div>
					<div class="input-tip">
						<span></span>
					...
					<div id="Mobile" style="display: <?=$mobile_display?>">
                        ...
                        <div class="form-item form-item-phonecode">
                            ...
                            <button id="getPhoneCode" class="btn-phonecode" type="button" onclick="get_randcode()" style="cursor:pointer;">获取验证码</button>
                            <i class="i-status"></i>
                        </div>
					</div>
					...
					<div id="Email" style="display: <?=$email_display?>">
                        ...
                        <div class="form-item form-item-phonecode">
                            ...
                            <button id="getEmailCode" class="btn-phonecode" type="button" onclick="get_randcode()" style="cursor:pointer;">获取验证码</button>
                            <i class="i-status"></i>
                        </div>
                    </div>
					...
					<div class="form-agreen">
						<div>
							...
							<input type="checkbox" name="agreen" checked="">我已阅读并同意<a href="javascript:;" id="protocol" onclick="registalert()">《用户注册协议》</a> </div>
						<div class="input-tip">
							<span></span>
						</div>
					</div>
	...
	<script>
		...
		function checkAcount() {
			...
			var errormsg = icons.error +
				'限6-20个字符，支持英文、数字、减号或下划线，不能以g或者G开头';
				'格式错误，仅支持字母、数字、“-”“_”的组合，不能以g或者G开头';
				'格式错误，仅支持字母、数字、“-”“_”的组合';
		...
		function userCallback() {
			...
			hideError(form_account);
			if(user_account) {
                var reg = /^(?![gG].*$)(?![0-9]*$)[a-zA-Z0-9]{6,20}$/;//字母或数字 不为纯数字
                var errormsg = icons.error + '6-20个字符，支持英文、数字，不能为纯数字，不能以g或者G开头';
		...
		function checkMobile() {
			...
			if(mobile) {
				//先匹配是否为手机号
				if(!isNaN(mobile) && /^1[3|4|5|6|7|8|9]{1}[0-9]{9}$/.test(mobile)) {
					//验证该手机号是否被注册过
        ...
        function get_randcode_email(){
            ...
            var ajaxurl = './index.php?ctl=Login&met=regCode&typ=json&email='+email;
            $.ajax({
                    type: "POST",
                    url: ajaxurl,
                    dataType: "json",
        ...
        function get_emailcode(){
            ...
            $.ajax({
                    ...
                    success: function (respone) {
                        if(respone.status == 250) {
                            var errormsg = icons.error + respone.msg;
                            onKeyupHandler(form_emailg, errormsg);
                        } else {
                            window.countDown();
                            alert('请查看邮件获取验证码!');
                        }
        ...
        function registclick(){
            ...
            $.post("./index.php?ctl=Login&met=register&typ=json",{"user_account":user_account,"user_password":user_password,"user_code":user_code,"mobile":mobile,"t":token, 'email':email, 'reg_checkcode':reg_checkcode,'from':from} ,function(data) {
                if(data.status == 200) {
                    k = data.data.k;
                    u = data.data.user_id;
                    if(callback){
                        window.location.href = decodeURIComponent(callback) + '&us=' + encodeURIComponent(u) + '&ks=' + encodeURIComponent(k) + '&type=' + type;
                    }else{
