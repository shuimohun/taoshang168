...
class UserCtl extends YLB_AppController
{
	...
	public function getUserInfo()
	{
        ...
        if(empty($data['user_mobile']) && empty($data['user_email']))
        {
            ...
            {
                header('location:' . YLB_Registry::get('url').'?ctl=User&met=security&op=mobiles');
                die;
            }
        }
	...
	public function getUserImg()
	{
		...
		if($this->typ == 'json')
        {
            $this->data->addBody(-140,$this->user['info']);
        }
        else
        {
            include $this->view->getView();
     ...
     * @access public
     */
    public function editUserGender()
    {
        $user_name = Perm::$row['user_name'];
        $edit_user_row['user_gender']     = request_int('user_gender');
        $flag = $this->userInfoModel->editInfoDetail($user_name, $edit_user_row);
