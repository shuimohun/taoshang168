 ...
 * @author     WenQingTeng
 */
class Base_AppVersionCtl extends YLB_AppController
{
	...
	public function checkSums()
	{
		...
		else
		{
			...
			$cmd = sprintf('svn export   --username %s --password %s -r r%s %s %s   2>&1', 'readonly', 'readonly1234', $upgrade_svn_version, '', $version_path);
			$result = shell_exec($cmd);
			
			//file_put_contents($version_path . '/', $version_file_content);
		}
		
		
		
		//处理无关文件
		$files = array('*.log', '*.md', '*.txt', 'private.pem', 'rsa_private_key.pem', '*.ini.php');
		$dirs  = array('build_tools', 'install', 'libraries');
		
		foreach ($files as $file)
		{
			$cmd = sprintf('find %s -name "%s" |xargs rm -rf', $version_path, $file);
			$result = shell_exec($cmd);
		}
		
		foreach ($dirs as $dir)
		{
			$cmd = sprintf('find %s -type d -name "%s" |xargs rm -rf', $version_path, $dir);
			$result = shell_exec($cmd);
		}
		...
		{
			//计算md5 file
			$data = YLB_Utils_File::getPhpFile($version_path, '*');
