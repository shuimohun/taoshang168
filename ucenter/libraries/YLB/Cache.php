...
final class YLB_Cache
{
    ...
    static public function create($name)
    {
        ...
        if (!isset(self::$cache[$name]))
        {
			...
			elseif (1 == $config_cache[$name]['cacheType'])
			{
				$key = '_file_' . $name;
            ...
            else
            {
				...
				{
					self::$cache[$name] = new YLB_Cache_Apcu($config_cache[$name]);
				}
				elseif (extension_loaded('apc') && function_exists('apc_fetch'))
				{
					self::$cache[$name] = new YLB_Cache_Apc($config_cache[$name]);
				}
				else
				{
					$key = '_file_' . $name;
