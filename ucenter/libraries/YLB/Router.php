 ...
 * @todo       
 */
class YLB_Router extends YLB_Controller
{
    ...
    public function checkUrl()
    {
        ...
        $int_row   = $YLB_Registry['int_row'];
        $float_row = $YLB_Registry['float_row'];
        ...
        else
        {
            ...
            throw new YLB_ProtocalException(sprintf(_('协议不存在: %s %s') , $_REQUEST['ctl'], $_REQUEST['met']));
            //die('发生错误，请检查控制器路径!');
        }
		*/
        ...
        else
        {
            ...
            error_header(404, 'Page Not Found');
            throw new YLB_ProtocalException(sprintf(_('file: %s does not exists') , $this->path));
    ...
    public function getData()
    {
            ...
            */
            ...
            try
            {
                ...
                //$PluginManager =  YLB_Registry::get('hook');
                //$PluginManager->trigger('data_to_client', $d);

				$protocol_data = YLB_Data::encodeProtocolData($d, $this->typ);
            ...
            catch(YLB_ProtocalException $e)
            {
                if ('cli' != SAPI)
                {
					if (YLB_Registry::get('error_url') && 'e'==$this->typ)
					{
						location_to(YLB_Registry::get('error_url') . '?msg=' . urlencode($e->getMessage()));
