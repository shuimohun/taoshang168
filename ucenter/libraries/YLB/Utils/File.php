...
class YLB_Utils_File
{
	...
	public static function getPhpFile($dir, $ext='php')
	{
		$files = array();
			...
			if ($handle = opendir($dir))
			{
				while (($file = readdir($handle)) !== false)
				{
					if ($file != "." && $file != "..")
					{
						if (is_dir($dir . "/" . $file))
						{
							$files = array_merge($files, YLB_Utils_File::getPhpFile($dir . "/" . $file, $ext));
						}
						else
						{
							$tmp_file = $dir . "/" . $file;
	...
	public static function copyDir($source, $destination, $skip=array())
	{
		...
		{
			if ($file != "." && $file != ".." && $file != ".svn" && !in_array($file, $skip))
			{
