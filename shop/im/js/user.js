...
function send_to_server_bysun(_user_account, content_you, msgid, im_send_content, content_type) {
    //聊天记录发送到数据库
    $.post(imbuilder_url + "?ctl=Api_Chatlog&met=add&typ=json", {
        u: _user_account,
        to: content_you,
        msgid: msgid,
...
function DO_sendMsg() {
    ...
    $.post('ajax.php', {
        ...
        $.ajax({
            type: "POST",
            url: imbuilder_url + '?ctl=UserApi&met=addRecentContacts&typ=json&k=' + k + '&u=' + u + '&contacts_id=' + content_you +'&contacts_time='+msgid,
            dataType: "json",
            async: false,
            success: function(rs) {
...
function DO_im_attachment_file() {
    ...
    var str = '<div class="progress progress-striped active">' + '<div class="bar" style="width: 40%;"></div>' + '</div>' + '<span imtype="msg_attach" class="bubble">' + '<a imtype="msg_attach_href" href="javascript:void(0);" target="_blank">' + '<span>' + '<img style="width:32px; height:32px; margin-right:5px; margin-left:5px;" src="img/attachment_icon.png"/>' + '</span>' + '<span imtype="msg_attach_name"></span>' + '</a>' + '<span style="font-size: small;margin-left:15px;"></span>' + '<input imtype="msg_attach_resend" type="file" accept="" style="display:none;margin: 0 auto;" onchange="DO_im_attachment_file_up(\'' + content_you + '_' + msgid + '\', \'' + msgid + '\')">' + '</span>';
    // 添加右侧消息
    var id = show_t_msg('temp_msg', 6, msgid, content_type, content_you, str);
...
function DO_im_attachment_file_up(id, oldMsgid) {
    ...
    if ($("#camera_button").find("i").hasClass("icon-picture")) {
        ...
        if ("image" == oFile.type.substring(0, oFile.type.indexOf("/"))) {
            ...
            var str = '<img imtype="msg_attach_src" src="' + url + '" style="max-width:' + imgWid + 'px; max-height:' + imgHei + 'px;" ' + '  onclick="open_img(this)"/>';
...
function EV_sendAttachMsg(oldMsgid, file, type, receiver, isresend) {
    ...
    var msgid = RL_YTX.sendMsg(obj, function(obj) {
        setTimeout(function() {
            ...
            /*$.post('/im/ajax_img.php',{str:msg.prop("outerHTML")},function(d){
        ...
        setTimeout(function() {
            ...
            msg.find('div[class="bar"]').parent().css('display', 'none');
            msg.find('span[imtype="msg_attach"]').css('display', '');
...
function show_t_msg(msg, msgtype, msgid, content_type, content_you, send_content) {
    ...
    /*if (n >= msg_max && n % msg_max == 1){
        obj_msg.append('<div clear_id="' + msgid + '" onclick="clear_msg(' + u_id + ',' + msgid +');" class="clear_msg"><a href="Javascript: void(0);">清除已上历史消息</a></div>');
    }*/
 ...
 * @constructor
 */
function HTML_sendMsg_addHTML(msg, msgtype, msgid, content_type, content_you, im_send_content) {
    var time = new Date();
    var ymdhis = "";
    ...
    var str = '<div contactor="sender" im_carousel="' + carou + '" msg="' + msg + '" im_msgtype="' + msgtype + '" id="' + content_you + '_' + msgid + '" content_type="' + content_type + '" content_you="' + content_you + '" class="alert alert-right alert-success" style="display:' + display + '">' + '<span imtype="resend" class="add-on" onclick="IM.EV_resendMsg(this)"' + ' style="display:none; cursor:pointer; position: relative; left: -40px; top: 0px;"><i class="icon-repeat"></i></span>' + '<img src="' + user_logo + '" style="float: right; width: 38px; height: 38px; border: 1px solid #ccc; background: #dcdcdc none repeat scroll 0 0 padding-box; border-radius: 8px; margin:10px 10px 0;">' + im_send_content + '<div class="name name-right" style="float:right;text-align:right;padding-right:60px;width:100%;">' + ymdhis + '</div></div>';
...
function initEmoji() {
    ...
    //console.log(emoji_div);
    for (var i in emoji.show_data) {
        var c = emoji.show_data[i];
        var out = emoji.replace_unified(c[0][0]);

        var content_emoji = '<span style="cursor:pointer; margin: 0 2px 0 4px;" ' + 'onclick="DO_chooseEmoji(\'' + i + '\', \'' + c[0][0] + '\')" ' + 'imtype="content_emoji">' + out + '</span>';
...
function _login(user_id) {
    ...
    if (flag) {
        _privateLogin(user_id, timestamp, function(obj) {
            EV_login(user_id, obj.sig, timestamp);
        }, function(obj) {
            //$('#navbar_user_account').removeAttr("readonly");
            alert("错误码_login：" + obj.code + "; 错误描述：" + obj.msg);
        });
    } else {
        //仅用于本地测试，官方不推荐这种方式应用在生产环境
        //没有服务器获取sig值时，可以使用如下代码获取sig
        var sig = hex_md5(_appId + user_id + timestamp + _appToken);
        EV_login(user_id, sig, timestamp);
    }
}
...
function EV_login(user_id, sig, timestamp) {
    ...
    $.ajax({
        ...
        success: function(result) {
            if (result.status == 200) {
                $.post(imbuilder_url + "?ctl=Login&met=checkToLogin&typ=json", {ks: result.data.ks,us: result.data.us}, function(data) {
                    if (data.status == 200) {
                        k = data.data.k;
                        u = data.data.user_id;
                        ...
                        RL_YTX.login(loginBuilder, function(obj) {
                            ...
                            _onMsgReceiveListener = RL_YTX.onMsgReceiveListener(function(obj) {
                                EV_onMsgReceiveListener(obj);
                            });
                            EV_getGroupList();

                            if (_local_historyver <= parseInt(obj.historyver) && parseInt(obj.historyver) < parseInt(obj.version)) {
...
function DO_push_createMsgDiv(obj) {
    ...
    var b_current_contact_you = DO_createMsgDiv_Help(you_sender, name, b_isGroupMsg);
    ...
    var gid = you_msgContent.match(/ctl=Goods_Goods&met=goods&(gid|goods_id)(=|\()(["|']?)([^"'>|\)]+())/i);
    if(gid && gid[4]){
        $.ajax({
            type: "POST",
            url: SiteUrl + '?ctl=Goods_Goods&met=getGoodsTitle&typ=json&gid=' + gid[4],
            dataType: "json",
            async: false,
            success: function (rs) {
                var goods_id = rs.data.goods_id;
                var goods_name = rs.data.goods_name;
                var goods_image = rs.data.goods_image;
                you_msgContent = '<div><a target="_blank" style="display: inline-block" href="' + SiteUrl + '?ctl=Goods_Goods&met=goods&gid=' + goods_id + '"><div style="float:left;width: 50px;height: 50px;"><img src=' + goods_image + ' style="width: 50px;height: 50px;"></div><div style="float:left;width: 220px;padding-left: 5px;font-size: 12px;height: 42px;overflow: hidden;color: #666;"> <span>' + goods_name + '</span> </div> </a> </div>';
            }
        });
    }

    var ajaxurl = imbuilder_url + '?ctl=UserApi&met=getUserInfo&typ=json&k=' + k + '&u=' + u + '&user_id=' + you_sender;
    $.ajax({
        type: "POST",
        url: ajaxurl,
        dataType: "json",
        ...
        success: function(rs) {
            if (rs.status == 200) {
                ...
                if(typeof friend_list[you_sender] === "undefined"){
                    friend_list[you_sender] = user_list[you_sender];
                    update_friends();
                }
            }
        }
    ...
    if (0 == obj.mcmEvent) {
        ...
        if (1 == msgType) {
            ...
            /* alert(you_msgContent);  //str  接受的信息内容
             alert(b_isGroupMsg);    //分组
             alert(version);         //msgid
             alert(time);            //时间
             alert(obj.msgType);     //信息类型 1
             alert(obj.mcmEvent);    //0 普通信息*/
            //alert(str);
    ...
    } else if (1 == obj.mcmEvent) {
        // 1 start消息
        HTML_pushMsg_addHTML(obj.msgType, you_sender, version, _contact_type_m, b_current_contact_you, name, you_msgContent);
    } else if (2 == obj.mcmEvent) {
        // 2 end消息
        HTML_pushMsg_addHTML(obj.msgType, you_sender, version, _contact_type_m, b_current_contact_you, name, "结束咨询");
    } else if (3 == obj.mcmEvent) {
        // 3发送mcm消息
        HTML_pushMsg_addHTML(obj.msgType, you_sender, version, _contact_type_m, b_current_contact_you, name, you_msgContent);
    } else if (53 == obj.mcmEvent) {
        // 3发送mcm消息
...
function EV_getMyInfo() {
    RL_YTX.getMyInfo(function(obj) {
        ...
        $.post(imbuilder_url + "?ctl=UserApi&met=getUserInfo&typ=json", {
            "k": k,
            "u": u
        }, function(data) {
            if (data.status == 200) {
...
function EV_syncMsg(sv, ev) {
    ...
    RL_YTX.syncMsg(obj, function(obj) {
        alert("错误码EV_syncMsg： " + obj.code + "; 错误描述：" + obj.msg);
    });
}
...
function EV_getGroupList() {
    if (user['u_id'] != '') {
        msg_dialog = '<div id="new_msg_dialog" class="msg-windows"><div class="user-tab-bar"><ul class="user-list" id="user_list"></ul></div><div class="msg-dialog">' + '<div id="dialog-body"  class="dialog-body">' + '<div class="msg-top"><dl class="user-info"><dt class="user-name"></dt><dd class="user-avatar avatar-0"><img src="" alt=""></dd><dd class="store-name"></dd></dl>' + '<span class="dialog-close" onclick="msg_dialog_close(\'new_msg_dialog\');">&nbsp;</span></div>' + '<div id="msg_list" class="msg-contnet"><div id="user_msg_list"></div></div>' + '<div class="msg-input-box"><div class="msg-input-title">' + '<div id="emoji_div" class="span8 popover top" style="display:none;">' + '<div class="popover-content" style="max-height: 97px; font-size:18px; overflow-y: auto">' + '</div>' + '</div>' + '<a id="chat_show_smilies" onclick="show_emoji_div()" class="chat_smiles">表情</a>' + '<a id="chat_show_img" href="javascript:void(0)" onclick="DO_im_attachment_file()" class="chat_img wkbutton_3">图片</a>' + '<span class="title">输入聊天信息</span><span class="chat-log-btn off" onclick="show_chat_log();">聊天记录<i></i></span></div>'
            + '<form id="msg_form"><pre name="send_message" id="send_message" class="textarea" onkeyup="send_keyup(event);" contenteditable="true" onfocus="send_focus();" ></pre><pre id="im_send_content_copy" style="display:none;"></pre>' + '<div class="msg-bottom"><div id="msg_count"></div><a href="JavaScript:void(0);" onclick="DO_sendMsg();" class="msg-button"><i></i>发送</a><div id="send_alert"></div></div></form></div></div>' + '<div id="dialog_chat_log" class="dialog_chat_log"></div><div id="dialog_right_clear" class="dialog_right_clear"></div></div><div id="dialog_clear" class="dialog_clear"></div></div>';
        var chat_user_list = '<div class="chat-box"><div class="chat-list"><div class="chat-list-top"><h1><i></i>联系人</h1><span class="minimize-chat-list" onclick="chat_show_list();"></span></div>' + '<div id="chat_user_list" class="chat-list-content"><div><dl id="chat_user_friends" ><dt onclick="chat_show_user_list(\'friends\');"><span class="hide"></span>最近联系人</dt><dd id="chat_friends" style="display: none;"></dd></dl>' + '</div></div>' + '</div><div class="bottom-bar"><span id="new_msg" class="ico" onclick="chat_show_list();"></span><a href="javascript:void(0)" onclick="chat_show_list();"><i></i></a></div></div>';
        ...
        ajaxurl = imbuilder_url + '?ctl=UserApi&met=getRecentContacts&typ=json';
        $.ajax({
            type: "POST",
            url: ajaxurl,
            data: {"k": k,"u": u},
    ...
    if (layout == 'layout/store_layout.php') {
        ...
        switch (act_op) {
            case "act_op":
                //不显示状态
                break;
            case "member_snsfriend_findlist":
            //会员中心好友中"查找好友"不显示状态
            case "member_snsfriend_follow":
            //会员中心好友中"我关注的"不显示状态
            case "member_snsfriend_fan":
                //会员中心好友中"关注我的"不显示状态
                break;
            case "brand_list":
            case "search_index":
...
function get_state(list) { //返回会员的状态并在页面显示
    ...
    if (layout == 'layout/store_layout.php') { //店铺页面
        $("a.message").each(function() {
            ...
            re.exec(url);
            var u_id = RegExp.$1;
            if ($(".shop-head-info").find(".chat").size() == 0) { //头部
...
function get_chat_log(time_from) {
    ...
    //聊天记录获取，根据天数
    $.get(ajax_chatlog_url + time_from, function(hl) {
    ...
    if (obj_chat_log.html() == '') {
        var chat_log_list = '<div class="chat-log-top"><h1><i></i>聊天记录</h1><span class="close-chat-log" onclick="show_chat_log();"></span></div>' + '<div id="chat_log_list" class="chat_log_list"><div id="chat_log_msg" class="chat-log-msg"></div></div><div class="chat-log-bottom"><div id="chat_time_from" class="chat_time_from">' + '<span time_id="7" onclick="get_chat_log(7);" class="current">7天</span><span time_id="15" onclick="get_chat_log(15);">15天</span><span time_id="30" onclick="get_chat_log(30);">30天</span></div>' + '</div>';
...
function chat(u_id) {
    ...
    if (typeof user_list[u_id] === "undefined" || typeof user_list[u_id]['avatar'] === "undefined") {
        var ajaxurl = imbuilder_url + '?ctl=UserApi&met=getUserInfo&typ=json&k=' + k + '&u=' + u + '&user_id=' + u_id;
        $.ajax({
            type: "GET",
            url: ajaxurl,
            dataType: "json",
            ...
            success: function(rs) {
                if (rs.status == 200) {
                    ...
                    var u_avatar = member['user_avatar'];

                    set_user_info(u_id, "u_name", u_name);
                    set_user_info(u_id, "avatar", u_avatar);
                    show_msg(u_id);

                    if(typeof friend_list[u_id] === "undefined"){
                        friend_list[u_id] = user_list[u_id];
                        update_friends();
                    }
    ...
    /*//获取传值cookie
    var goods_id_y = getCookie('goods_id_y');
    var goods_name_y = getCookie('goods_name_y');
    var goods_image_y = getCookie('goods_image_y');

    if (goods_name_y.length > 30) {
        goods_name_y = goods_name_y.substr(0, 30) + '...';
    }
    ...
    content_type = 'C';
    if(type){
        var  htm = '<div><div style="float:left;width: 220px;padding-left: 5px;font-size: 12px;height: 15px;overflow: hidden;color: #666;"> <span>' + type + '</span> </div> </a> </div>';
    }else{
        //im聊天样式+自动发送咨询商品
        var htm = '<div><a target="_blank" style="display: inline-block" href="' + SiteUrl + '?ctl=Goods_Goods&met=goods&type=goods&gid=' + goods_id_y + '"><div style="float:left;width: 50px;height: 50px;"><img src=' + goods_image_y + ' style="width: 50px;height: 50px;"></div><div style="float:left;width: 220px;padding-left: 5px;font-size: 12px;height: 42px;overflow: hidden;color: #666;"> <span>' + goods_name_y + '</span> </div> </a> </div>';
    }

    var msgid = new Date().getTime();

    show_t_msg('msg', '1', msgid, content_type, u_id, htm);

    // 发送消息至服务器
    if (_contact_type_c == content_type) {
        EV_sendTextMsg(msgid, htm, u_id, false);
    } else {
        EV_sendMcmMsg(msgid, htm, u_id, false);
    };*/
...
function show_chat_log() {
    ...
    if (typeof chat_log['u_id'] === "undefined" || chat_log['u_id'] != user_show) {
        ...
        //聊天记录获取
        ajax_chatlog_url = ApiUrl + "?ctl=Api_Chatlog&met=get&u=" + u + "&to=" + user_show + "&day=";
...
function show_msg(u_id) {
    ...
    if (user_show != u_id) {
        ...
        obj.find(".user-info").html('<dt class="user-name">' + u_name + '</dt><dd class="user-avatar avatar-' + user_info['online'] + '"><img src="' + user_info['avatar'] + '"></dd>' + add_html);
    ...
    for (var m_id in msg_list[u_id]) {
        ...
        if (obj.find("div[m_id='" + m_id + "']").size() == 0) {
            ...
            show_f_msg(msg);
            update_msg(u_id);
            //console.log('update_msg(u_id);');
            delete msg_list[u_id][m_id]; //删除消息
            //console.log('delete msg_list[u_id][m_id]');
            if (m_id > max_id) {
                max_id = m_id;
            }
        }
...
function show_f_msg(type, msg, u_id, avatar, msgid, time) {
    ...
    if (type == 4) {
        ...
        text_append += ymdhis + '</dt>';
        text_append += '<dd class="from-msg-text">';
        text_append += '<img src="' + msg + '" style="cursor:pointer;max-width:' + imgWid + 'px; max-height:' + imgHei + 'px;" onclick="open_img(this)" ></dd>';
        text_append += '<dd class="arrow"></dd>';
        text_append += '</dl>';
        text_append += '</div>';
    }
...
function alertWin(title, msg, w, h, src) {
    ...
    if (src == '' && msg == '') src = 'main.php?m=album&s=admin_album&nohead=true';
        ...
        if (typeof(msg.innerHTML) != 'undefined') {
            if (temp_con != '') msgBox.innerHTML = temp_con;
            else {
                msgBox.innerHTML = msg.innerHTML;
                temp_con = msg.innerHTML;
                msg.innerHTML = '';
            }
        } else msgBox.innerHTML = msg;
    }
