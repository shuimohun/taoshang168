...
YLB_Registry::set('magic_quotes_gpc', get_magic_quotes_gpc());
YLB_Registry::set('magic_quotes_runtime', get_magic_quotes_runtime());
...
if (function_exists('_'))
{
	...
	new YLB_Locale($path, $domain, $codeset, $lang);
	*/
}
else
{
 ...
 *
 * @param $intNum    要判断的数字
 * @param int $scope 1 > 0 ,0 >= 0, -1 所有整数;
 * @return bool
 */
...
function get_url($url, $arr_param=array(), $typ='JSON', $method='POST', $sign_key='', $timeout=10, $curl_header=array())
{
	...
	YLB_Log::log('$res : ' . $res, YLB_Log::INFO, 'get_url_with_encrypt');
...
function get_url_with_encrypt($key, $url, $formvars=array(), $typ='JSON', $method='POST')
{
	...
	YLB_Log::log('$tmp_str:' . $tmp_str, YLB_Log::INFO, 'get_url_with_encrypt');
	YLB_Log::log('$url:' . $url, YLB_Log::INFO, 'get_url_with_encrypt');
	$formvars["token"] = md5($tmp_str);

	YLB_Log::log($hash_row, YLB_Log::INFO, 'get_url_with_encrypt');
	YLB_Log::log($formvars, YLB_Log::INFO, 'get_url_with_encrypt');

	$rs = get_url($url, $formvars, $typ, $method);
	YLB_Log::log($rs, YLB_Log::INFO, 'get_url_with_encrypt');
	return $rs;
}

function check_url_with_encrypt($key, $formvars=array())
{
	YLB_Log::log($formvars, YLB_Log::INFO, 'check_url_with_encrypt');
	...
	YLB_Log::log('$tmp_str:' . $tmp_str, YLB_Log::INFO, 'check_url_with_encrypt');
	YLB_Log::log('md5-key:' . md5($tmp_str), YLB_Log::INFO, 'check_url_with_encrypt');
...
/**
 * 将大数字转换为中文 1001->1千+ 10001->1万+
 * @param $n
 * @return string
 */
function number_format_hanzi($n)
{
    $a = array('千','万');
    $l = strlen($n);
    if($l > 3)
    {
        $n = substr($n,0,1) . $a[$l-4].'+';
    }
    return $n;
}
...
function download_url($url, $timeout = 300)
{
	...
	$tmpfname = YLB_Utils_File::tempnam($url_filename);
	if (!$tmpfname)
	{
