...
class YLB_Model
{
    ...
    //Zhenzh
    protected function _insertII(&$a,$flag = false)
    {
        $data_row = array();

        $sql = 'if not exists (select '.$this->_tablePrimaryKey.' from '.$this->_tableName;
	...
	protected function getNum($cond_row)
	{
		if ($cond_row)
		{
			foreach ($cond_row as $k=>$v)
			{
                ...
                if (count($k_row) > 2)
                {
                    $this->sql->setWhere($k_row[0], $v, $k_row[1], $k_row[2]);
                }
                else if (count($k_row) > 1)
                {
	...
	public function getRowCount($cond_row)
    {
	...
	public function listById($cond_row, $order_row = array(), $ids = array(),$page=1, $rows=100, $flag=true)
    {
        //需要分页如何高效，易扩展
        $offset = $rows * ($page - 1);
        $this->sql->setLimit($offset, $rows);
        ...
        if ($key_rows)
        {
            $data_tmp_rows = $this->get($ids, null, $order_row);
    ...
    /**
     * 根据其它键值，从数据库读取数据
     * @param  string   $field  查找的字段名称
     * @param  int   $table_primary_key_value  键值
     * @return array $rows 返回的查询内容
     * @access public
     */
    protected function otherGet($field=null,$table_primary_key_value=null,$order_row=array(),$key_row=null)
    {
        $rows = array();

        if($field == null)
        {
            $field = $this->_tablePrimaryKey;
        }
        ...
        if (is_array($table_primary_key_value))
        {
            ...
            if ($this->_cacheFlag)
            {
                ...
                if (!empty($need_cache_id_name))
                {
                    $this->sql->setWhere($field, $need_cache_id_name, 'IN');
                    $rows_db = $this->_select();
                }
            ...
            {
                $this->sql->setWhere($field, $table_primary_key_value, 'IN');
                $rows = $this->_select();
        ...
        else
        {
            ...
            if (($this->_cacheFlag && false===$rows) || !$rows)
            {
                if ($table_primary_key_value && '*'!=$table_primary_key_value)
                {
                    $this->sql->setWhere($field, $table_primary_key_value);
                }
                else
                {
                    if ('*' != $table_primary_key_value)
     ...
     * @access public
     */
    protected function addII($field_row, $return_insert_id=false)
    {
        //防止cache有误，可以考虑清下对应的cache，容错
        $add_flag = $this->_insertII($field_row);
     ...
     * @return mixed
     */
    public function select($field = '*',$cond_row,$group='',$order='',$flag = 'ASC')
    {
        if ($cond_row)
        {
            foreach ($cond_row as $k=>$v)
    ...
    public function selectCountSql($sql)
    {
     ...
     * @return array
     */
    public function select_sql($sql, $page=1, $rows=100, $flag=true)
    {
        $offset = $rows * ($page - 1);
        $this->sql->setLimit($offset, $rows);
        $total = $this->getFoundRows();
    ...
    public function selects($field = '*',$cond_row,$group,$order,$page=1,$rows=100,$asc='ASC')
    {
        $offset = $rows * ($page - 1);
        $this->sql->setLimit($offset, $rows);
        $total = $this->getFoundRows();
