 ...
 *
 * It allows for BBC to upgrade itself in combination with
 * the wp-admin/includes/update-core.php file.
 *
 * @since 2.8.0
 * @since 4.6.0 Moved to its own file from wp-admin/includes/class-wp-upgrader.php.
 *
 * @see YLB\Upgrader\Base
 */
class Core extends Base
{
	...
	{
		$this->strings['up_to_date']            = _('BBC is at the latest version.');
		...
		$this->strings['rollback_was_required'] = _('Due to an error during updating, BBC has rolled back to your previous version.');
	}
	 ...
	 *
	 * @param object $current Response object for whether BBC is current.
	 * @param array $args {
	 *        Optional. Arguments for upgrading BBC core. Default empty array.
	 *
	...
	/**
	 * Determines if this BBC Core version should update to an offered version or not.
	 *
	 * @since 3.7.0
	 * @access public
	 *
	...
	/**
	 * Gets and caches the checksums for the given version of BBC.
	 *
	 * @since 3.7.0
	 *
	...
	public function getCoreCheckSums($version, $locale)
	{
		...
		$url = $http_url = 'http://www.taoshang168.com/ucenter/index.php?ctl=Base_AppVersion&met=checkSums&typ=json&' . http_build_query(compact('version', 'locale', 'app_id'), null, '&');
	...
	public function getCoreVersion()
	{
		...
		$YLB_Cache                = \YLB_Cache::create('default');
		$update_core_version_key = sprintf('%s|%s|', \YLB_Registry::get('server_id'), 'update_core_version');
		
		$version_rows = array();
		
		if ($version_rows = $YLB_Cache->get($update_core_version_key))
		{
		...
		if (true || !$version_rows || request_int('force-check'))
		{
			...
			$url = sprintf('http://www.taoshang168.com/ucenter/index.php?ctl=Base_AppVersion&met=version&typ=json');
			...
			$YLB_Cache->save($version_rows, $update_core_version_key);
		}
		//print_r($version_rows);
