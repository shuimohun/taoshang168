...
class Exec
{
	...
	function updateCore($from, $to)
	{
		...
		{
			if (\YLB_Utils_File::exists($from . $root . '/configs/version.php'))
			{
				$distro = $root;
				break;
			}
		...
		if (!$distro)
		{
			\YLB_Utils_File::cleanDir($from, true);
		...
		if (!\YLB_Utils_File::copy($from . $distro . '/configs/version.php', $versions_file))
		{
			\YLB_Utils_File::delete($from, true);
		...
		\YLB_Utils_File::delete($versions_file);
		...
		*/
		...
		\YLB_Utils_File::delete($maintenance_file);
		file_put_contents($maintenance_file, $maintenance_string);
		
		update_feedback(_('正在复制所需的文件… '));
		
		// Copy new versions of WP files into place.
		//$result = \YLB_Utils_File::copyDir( $from . $distro, $to, $skip );
		$result = \YLB_Utils_File::copyDir($from, $to, $skip);
		
		
		// Since we know the core files have copied over, we can now copy the version file
		if ($result)
		{
			if (!\YLB_Utils_File::copy($from . $distro . '/configs/version.php', $to . $distro . '/configs/version.php'))
			{
				\YLB_Utils_File::delete($from, true);
		...
		*/
		...
		{
			throw new \Exception(sprintf(_('<strong>ERROR</strong>: BBC %1$s requires MySQL %2$s or higher'), $client_db_version, $required_mysql_version));
		}
	...
	public function makeDbCurrentSilent($structure_from)
	{
		...
		*/
		...
		if (!$databaseDiff->isEmptyDifferences())
		{
			...
			if ($flag)
			{
				$DbManage = new \YLB_Utils_DbManage($db_to, $this->handle->dbPrefix, $this->handle->dbPrefixBase, false);
