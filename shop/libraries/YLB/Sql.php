...
class YLB_Sql
{
	...
	private $limit  = ' LIMIT 2000 ';        /* 构造Sql语句Limit        */
	private $order  =  '';
		...
		switch ($symbol)
		{
			...
			case 'LIKE' :
				if(is_array($val))
				{
					...
					{
						$item = untrim($item);
						$this->where .= ' ' . $join . ' ' . $key . ' ' . $symbol . ' ' . $item;
					}
				}
				else
				{
	...
	public function setLimit($offset = 0, $rows = 2000)
	{
		$offset = max($offset, 0);
	...
	public function exePreDb()
	{
		...
		foreach (self::$dbTransRow as $key => $value)
		{
			...
			if (!$result)
			{
				...
				YLB_Log::log(encode_json($msg), YLB_Log::ERROR, 'transaction');
	...
	public function exePreRedis()
	{
		...
		{
			$objRedis = YLB_Cache::create('redis_data');
			$objRedis->startTrans();
