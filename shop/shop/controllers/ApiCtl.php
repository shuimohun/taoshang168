			...
			else
			{
				...
				if (!$check_db_rows)
				{
					...
					while($mysql=$this->GetNextSQL())
					{
						...
						if (false===$flag)
						{
							YLB_Log::log($mysql, YLB_Log::INFO, 'create_db');
						}
...
EOF;
						...
						$db_config_contents = "<?php\n	define(\"TABEL_PREFIX\", \"YLB_admin_\"); //表前缀;\n return ";
						$db_config_contents .= var_export($db_config_row, true);
						$db_config_contents .= ";\n?>";
...
\$shop_api_url = "http://{$url_row['host']}/index.php";
\$shop_app_id = 102;
\$shop_wap_url = "http://www.taoshang168.com/shop_wap/";
?>
EOF;
						file_put_contents($db_config_file, $db_config_contents);
	...
	{
        $data['code'] = Web_ConfigModel::value('mobile_apk_version');
        $data['name'] = Web_ConfigModel::value('mobile_apk_name');
        $data['mode'] = Web_ConfigModel::value('mobile_apk_mode');
        $data['desc'] = Web_ConfigModel::value('mobile_apk_desc');
        $data['downloadUrl'] = Web_ConfigModel::value('mobile_apk');
        $data['publishDate'] = Web_ConfigModel::value('mobile_apk_date');
        //$data['ios_download_url'] = Web_ConfigModel::value('mobile_ios');
        $data['title1'] = Web_ConfigModel::value('mobile_title1') ? Web_ConfigModel::value('mobile_title1') : '淘尚168商城';
        $data['title2'] = Web_ConfigModel::value('mobile_title2') ? Web_ConfigModel::value('mobile_title2') : '淘尚168商城';
		...
		{
            $this->data->addBody(-1,array(),'没有版本控制',250);
        }
	}
}
?>
