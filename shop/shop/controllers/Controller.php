...
class Controller extends YLB_AppController
{
	...
	public function catIndex()
	{
		...
		if($this->site_id == -3)
        {
            ...
            {
                if($value['cat_id'] == 2355)
                {
                    $data = $value['cat_nav']['goods_cat_nav_recommend_display'];
                }
            }
        }
		return $data;
	...
	public function footIndex()
	{
		...
		$data_help_foot  = $Help_GroupModel->getSiteFooterHelp();

		return $data_help_foot;
	}
	...
	public function navIndex()
	{
		...
		if($data['items'])
		{
			foreach($data['items'] as $key=>$value)
			{
                ...
                {
                    if(preg_match("/ctl=ScareBuy/", $value['nav_url']))
                    {
	...
	public function webConfig()
	{
        ...
        $web['self_logo']      = Web_ConfigModel::value("self_logo");//超市logo
        $web['fresh_logo']     = Web_ConfigModel::value("fresh_logo");//生鲜logo
        $web['supplier_logo']  = Web_ConfigModel::value("supplier_logo");//供应商logo
