...
class RedPacketCtl extends Controller
{
    ...
    public function redPacket()
    {
       ...
       //分页
        ...
        $YLB_Page->totalRows = $data['redpacket']['totalsize'];
        ...
        $title             = Web_ConfigModel::value("hongbao_title");//首页名;
        $this->keyword     = Web_ConfigModel::value("hongbao_keyword");//关键字;
        $this->description = Web_ConfigModel::value("hongbao_description");//描述;
    ...
    public function receiveRedPacket()
    {
        if(Perm::checkUserPerm())
        {
            ...
            {
                $ava_flag = true;
                if($row['redpacket_t_total'] && ($row['redpacket_t_total'] == $row['redpacket_t_giveout']))
                {
                    $ava_flag = false;
                    $msg  = _('红包已被领完');
                }
                else
                {
                    ...
                    if($row['redpacket_t_user_grade_limit'] > 1)
                    {
                        $user_info  = $this->userInfoModel->getUserInfo(array("user_id" => Perm::$userId));
                        if($row['redpacket_t_user_grade_limit'] > $user_info['user_grade'])
                        {
                            $ava_flag = false;
                            $msg  = _('对不起，您的等级不够！');
                        }
    ...
    public function getRedPacketByBase()
    {
        $base = request_string('base');
        $typ = request_string('typ');
        if($typ != 'json')
        {
            $this->data->setError(-140,'数据错误');die;
        }
        if($base == 'defaults')
        {
            $data['redpacket']['item'] = $this->redPacketBaseModel->getTemp('defaults');
            if($data)
            {
                $msg = 'success';
                $status = '200';
        ...
        {
            $data['redpacket']['item'] = $this->redPacketBaseModel->getTemp('count');
        ...
        {
            $data['redpacket']['item'] = $this->redPacketBaseModel->getTemp('much');
            if($data)
            {
                $msg = 'success';
                $status = '200';
        ...
        {
            $data['redpacket']['item'] = $this->redPacketBaseModel->getTemp('can');
            if($data)
            {
                $msg = 'success';
                $status = '200';
    ...
    public function redIndex()
    {
        ...
        if($typ != 'json')
        {
            $this->data->setError(-140,'获取数据失败');die;
        }
        ...
        if($base)
        {
            if(empty($tab))
            {
                $data = $this->redPacketBaseModel->redClassify($base);

            }
            else
            {
                $data = $this->redPacketBaseModel->redClassify($base,$tab);
            }
            if($data && count($data['item']) > 0)
            {
                $data['redpacket']['item'] = $data;
            }
            else
            {
                $data['redpacket']['item'] = array();
            }
            $this->data->addBody(-140,$data);
        }
        else
