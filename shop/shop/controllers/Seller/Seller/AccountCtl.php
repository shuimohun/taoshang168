...
class Seller_Seller_AccountCtl extends Seller_Controller
{
    ...
    public function accountList()
    {
        if(request_string('act') == 'add')
        {
            ...
            {
                //权限组不存在，需要先创建权限组
                location_to(YLB_Registry::get('url') . '?ctl=Seller_Seller_Group&met=groupList&act=add&typ=e');
            }
        ...
        else
        {
            ...
            if($data['items'])
            {
                ...
                foreach($data['items'] as $key=>$value)
                {
                    ...
                    {
                        $data['items'][$key]['group_name'] = _('未知的用户组');
                    }
                }
            }
    ...
    **/
    public function saveAccount()
    {
        ...
        {
            check_rs(false, $rs_row);
            $msg_label   = _('卖家账号已存在');    //一个用户不能管理多个店铺
        }
        if(is_ok($rs_row) && $this->isShopNameExist($seller_name))
        {
            check_rs(false, $rs_row);
            $msg_label   = _('卖家账号已存在');    //自营
        }
        if(is_ok($rs_row))
        {
            $field_row = array();
        ...
        {
            $msg    = isset($msg_label)?$msg_label:_('添加失败！');
            $status = 250;
        }
    ...
    */
    public function editAccountSave()
    {
        ...
        {
            $msg    = isset($msg_label)?$msg_label:_('编辑失败！');
            $status = 250;
        }
    ...
    */
    ...
    private function isShopNameExist($seller_name) {
        $cond_row = array();
        $cond_row['user_name'] = $seller_name;
        $Shop_base = new Shop_BaseModel();
        return  $Shop_base->isShopExist($cond_row);
    }
