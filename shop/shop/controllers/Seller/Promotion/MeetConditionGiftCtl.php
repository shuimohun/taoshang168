...
class Seller_Promotion_MeetConditionGiftCtl extends Seller_Controller
{
	...
	public function add()
	{
		...
		if (!$this->self_support_flag)  //普通店铺
		{
			if (!$this->combo_flag)
			{
				location_to(YLB_Registry::get('url') . '?ctl=Seller_Promotion_MeetConditionGift&met=index&typ=e');
			}
			else
			{
                $data['combo'] = $this->manSongQuotaModel->getManSongQuotaDetailByWhere(array('shop_id' => Perm::$shopId));
			}
		}
		else // 自营店铺
		{
            $data['combo']['combo_end_time'] = date("Y-m-d H:i:s", strtotime("11 june 2030"));
		}

		if ($this->getLatestManSongDate())
		{
            $data['combo']['combo_start_time'] = $this->getLatestManSongDate();
		}
		else
		{
            $data['combo']['combo_start_time'] = get_date_time();
		}
	...
	public function getLatestManSongDate()
	{
        ...
        $row = $this->manSongBaseModel->getOneByWhere(array('shop_id' => Perm::$shopId),array('mansong_end_time'=>'DESC'));
        if($row)
        {
            $latest_date = $row['mansong_end_time'];
        }
			...
			if ($check_post_data_flag)
			{
				...
				if ($flag)
				{
					foreach ($mansong_rule as $key => $rule)
					{
                        ...
                        if($field_rule_row['rule_price'] > 0)
                        {
                            if($field_rule_row['goods_id'])
                            {
                                $goods_row = $this->goodsBaseModel->getKeyByWhere(array('goods_id'=>$field_rule_row['goods_id'],'shop_id'=>Perm::$shopId));
                                //检查赠品是否是该店铺的商品
                                if ($goods_row && $field_rule_row['rule_discount'] >= 0)
                                {
                            ...
                            else if($field_rule_row['rule_discount'] > 0)
                            {
                                $insert_flag = $this->manSongRuleModel->addManSongRule($field_rule_row, true);
                                check_rs($insert_flag, $rs_row);
                            }
	...
	public function removeManSong()
	{
		...
		if ($check_right['shop_id'] == Perm::$shopId)
		{
            ...
            //删除满送活动及活动下的规则
			$flag = $this->manSongBaseModel->removeManSongActItem($mansong_id);
