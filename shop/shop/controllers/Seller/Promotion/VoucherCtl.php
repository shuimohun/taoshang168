...
class Seller_Promotion_VoucherCtl extends Seller_Controller
{
	...
	{
	    $BaseGoodsModel = new Goods_BaseModel();
		...
		if (request_string('op') == 'detail')
		{
			...
			//新增 若有goods则取商品
            if($data['voucher_t_wap_show_goods']){
                $goods_base = explode(',',$data['voucher_t_wap_show_goods']);
                foreach($goods_base as $key=>$value){
                    $base = $BaseGoodsModel->checkGoodsII($value);
                   if($base){
                       $data['goodslist'][]= $base['goods_base'];
                   }
                }
            }
	...
	public function addVoucherTemp()
    {
		...
		$field_row['voucher_t_start_date']       = date("Y-m-d");                            //開始時間
        ...
        else if($field_row['voucher_t_limit'] < $field_row['voucher_t_price'])
        {
            $check_post_data_flag = false;
            $msg_label = _('消费金额必须大于代金券面额！');
        }
        ...
        //新增添加商品 start
         $goods_row = request_row('join_act_goods_id');
        if($goods_row){
            $field_row['voucher_t_wap_show_goods'] = trim(implode(',',$goods_row),',');
        }
        //新增添加商品  end
...
//            }
	...
	public function getShopGoods(){
        ...
        $offset            = request_int('firstRow', 0);
        $page              = ceil_r($offset / $rows);
        $CommonGoodsModel  = new Goods_CommonModel();
        //需要加入商品状态限定
        $cond_row['shop_id'] = Perm::$shopId;
        ...
        //只取状态正常的商品的
        $data = $CommonGoodsModel->getNormalSateGoodsBase($cond_row, array('goods_id' => 'DESC'), $page, $rows);
