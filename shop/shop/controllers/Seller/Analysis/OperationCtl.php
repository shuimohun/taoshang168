...
class Seller_Analysis_OperationCtl extends Seller_Controller
{
    ...
    public function index()
    {
        ...
        if(!request_string('start_time')){
            $cond_row['start_time'] = '1970-01-01';
        }else{
            $cond_row['start_time'] = request_string('start_time');
        }
        if(!request_string('end_time')){
            $cond_row['end_time'] = date("Y-m-d H:i:s",time());
        }else{
            $cond_row['end_time'] = request_string('end_time');
        }
