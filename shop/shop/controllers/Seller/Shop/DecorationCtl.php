...
class Seller_Shop_DecorationCtl extends Seller_Controller
{
	...
	public function editRenovation()
	{
        ...
        $flag = $renovation_list = $this->shopBaseModel->editBase($shop_id, $renovation);
		if ($flag === FALSE)
		{
			$status = 250;
			$msg    = _('failure');
	...
	public function goodsSearch()
	{
		...
		if(request_row('common_id'))
        {
            $condition['common_id:NOT IN'] = request_row('common_id');
        }
		$common_list                     = $GoodsCommonModel->getGoodsListII($condition, array(), 1, 10);
	...
	public function decorationBlockSave()
	{
		...
		/*if(strlen($content) > 46000)
        {
            $data['error'] = '最多上传46000个字符';
            echo json_encode($data);
            die;
        }*/
