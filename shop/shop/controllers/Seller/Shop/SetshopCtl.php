...
class Seller_Shop_SetshopCtl extends Seller_Controller {
    ...
    public function slideWap()
    {
        ...
        $shop_id        = Perm::$shopId;
        $re             = $this->shopBaseModel->getOne($shop_id);
        $de['slide']    = $re['shop_slide_wap'] ? explode(',', $re['shop_slide_wap']) : "";
        $de['slideurl'] = $re['shop_slide_wap'] ? explode(',', $re['shop_slideurl_wap']) : "";
        if ('json' == $this->typ)
        {
            $status           = 200;
            $msg              = _('success');
    ...
    public function editSlideWap()
    {

        $data['shop_slide_wap']    = implode(",", request_row("slide"));
        $data['shop_slideurl_wap'] = implode(",", request_row("slideurl"));
        $shop_id               = Perm::$shopId;
        $flag                  = $this->shopBaseModel->editBase($shop_id, $data);
        if ($flag === false)
        {
