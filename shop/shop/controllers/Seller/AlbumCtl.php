...
class Seller_AlbumCtl extends Seller_Controller
{
	...
	{
	    if(request_string('type') == 'recycle')
        {
            //图片回收站
            $this->view->setMet('recycle');
        }
        include $this->view->getView();
	...
	public function lists()
	{
        ...
        $rows    = $YLB_Page->listRows = request_int('rows',15);
		$user = request_string('user');
        ...
        {
            $cond_row['upload_name:LIKE'] = '%'.$search_key.'%';
        }
        ...
        {
            $cond_row['is_recycle'] = $param['recycle'];
        }
        else
        {
            $cond_row['is_recycle'] = Upload_BaseModel::NORECYCLE;
        }

        $order_row['upload_id'] = 'desc';
...
//		$this->uploadAlbumModel->editAlbum($data['album_id'], array());
    ...
    public function removeRecycle()
    {
        ...
        if (is_array($upload_id))
        {
            foreach ($upload_id as $id)
            {
                $flag = $this->uploadModel->removeImageRecycle($id);
            }
        }
        else
        {
            $flag = $this->uploadModel->removeImageRecycle($upload_id);
        }
	...
	public function getAlbumList()
	{
		...
		$data             = $this->uploadAlbumModel->getAlbumList($condi,array('album_id'=>'desc'),$page,$rows);
	...
	//回收站图片还原
    public function ruturnBackRecycle()
    {
        $upload_id = $_REQUEST['upload_id'];
        ...
        if(is_array($upload_id))
        {
            foreach ($upload_id as $id)
            {
                $upload = $this->uploadModel->getOne($id);
                if($upload)
                {
                    $flag = $this->uploadModel->editUpload($id, array('is_recycle'=>Upload_BaseModel::NORECYCLE));
                }
            }
        }
        else
        {
            $upload = $this->uploadModel->getOne($upload_id);
            if($upload)
            {
                $flag = $this->uploadModel->editUpload($upload_id, array('is_recycle'=>Upload_BaseModel::NORECYCLE));
            }
        }
