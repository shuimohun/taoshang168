...
class Supplier_GoodsCtl extends Controller
{
	...
	public function addto_distributor()
	{
		...
		if($old_common)
        {
            ...
            if($shop_info['shop_all_class'])
            {
                $cat_flag = true;
            }
            else
            {
                $cat_flag = $GoodsCatModel->isHaveCatRight($old_common['cat_id']);
            }

            if($cat_flag)
            {
                //查看是否已经有同步数据
                $check_common = $Goods_CommonModel->getKeyByWhere(array('shop_id'=>$shop_id,'common_parent_id'=>$old_common_id,'product_is_behalf_delivery' => 1));
                ...
                if(!$check_common)
                {
                    ...
                    if($shopDistributorBase['distributor_enable'] == 1 && (array_intersect($old_shop_cat_id, $allow_shop_cat) || empty($old_shop_cat_id)))
                    {
                    ...
                    else if($shopDistributorBase['distributor_enable'] == '0')
                    {
                        $msg = _('分销商申请未通过！');
                        $status = 250;
                    }
    ...
    public function goodslist()
    {
        ...
        else
        {
            ...
            $cond_row['product_distributor_flag'] = 1;
            $hot_sale = $Goods_CommonModel->getGoodsListII($cond_row, $hot_order_row, 1, 6);
            ...
            if (!$hot_sale)
            {
                ...
                $hot_order_row['common_salenum'] = 'DESC';
                $hot_sale = $Goods_CommonModel->getGoodsListII($hot_cond_row, $hot_order_row, 1, 6);
                $hot_sale = $hot_sale['items'];
            }
            ...
            $com_cond_row['product_distributor_flag'] = 1;
            //如果有查找的分类就显示该分类下的推广商品，如果没有传递分类就显示最新设置的分类推广
            if ($cat_id)
            {
                $rec_cond_row['goods_cat_id'] = $cat_id;
            }
            $recommend_row = $Goods_RecommendModel->getRecommendGoods($rec_cond_row, $rec_order_row,$com_cond_row);
	...
	public function goodslistII()
	{
		...
		$cond_row['common_state'] = Goods_CommonModel::GOODS_STATE_NORMAL;
		$cond_row['common_verify'] = Goods_CommonModel::GOODS_VERIFY_ALLOW;
		$data                     = $Goods_CommonModel->getGoodsListII($cond_row, $order_row, $page, $rows, $property_value_row);
...
//		var_dump($data['items']);exit;
    ...
    public function goods()
    {
        ...
        if($cid && !$goods_id)
        {
            $goods_id = $Goods_CommonModel->getGoodsId($cid);
        }
        ...
        $Goods_BaseModel = new Goods_BaseModel();
        $goods_detail    = $Goods_BaseModel->getGoodsDetailByGoodId($goods_id);
        ...
        else
        {
            ...
            if (Perm::checkUserPerm())
            {
                ...
                //先判断该用户是否浏览过该商品
                $foot_cond_row['user_id']   = Perm::$userId;
                ...
                else
                {
                    //如果没有浏览过改商品则插入数据
                    $read_add_row                   = array();
                    $read_add_row['user_id']        = Perm::$userId;
            ...
            //获取商品所在地
            $Base_DistrictModel = new Base_DistrictModel();
            $goods_location_row = $Base_DistrictModel->getByWhere(array('district_id:IN' => $goods_detail['goods_common']['common_location']));
            $goods_location = '';
        ...
        if ('json' == $this->typ)
        {
            ...
            //商品详情
            $goods_info = array_merge($goods_detail['goods_common'], $goods_detail['goods_base']);

            //好评率
            $Goods_EvaluationModel =  new Goods_EvaluationModel();
            $all_count    = $Goods_EvaluationModel->countEvaluation($common_id, 'all');
            $good_count   = $Goods_EvaluationModel->countEvaluation($goods_detail['goods_common']['common_id'], 'good');
            if($all_count != 0)
            {
	...
	/**
	 * 商品详情页  goodsdetailinfo  备份Zhenzh
	 * @access public
	 */
	public function goodsII()
	{
		$Goods_CommonModel = new Goods_CommonModel();
		$cid = request_int('cid');
		$goods_id        = request_int('gid', request_int('goods_id'));
		...
		else
		{
			...
			if ($user_id)
			{
				...
				//商品限购数量判断
				if ($goods_detail['goods_common']['common_limit'] > 0 && $order_goods_count >= $goods_detail['goods_common']['common_limit'])
				{
					$IsHaveBuy = 1;
				}
