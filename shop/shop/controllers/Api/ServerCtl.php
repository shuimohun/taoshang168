...
class Api_ServerCtl extends Api_Controller
{
	...
	public function create()
	{
		...
		$paycenter_url = request_string('paycenter_url', 'http://www.taoshang168.com/paycenter/index.php');
		$ucenter_url   = request_string('ucenter_url', 'http://www.taoshang168.com/ucenter/index.php');
			...
			else
			{
				...
				if (!$check_db_rows)
				{
					...
					foreach ($dir as $item)
					{
						...
						if (is_file($file))
						{
							$flag = $db->import($file, TABEL_PREFIX, 'YLB_', false);
					...
					foreach ($dir as $item)
					{
						...
						if (is_file($file))
						{
							$flag = $db->import($file, 'YLB_admin_', 'YLB_admin_', false);
							check_rs($flag, $rs_row);
						}
					}
