...
class Api_Goods_CatCtl extends Api_Controller
{
    ...
    public function getCatNew()
    {
        ...
        $Goods_CatModel = new Goods_CatModel();
        $data           = $Goods_CatModel->getCatTreeDataByStep($cat_id, 0, 0);
        $this->data->addBody(-140, array_values($data));
    }

    //Zhenzh 根据一个cat_id 取出所有上级分类 及下级分类
    public function getCat()
    {
        $cat_id    = request_string('cat_id', 0);
        $Goods_CatModel = new Goods_CatModel();
        $cat_all = $Goods_CatModel->getAllParentCat($cat_id);

        $cat_all = array_reverse($cat_all);

        $this->data->addBody(-140, array_values($cat_all));
    }
	...
	public function addCat()
	{
        ...
        $Goods_CatModel = new Goods_CatModel();
		$cat_id = $Goods_CatModel->addCat($data, true);
	...
	public function editGoodsCat()
	{
		...
		if ($flag !== false && !empty($edit_data))
		{
			...
			if($cat_pic)
            {
                ...
                $goods_top_id = $GoodsTopCatModel->getKeyByWhere(array('cat_id'=>$cat_id));
                $GoodsTopCatModel->editTopCat($goods_top_id,array('cat_pic'=>$cat_pic));
            }
		}
		if ($flag !== false)
		{
