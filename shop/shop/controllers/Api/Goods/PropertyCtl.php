...
class Api_Goods_PropertyCtl extends Api_Controller
{
	...
	public function getPropertyValue()
	{
		...
		$property_value_row  = $Goods_PropertyValue->getByWhere(array('property_id' => $property_id),array('property_value_displayorder'=>'asc'));
	...
	public function addProperty()
	{
        ...
        if (empty($type_id))
        {
            ...
            $type_base       = $Goods_TypeModel->getByWhere(array('type_draft' => 1,'operate_id'=>$user_id));
            //如果没有草稿则创建
            if (empty($type_base))
            {
                $type_id = $Goods_TypeModel->addType(array('type_draft' => 1,'operate_id'=>$user_id), true);
            }
            else
            {
        ...
        $edit_row['property_is_display']    = request_int('property_is_display');
