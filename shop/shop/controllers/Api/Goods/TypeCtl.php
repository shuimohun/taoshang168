...
class Api_Goods_TypeCtl extends Api_Controller
{
	...
	public function lists()
	{
		...
		$data            = $Goods_TypeModel->getTypeList($condi, $sord, $page, $rows);
//        var_dump($data);die;
        if ($data)
        {
            $msg    = _('success');
	...
	public function addType()
	{
        ...
        $data['operate_id'] = $user_id;

		//加入草稿 优先判断有没有草稿
		$goods_base = $Goods_TypeModel->getByWhere(array('type_draft' => 1,'operate_id'=>$user_id));
    ...
    {
        $type_id = request_int('type_id');
        $data    = $this->goodsTypeModel->getone($type_id);
