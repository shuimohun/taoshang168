        ...
        if ( is_array($layout_list) )
        {
            ...
            if ( !empty($layout_list) )
            {
                ...
                foreach($layout_list as $item_id => $item_data )
                {
                    ...
                    if ( $item_data['mb_tpl_layout_type'] == 'shop' )
                    {
                        $shop_ids = $item_data['mb_tpl_layout_data'];

                        $shopBaseModel = new Shop_BaseModel();
                        $shop_list = $shopBaseModel->getByWhere(array('shop_id:IN'=>$shop_ids));
                        if($shop_list)
                        {
                            $layout_data = array();
                            foreach ($shop_list as $shop_key => $shop_value)
                            {
                                $layout_data[$shop_key]['shop_id'] = $shop_value['shop_id'];
                                $layout_data[$shop_key]['shop_name'] = $shop_value['shop_name'];
                                $layout_data[$shop_key]['shop_status'] = $shop_value['shop_status'];
                                $layout_data[$shop_key]['shop_logo'] = $shop_value['shop_logo'];
                            }
                            $layout_list[$item_id]['mb_tpl_layout_data'] = array_values($layout_data);
                        }
