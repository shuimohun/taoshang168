...
//
	...
	public function shopIndex()
	{
		...
		if($data['items'])
        {
            $goods_cat_ids = array_column($data['items'],'goods_cat_id');
            $GoodsCatModel = new Goods_CatModel();
            $goods_cat_rows = $GoodsCatModel->getByWhere(array('cat_id:IN'=>$goods_cat_ids));

            foreach ($data['items'] as $key=>$value)
            {
                if(array_key_exists($value['goods_cat_id'],$goods_cat_rows))
                {
                    $data['items'][$key]['goods_cat_name'] = $goods_cat_rows[$value['goods_cat_id']]['cat_name'];
                }
            }
        }
	...
	public function editShopclassrow()
	{
		...
		$data['shop_class_deposit']      = request_row("shop_class_deposit");
		$data['shop_class_displayorder'] = request_row("shop_class_displayorder");
        $data['goods_cat_id']            = request_int("goods_cat_id");
		$add                             = $this->shopClassModel->editClass($shop_class_id, $data);
        $data['goods_cat_name']          = request_string("goods_cat_name");
		$data['shop_class_id']           = $shop_class_id;
		$this->data->addBody(-140, $data);
	}
