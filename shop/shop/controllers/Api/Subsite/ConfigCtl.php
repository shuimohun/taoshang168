 ...
 * @author     WenQingTeng
 */
class Api_Subsite_ConfigCtl extends Api_Controller
{
	...
	{
//		$sub_site_parent_id = request_int('nodeid', 0);
//		$sub_site_level     = request_int('n_level', 0);
//		$data = $this->Subsite_BaseModel->getSubsiteTree($sub_site_parent_id, false, $sub_site_level); //暂时不做分站的分级
	...
	public function addSubsite(){	
		...
		$data['sub_site_domain'] 			= request_string('sub_site_domain');
		...
		if(!$data['sub_site_name'] || !$data['sub_site_domain']){
			$msg 	= _('必须填写分站名和分站域名前缀');
			$status = 250;
		}else{
	...
	{	
		...
		$edit_data['sub_site_parent_id'] 		= request_int('parent_subsite');
		$edit_data['sub_site_domain'] 			= request_string('sub_site_domain');
		...
		if(!$edit_data['sub_site_name']||!$edit_data['sub_site_domain']){
			$msg 	= _('必须填写分站名和分站域名前缀');
			$status = 250;
		}else{
			// 执行修改操作
    ...
    public function addSubsiteDistrict(){
        ...
        if(!trim($district_ids,',')){
            $error = array('flag'=>false, 'tip'=>'请选择分站地区');
        }else if($num1 !== $num2){
