...
class Api_Operation_ContractCtl extends Api_Controller
{
	...
	public function getTypeList()
	{
		...
		$sidx = request_string('sidx','contract_type_sort');
		...
		$data = $this->contractTypeModel->getContractTypeList(array(), $sort, $page, $rows);
	...
	public function editContract()
	{
        ...
        $flag = true;
		$contract = $this->contractModel->getOneContract($contract_id);
		if($contract)
        {
            if($contract['contract_state'] != $data['contract_state'])
            {
                $flag = $this->contractModel->editContract($contract_id, $data);
            }
        }
	...
	public function getContractStateList()
	{
        ...
        if($typ=='shz'){
            $cond_row['contract_log_state:in'] = array("1","2");
        }
		if ($oname != "number")
		{
			$sort[$oname] = $osort;
