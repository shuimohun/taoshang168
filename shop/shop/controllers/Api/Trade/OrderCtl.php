...
class Api_Trade_OrderCtl extends Api_Controller
{
	...
	public function getOrderList()
	{
        ...
        if($type==1){
            ...
            $cond_row['payment_time:>='] = date('Y-m-d 00:00:00', time());
            $cond_row['payment_time:<='] = get_date_time();

        }else{
            if (request_int('order_status') && request_int('order_status') > 0)
            {
                $cond_row['order_status'] = request_int('order_status');
            }
        }

        if (request_int('payment_channel_id') && request_int('payment_channel_id') > 0)
        {
            $cond_row['payment_channel_id'] = request_int('payment_channel_id');
        }
        ...
        if($sub_flag == false){
            ...
            $data = $this->Order_BaseModel->getPlatOrderList($cond_row, $order_row, $page, $rows);
            $t = time();
            $cond_row['order_status'] = 2;
            if(!request_string('payment_date_f')){
                $cond_row['payment_time:>='] = date("Y-m-d H:i:s",mktime(0,0,0,date("m",$t),date("d",$t),date("Y",$t)));  //当天开始时间
            }
            if(!request_string('payment_date_t')){
                $cond_row['payment_time:<='] = date("Y-m-d H:i:s",mktime(23,59,59,date("m",$t),date("d",$t),date("Y",$t))); //当天结束时间
            }
            $field=' sum(order_goods_amount)as order_goods_amount ';
            $order_count_price =  $this->Order_BaseModel->getStatistics($field,$cond_row);
            $data['order_count_price'] =$order_count_price[0]['order_goods_amount'];
            $data['order_count'] = $this->Order_BaseModel->getCount($cond_row);
            if ($data['records'] > 0)
            {
                $status = 200;
                $msg    = _('success');
	...
	public function editOrderRowSatus()
	{
        ...
        $payment_channel_id = request_string('payment_channel_id');
		...
		if (is_array($order_id))
		{
			...
			foreach ($order_id as $key => $val)
			{
				...
				$flag = $this->tradeOrderModel->editOrderStatusAferPay($val,$uorder_id,$payment_channel_id);
                ...
                if(!empty($sp_order)){
                    foreach ($sp_order as $k => $v) {
                        $this->tradeOrderModel->editOrderStatusAferPay($v['order_id'],null,$payment_channel_id);
		...
		else
		{
            ...
            YLB_Log::log("r1=" . $order_id, YLB_Log::INFO, 'zhifu');
			$flag = $this->tradeOrderModel->editOrderStatusAferPay($order_id,$uorder_id,$payment_channel_id);
     ...
     * 根据订单id获取该订单下的交易快照
     */
	public function getSnapshotByOrder()
    {
        $order_id = request_string('order_id');
        $OrderGoodsSnapshotModel = new Order_GoodsSnapshotModel();
        $data = $OrderGoodsSnapshotModel->getByWhere(array('order_id'=>$order_id));
