...
class WebPosApi_OrderCtl extends WebPosApi_Controller
{
	...
	public function getcontactInfo()
	{
		...
		$money_row = get_url_with_encrypt($key, sprintf('%sindex.php?ctl=Api_User_Info&met=getUserResourceInfo&typ=json', YLB_Registry::get('paycenter_api_url')), $formvars);
		...
		if($paymentMethod != 1)    //不是现金支付
		{
			...
			if ($rs['status'] == 200)
			{
				...
				if($paymentMethod == 2) //账户余额支付
				{
					...
					$pay_res = get_url_with_encrypt($key, sprintf('%sindex.php?ctl=Api_Pay_Pay&met=money&typ=json', YLB_Registry::get('paycenter_api_url')), $formvars);
					fb($pay_res);
				}
