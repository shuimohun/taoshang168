 ...
 * @author     Zhenzh
 */
class Self_GoodsCtl extends Controller
{
    ...
    public function goodslist()
    {
        ...
        else
        {
            ...
            if (!$hot_sale)
            {
                ...
                $hot_cond_row['shop_self_support'] = 1;
            ...
            $com_cond_row['shop_self_support'] = 1;
	...
	public function goodslistII()
	{
		...
		//YLB_share b 减免后价格：a.common_price - b.share_total_price 分享立减：b.promotion_total_price  分享立赚：b.promotion_total_price
		$shareModel = new Share_BaseModel();
		//$sharesData = $shareModel->getShareByCommonids($common_ids);
		...
		foreach($data['items'] as $key=>$val){
			foreach($sharesData as $k=>$v){
				if($v['common_id'] == $val['common_id']){
					$data['items'][$key]['share_total_price'] = $v['share_total_price'];
					$data['items'][$key]['promotion_total_price'] = $v['promotion_total_price'];
					$data['items'][$key]['share_price']['sqq'] = $v['sqq'];        //qq好友
					$data['items'][$key]['share_price']['qzone'] = $v['qzone'];    //qq空间
					$data['items'][$key]['share_price']['tsina'] = $v['tsina'];    //新浪
					$data['items'][$key]['share_price']['weixin'] = $v['weixin'];  //微信
					$data['items'][$key]['share_price']['weixin_timeline'] = $v['weixin_timeline'];    //微信朋友圈
				}
			}
		}
...
//		echo '<pre>';print_r($uv);exit;
    ...
    public function act()
    {
        ...
        $cond_row['shop_self_support'] = '1';
        $cond_row['product_distributor_flag'] = 0;
        $data = $GoodsCommonModel->getGoodsListII($cond_row,array('common_add_time'=>'DESC'),0,100);
