 ...
 * @author     WenQingTeng
 */
class RuleCtl extends Controller
{
    public function index(){
        if (Perm::$shopId) {
            ...
            $cond3['help_or_rule'] = '规则';
            $order3['help_grop_sort'] = 'asc';
            $Type = $HelpBase->listByWhere($cond3, $order3, $page3, $rows3);
            $cond4['help_type'] = 0;
            $rows4 = 3;
            $page4 = 1;
            $order4['help_sort'] = "asc";
            $cond4['help_or_rule'] = '规则';

            $Top = $HelpBase->listByWhere($cond4, $order4, $page4, $rows4);
            include $this->view->getView();
        }else{
            echo  "<script> alert( '不是商家用户,无法访问!'); window.location.href='javascript:history.go(-1)'</script>";
