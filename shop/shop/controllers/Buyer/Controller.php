...
class Buyer_Controller extends Controller
{
	...
	public function initBuyerInfo()
	{
		...
		$order_row['information_islook'] = 0;
		$order_row['information_status'] = 1;
		$this->Message = $this->informationBaseModel->getCount($order_row);
		$user_am = $this->user['info']['user_am'];

		if($user_am){
			$row = explode(",",$user_am);
			$order_row                   = array();
			$order_row['information_id:in']   = $row;
			$this->Umessage                      = $this->informationBaseModel->getCount($order_row);
			$this->Message  = $this->Message*1 - $this->Umessage*1;
		}

		$this->countMessage['information']      = $this->Message;
		$this->countMessage['countMessage'] = $this->countMessage['receive'] * 1 +$this->countMessage['message'] * 1 + $this->countMessage['information'] * 1;


        $CartModel = new CartModel();
        $this->cart_count  = $CartModel->getCartGoodsNum(array('user_id'=>$uid));
