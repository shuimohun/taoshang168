 ...
 *
 */
class Plugin_Analytics implements YLB_Plugin_Interface
{
	function __construct()
	{
        ...
        //关键字
		YLB_Plugin_Manager::getInstance()->register('analyticsKeywords', $this, 'keywords');
        //注册会员
        YLB_Plugin_Manager::getInstance()->register('analyticsMemberAdd', $this, 'memberAdd');
        //入驻商家
        YLB_Plugin_Manager::getInstance()->register('analyticsShopAdd', $this, 'shopAdd');
        //商家信誉
        YLB_Plugin_Manager::getInstance()->register('analyticsShopCredit', $this, 'shopCredit');
        //添加订单
        YLB_Plugin_Manager::getInstance()->register('analyticsOrderAdd', $this, 'orderAdd');
        //更新订单
        YLB_Plugin_Manager::getInstance()->register('analyticsUpdateOrderStatus', $this, 'updateOrderStatus');
        //添加商品
        YLB_Plugin_Manager::getInstance()->register('analyticsGoodsAdd', $this, 'goodsAdd');
        //更新更商品
        YLB_Plugin_Manager::getInstance()->register('analyticsGoodsEdit', $this, 'goodsEdit');
        //uv
        YLB_Plugin_Manager::getInstance()->register('analyticsUvCount', $this, 'uvCount');
        //商品收藏
        YLB_Plugin_Manager::getInstance()->register('analyticsProductCollect', $this, 'productCollect');
        //商品取消收藏
        YLB_Plugin_Manager::getInstance()->register('analyticsProductCancleCollect', $this, 'productCancleCollect');
        //商品评分
        YLB_Plugin_Manager::getInstance()->register('analyticsScore', $this, 'Score');
        //店铺收藏
        YLB_Plugin_Manager::getInstance()->register('analyticsShopCollect', $this, 'shopCollect');
        //店铺取消收藏
        YLB_Plugin_Manager::getInstance()->register('analyticsShopCancleCollect', $this, 'shopCancleCollect');
    ...
    public function keywords($keywords){
        ...
        $formvars['app_id']    = YLB_Registry::get('analytics_app_id');
        $init_rs = $this->_sendData(YLB_Registry::get('analytics_api_key'), sprintf('%s?ctl=Api_Shop_Statistics&met=keywords&typ=json', YLB_Registry::get('analytics_api_url')), $formvars);
        return $init_rs;
    }
    ...
    public function memberAdd($formvars = array()){
        ...
        $formvars['app_id']    = YLB_Registry::get('analytics_app_id');
        $init_rs = $this->_sendData(YLB_Registry::get('analytics_api_key'), sprintf('%s?ctl=Api_Shop_Statistics&met=memberAdd&typ=json', YLB_Registry::get('analytics_api_url')), $formvars);
        return $init_rs;
    }
    ...
    public function shopAdd($formvars = array()){
        ...
        $formvars['app_id']    = YLB_Registry::get('analytics_app_id');
        $init_rs = $this->_sendData(YLB_Registry::get('analytics_api_key'), sprintf('%s?ctl=Api_Shop_Statistics&met=shopAdd&typ=json', YLB_Registry::get('analytics_api_url')), $formvars);
        return $init_rs;
...
//        }
//        $formvars['app_id']    = YLB_Registry::get('analytics_app_id');
//        $init_rs = $this->_sendData(YLB_Registry::get('analytics_api_key'), sprintf('%s?ctl=Api_Shop_Statistics&met=shopUpdate&typ=json', YLB_Registry::get('analytics_api_url')), $formvars);
//        return $init_rs;
//
//    }
    ...
    public function orderAdd($data = array()){
        ...
        $analytics_data['app_id']    = YLB_Registry::get('analytics_app_id');
        $init_rs = $this->_sendData(YLB_Registry::get('analytics_api_key'), sprintf('%s?ctl=Api_Shop_Statistics&met=orderAdd&typ=json', YLB_Registry::get('analytics_api_url')), $analytics_data);
        return $init_rs;
    }
...
//        echo '<pre>';print_r($Goods);exit;
        ...
        if (!empty($goods_ids))
        {
            $goods_base_ids = array_column($goods_ids, 'goods_id');
        }
        $goods_base_ids = array_values($goods_base_ids);
        foreach($goods_base_ids as $key=>$value)
        {
            $formvars = array();
            ...
            $formvars['url'] = YLB_Registry::get('url').'?ctl=Goods_Goods&met=goods&type=goods&gid='.$goods_base['goods_id'];
            ...
            $init_rs = $this->_sendData(YLB_Registry::get('analytics_api_key'), sprintf('%s?ctl=Api_Shop_Statistics&met=productAdd&typ=json', YLB_Registry::get('analytics_api_url')), $formvars);
        }
    }
    ...
    public function goodsEdit($Goods){
        ...
        foreach($goods_base_ids as $key=>$value)
        {
            ...
            $formvars['app_id']    = YLB_Registry::get('analytics_app_id');
            $init_rs = $this->_sendData(YLB_Registry::get('analytics_api_key'), sprintf('%s?ctl=Api_Shop_Statistics&met=productUpdate&typ=json', YLB_Registry::get('analytics_api_url')), $formvars);
        }
    }
    ...
    public function uvCount($uvCount)
    {
        ...
        $formvars['app_id']    = YLB_Registry::get('analytics_app_id');
        $init_rs = $this->_sendData(YLB_Registry::get('analytics_api_key'), sprintf('%s?ctl=Api_Shop_Statistics&met=totalCount&typ=json', YLB_Registry::get('analytics_api_url')), $formvars);
//        echo '<pre>';print_r($init_rs);exit;
    ...
    public function productCollect($data)
    {
        ...
        $formvars['app_id']    = YLB_Registry::get('analytics_app_id');
        $init_rs = $this->_sendData(YLB_Registry::get('analytics_api_key'), sprintf('%s?ctl=Api_Shop_Statistics&met=productFollow&typ=json', YLB_Registry::get('analytics_api_url')), $formvars);
        return $init_rs;
    }
    ...
    public function productCancleCollect($data)
    {
        ...
        $formvars['app_id']    = YLB_Registry::get('analytics_app_id');
        $init_rs = $this->_sendData(YLB_Registry::get('analytics_api_key'), sprintf('%s?ctl=Api_Shop_Statistics&met=productCancleFollow&typ=json', YLB_Registry::get('analytics_api_url')), $formvars);
        return $init_rs;
    }
...
//        echo '<pre>';print_r($formvars);exit;
        $formvars['app_id']    = YLB_Registry::get('analytics_app_id');
        $init_rs = $this->_sendData(YLB_Registry::get('analytics_api_key'), sprintf('%s?ctl=Api_Shop_Statistics&met=Score&typ=json', YLB_Registry::get('analytics_api_url')), $formvars);
        return $init_rs;
    }
    ...
    public function shopCollect($data)
    {
        ...
        $formvars['app_id']    = YLB_Registry::get('analytics_app_id');
        $init_rs = $this->_sendData(YLB_Registry::get('analytics_api_key'), sprintf('%s?ctl=Api_Shop_Statistics&met=shopCollect&typ=json', YLB_Registry::get('analytics_api_url')), $formvars);
        return $init_rs;
    }
    ...
    public function shopCancleCollect($data)
    {
        ...
        //echo '<pre>';print_r($formvars);exit;
        $formvars['app_id']    = YLB_Registry::get('analytics_app_id');
        $init_rs = $this->_sendData(YLB_Registry::get('analytics_api_key'), sprintf('%s?ctl=Api_Shop_Statistics&met=shopCancleCollect&typ=json', YLB_Registry::get('analytics_api_url')), $formvars);
        return $init_rs;
    }
    ...
    public function updateOrderStatus($formvars = array()){
        ...
        $formvars['app_id']    = YLB_Registry::get('analytics_app_id');
        $init_rs = $this->_sendData(YLB_Registry::get('analytics_api_key'), sprintf('%s?ctl=Api_Shop_Statistics&met=updateOrderStatus&typ=json', YLB_Registry::get('analytics_api_url')), $formvars);
        return $init_rs;
    }
