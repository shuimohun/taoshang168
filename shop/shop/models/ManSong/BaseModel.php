		...
		if ($row)
		{
			...
			if ($row['rule'])
			{
				foreach ($row['rule'] as $key => $value)
				{
					if ($value['goods_id'])
					{
						$rule_goods = $this->Goods_BaseModel->getNormalGoodsBase($value['goods_id']);
                        if ($rule_goods)
                        {
                            $row['rule'][$key]['common_id']   = $rule_goods['common_id'];
                            ...
                            $row['rule'][$key]['goods_share_price'] = $rule_goods['goods_share_price'];
                            $row['rule'][$key]['goods_promotion_price'] = $rule_goods['goods_promotion_price'];
                            $row['rule'][$key]['goods_salenum'] = $rule_goods['goods_salenum'];
                        }
                        else
                        {
                            unset($row['rule'][$key]['goods_id']);
                            $row['rule'][$key]['goods_id'] = '0';
                        }
					}
				}
			}
