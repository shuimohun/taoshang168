...
class RedPacket_TempModel extends RedPacket_Temp
{
	...
	public function getRedPacketTempList($cond_row = array(), $order_row = array(), $page = 1, $rows = 100)
	{
        ...
        if($rows['items'])
        {
            ...
            {
                $rows['items'][$key]['redpacket_t_state_label']         = _(self::$redpacket_state_map[$value['redpacket_t_state']]);
                $rows['items'][$key]['redpacket_t_access_method_label'] = _(self::$redpacket_access_method_map[$value['redpacket_t_access_method']]);
                $rows['items'][$key]['redpacket_t_recommend_label']     = _(self::$redpacket_recommend_map[$value['redpacket_t_recommend']]);
                ...
                if (strtotime($value['redpacket_t_end_date']) < time())
                {
                    ...
                    $rows['items'][$key]['redpacket_t_state_label'] = _(self::$redpacket_state_map[self::INVALID]);
    ...
    public function getRedPacketTempInfoById($redpacket_t_id)
    {
        ...
        if ($row)
        {
            ...
            $row['redpacket_t_access_method_label'] = _(self::$redpacket_access_method_map[$row['redpacket_t_access_method']]);
            $row['redpacket_t_state_label']         = _(self::$redpacket_state_map[$row['redpacket_t_state']]);
            $row['redpacket_t_recommend_label']     = _(self::$redpacket_recommend_map[$row['redpacket_t_recommend']]);
