<?php if (!defined('ROOT_PATH'))
{
	exit('No Permission');
}

/**
 * User: zzh
 * Date: 2017/02/16
 * Time: 17:25
 */
class Price_QuotaModel extends Price_Quota
{
	public function getXianShiQuotaList($cond_row = array(), $order_row = array(), $page = 1, $rows = 100)
	{
		return $this->listByWhere($cond_row, $order_row, $page, $rows);
	}

	public function getPriceComboList($cond_row, $order_row, $page, $rows)
	{
		return $this->listByWhere($cond_row, $order_row, $page, $rows);
	}

	/**
	 *根据店铺ID获取店铺折扣套餐
	 * @param $shop_id 店铺id
	 * @return array
	 */
	public function getPriceQuotaByShopID($shop_id)
	{
		return $this->getOneByWhere(array('shop_id' => $shop_id));
	}

	/*检查套餐状态*/
	public function checkQuotaStateByShopId($shop_id)
	{
		$row                           = array();
		$cond_row['shop_id']           = $shop_id;
		$cond_row['combo_end_time:>='] = date('Y-m-d H:i:s');
		$row                           = $this->getOneByWhere($cond_row);

		if (!empty($row))
		{
			return true;
		}
		else
		{
			return false;
		}
	}
	
	public function removePriceQuotaItem($combo_id)
	{
		$del_flag = $this->remove($combo_id);

		return $del_flag;
	}

	/*
	 * 购买套餐
	 * */
	public function addPriceCombo($field_row, $return_insert_id)
	{
		// var_dump($field_row);die;
		return $this->add($field_row, $return_insert_id);
	}

	/*
	 * 套餐续费
	 * */
	public function renewPriceCombo($combo_id, $field_row)
	{
		return $this->edit($combo_id, $field_row);
	}
}