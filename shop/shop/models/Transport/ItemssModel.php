 ...
 * @author     WenQingTeng
 */
class Transport_ItemssModel extends Transport_Items
{
    /**
     * 读取店铺列表
     *
    ...
    public function delItems($transport_item_id = null)
    {
        $flag = $this->removeItem($transport_item_id);

        return $flag;
    }

    public function delItemNum($transport_type_id = null)
    {
        $data = $this->getByWhere(array('transport_type_id' => $transport_type_id));
        fb($data);
        foreach ($data as $key=>$value)
        {
            $flag = $this->removeItem($value['transport_item_id']);
        }
