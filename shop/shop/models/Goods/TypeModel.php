...
//		$Goods_TypeModel      = new Goods_TypeModel();        //商品类型表
		...
		if (!empty($type_rows))
		{
			...
			//商品属性值表
            $goods_property_rows = $Goods_PropertyModel->getByWhere(array('type_id' => $type_id),array('property_displayorder'=>'asc'));
			if (!empty($goods_property_rows))
			{
				foreach ($goods_property_rows as $key_property => $value_property)
				{
					//获取对应的属性值
					if ($value_property['property_format'] == 'select' || $value_property['property_format'] == 'checkbox' || $value_property['property_format'] == 'text')
					{
                        $cond_row['property_id'] = $value_property['property_id'];
                        $order_row['property_value_displayorder'] = 'asc';
						$property_values                   = $Goods_PropertyValueModel->getByWhere($cond_row,$order_row);
	...
	public function getTypeInfoByPublishGoods($cat_id = 0,$shop_id=0)
	{
		...
		else
		{
			...
			if ( !empty($data['property']) )
			{
				...
				{
				    if($property_data['property_format'] != 'text')
                    {
	...
	public function getPropertyByTypeId($type_id)
    {
        $data = array();
        $Goods_PropertyModel = new Goods_PropertyModel();    //商品属性表
        $Goods_PropertyValueModel = new Goods_PropertyValueModel(); // 商品属性值表
        ...
        if (!empty($data))
        {
            foreach ($data as $pro_k => $property_data)
            {
                if($property_data['property_values'])
                {
                    if($property_data['property_format'] != 'text')
                    {
                        foreach ($property_data['property_values'] as $pro_v_k => $pro_v_d)
                        {
                            if (empty($pro_v_d['property_value_name']))
                            {
                                unset($data[$pro_k]['property_values'][$pro_v_k]);
                            }
                        }
                    }
                }
