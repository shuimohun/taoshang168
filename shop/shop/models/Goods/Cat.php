 ...
 * @todo
 */
class Goods_Cat extends YLB_Model
{
	...
	public function getCatChildId($cat_parent_id = 0, $recursive = true)
	{
		...
		{
			$cond_row = array('cat_parent_id:in' => $cat_parent_id,'cat_is_display'=>1);
		}
		else
		{
			$cond_row = array('cat_parent_id' => $cat_parent_id,'cat_is_display'=>1);
		}
	...
	public function getCatTreeData($cat_parent_id = 0, $recursive = true, $level = 0, $filter = false)
	{
		...
		//设置cache
		$Cache = YLB_Cache::create('cat_base');
		...
		foreach ($cat_rows as $key => $cat_row)
		{
			...
			if ($recursive)
			{
				...
				$rs = call_user_func_array(array($this,'getCatTreeData'), array($cat_row['cat_id'],$recursive,$level));
			...
			{
                $rs = $this->getcatChildId($cat_row['cat_id'], false);
     ...
     * @return array
     */
    public function getCatTreeDataByStep($cat_parent_id = 0, $recursive = true, $level = 0,$skey)
    {
        $level++;

        $cond_row['cat_parent_id'] = $cat_parent_id;
        ...
        if($skey)
        {
            ...
            {
                unset($cond_row);
                $cond_row['cat_name:LIKE'] = '%'.$skey.'%';
            }
        }


        $cond_row['cat_is_display'] = 1;
        $cat_rows = $this->getByWhere($cond_row, array('cat_displayorder' => 'ASC'));
        if($recursive)
        {
            //类似数据可以放到前端整理
