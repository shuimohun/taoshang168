...
class Points_OrderModel extends Points_Order
{
	...
	public function getPointsOrderListByWhere($cond_row, $order_row = array('points_order_id'=>'DESC'), $page, $rows)
	{
		...
		if ($order_rows['items'])
		{
			...
			if ($points_order_id_row)
			{
               ...
               {
                    $points_goods_id[] = $value['points_goodsid'];
               }

               $points_goods_id = array_unique($points_goods_id);

               foreach($points_goods_id as $key=>$value)
               {
                   $points_goods[]= $this->Points_GoodsModel->getOne($value);
               }

               foreach($points_order_goods_row as $key=>$value)
               {
                   foreach ($points_goods as $k=>$v)
                   {
                       if($value['points_goodsid'] == $v['points_goods_id'])
                       {
                            $points_order_goods_row[$key]['points_goods_price'] = $v['points_goods_price'];
                       }
                   }
               }
			...
			foreach ($order_rows['items'] as $key => $value)
			{
			    $order_rows['items'][$key]['points_orderstate_str'] = self::$order_state_map[$value['points_orderstate']];
	...
	/*points_order_id 为金蛋订单表主键id,并非订单id*/
