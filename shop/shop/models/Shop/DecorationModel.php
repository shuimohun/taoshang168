...
//		var_dump($store_decoration_info);
	...
	public function getStoreDecorationBlockList($condition)
	{
		...
		foreach ($list as $key => $value)
		{
            ...
            if($value['block_content']){
                foreach ($value['block_content'] as $k => $v){
                    if($v['goods_id']){
                        $field = 'goods_share_price,goods_promotion_price';
                        $cond_row['goods_id'] = $v['goods_id'];
                        $res = $goodsBaseModel->select($field,$cond_row);
                        if($res){
                            $list[$key]['block_content'][$k]['common_share_price'] = $res[0]['goods_share_price'];
                            $list[$key]['block_content'][$k]['common_promotion_price'] = $res[0]['goods_promotion_price'];
                        }
                    }
                }
            }
