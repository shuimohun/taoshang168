...
if (!defined('ROOT_PATH'))
{
	...
	$data = $Order_BaseModel->getOrderBaseList($cond_row);

    $paycenter_api_key = YLB_Registry::get('paycenter_api_key');
	...
	if($data['items'])
	{
		...
		{	
			...
			foreach($directseller_member as $ks=>$vs)
			{
				if($vs)
				{
					$User_InfoModel->editInfo($vs,array('user_directseller_commission'=>$directseller_commission[$ks]),true);
					...
					$rs = get_url_with_encrypt($paycenter_api_key, sprintf('%s?ctl=Api_Pay_Pay&met=directsellerOrder&typ=json', $paycenter_api_url), $formvars);
				}
			}
