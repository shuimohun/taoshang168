...
input.error[type="text"], input.error[type="password"], textarea.error {
    ...
    outline: 0 none;
      ...
      <div class="member_infor_content">
        <div class="tabmenu">
          <ul class="tab pngFix">
           <li class="active"><a href="<?= YLB_Registry::get('url') ?>?ctl=Buyer_User&met=friend"><?=_('查找好友')?></a></li>
			<li><a href="<?= YLB_Registry::get('url') ?>?ctl=Buyer_User&met=friend&op=follow"><?=_('我关注的')?></a></li>
            <li><a href="<?= YLB_Registry::get('url') ?>?ctl=Buyer_User&met=friend&op=fan"><?=_('关注我的')?></a></li>
			</ul>
        </div>
	<div class="ncm-friend-find"> 
          <!-- 搜索好友start -->
          <form id="search_form" method="post" action="<?= YLB_Registry::get('url') ?>?ctl=Buyer_User&met=friend">
			...
			<ul class="ncm-friend-list">
				...
				<li id="recordone_76">
				  ...
				  <dl class="info">
					...
					<dd></dd>
					<dd><a href="<?= YLB_Registry::get('url') ?>?ctl=Buyer_Message&met=message&op=sendMessage&id=<?=$v['user_id']?>" title="<?=_('站内信')?>"><i class="icon-envelope"></i><?=_('站内信')?></a> </dd>
				  </dl>
				  <div class="follow">
                  ...
                  <div class="friends">
					...
					<?php if(!empty($val['user']['items'])){?>
                     <div>
                      ...
                      <a href="javascript:void(0);" class="care bbc_btns" nctype="batchFollow" data-param="{'ctl':'Buyer_User','met':'addFriends','id':'<?=$val['user_tag_id']?>'}"><i class="iconfont icon-jia"></i><?=_('关注TA们')?></a> </div>
					<?php }else{?>
					   <?=_('暂时没有发现贴上该标签的用户。')?>
						<a href="<?= YLB_Registry::get('url') ?>?ctl=Buyer_User&met=tag"><?=_('点击这里贴上自己的兴趣爱好。')?></a>
					<?php }?>
                    </div>
                </div>
              </dd>
