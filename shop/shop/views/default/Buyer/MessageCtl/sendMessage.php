...
<div class="aright">
	<div class="member_infor_content">
      <div class="tabmenu">
		<ul class="tab">
			<li><a href="<?= YLB_Registry::get('url') ?>?ctl=Buyer_Message&met=message"><?=_('系统消息')?><?=($this->countMessage['message'] ? sprintf('(%d)', $this->countMessage['message']) : '')?></a></li>
...
<!--            <li>
			...
			<li class="active"><a href="<?= YLB_Registry::get('url') ?>?ctl=Buyer_Message&met=message&op=sendMessage"><?=_('发送站内信')?></a></li>
        </ul>
    </div>
	...
	<ul>
	   ...
	   <?php foreach($data['items'] as $key=>$val){?>
        <li><a href="javascript:void(0);" id="<?=$val['friend_name']?>" nc_type="to_member_name">
          <div class="avatar"><img src="<?= YLB_Registry::get('ucenter_api_url') ?>?ctl=Index&met=img&user_id=<?=$val['friend_id']?>"></div>
          <p><?=$val['friend_name']?></p>
        </a></li>
	   <?php }?>       
	   <?php }?>       
