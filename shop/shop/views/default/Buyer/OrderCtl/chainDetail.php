          ...
          <div class="ncm-order-info">
            ...
            <div class="ncm-order-condition">
              ...
              <ul>
                ...
                <li><?=_('1. 您还没有去门店自提。')?></li>
                <li><?=_('2. 您的自提码是')?><?=$Order_GoodsChainCode['chain_code_id']?>。</li>
                <?php if($data['payment_id'] == PaymentChannlModel::PAY_ONLINE):?>
                <li><?=_('3. 如果您想取消购买，请与商家沟通后对订单进行')?><a href="<?= YLB_Registry::get('url') ?>?ctl=Buyer_Service_Return&met=index&act=add&oid=<?=($data['order_id'])?>" class="ncbtn-mini bbc_btns"><?=_('申请退款')?></a><?=_('操作。')?></li>
                <?php endif;?>
              </ul>
            </div>
			<?php endif;?>
          ...
          <table>
              ...
              <tbody class="tboy">
                                    ...
                                    <?php if($data['order_status'] != Order_StateModel::ORDER_WAIT_PAY && $data['order_status'] != Order_StateModel::ORDER_PAYED  && $data['order_status'] != Order_StateModel::ORDER_CANCEL){?>
                                     ...
                                     <?php }?>
                                        ...
                                        <?php if(($data['order_status'] == Order_StateModel::ORDER_FINISH && $data['complain_status']) || $data['order_status'] != Order_StateModel::ORDER_CANCEL){?>
                                            <p><a target="_blank" href="<?= YLB_Registry::get('url') ?>?ctl=Buyer_Service_Complain&met=index&act=add&gid=<?=($ogval['order_goods_id'])?>">
                                                <?=_('交易投诉')?>
                                            </a></p>
                                        <?php }?>
                                    </p>
				...
				<td class="td_rborder">
                   ...
                   <?php if($data['order_status'] == Order_StateModel::ORDER_WAIT_PAY  && $data['payment_id'] == PaymentChannlModel::PAY_CHAINPYA ){?>
                        <p class="getit"><?=_('门店付款')?></p>
                   <?php }?>
				...
				<!--S  未付款订单 -->
                    ...
                    <?php endif; ?>
                <!--E  未付款订单 -->
                <?php if(($data['order_status'] == Order_StateModel::ORDER_SELF_PICKUP && $data['payment_id'] != PaymentChannlModel::PAY_CHAINPYA) || $data['order_status'] == Order_StateModel::ORDER_FINISH){?>
