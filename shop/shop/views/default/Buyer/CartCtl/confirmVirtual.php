...
<link rel="stylesheet" type="text/css" href="<?= $this->view->css ?>/shop-cart.css" />
    ...
    <div class="">
      <h3 class="confirm"><?=_('电子兑换码/券接收方式')?></h3>
    </div>
    <div id="invoice_list" class="ncc-candidate-items">
      ...
      <p class=""><i class="icon-info-sign"></i><?=_('您本次购买的商品不需要收货地址，请正确输入接收手机号码，确保及时获得“电子兑换码”。可使用您已经绑定的手机或重新输入其它手机号码。')?></p>
    </div>
  </div>
					...
					<div class="bus_imfor clearfix">
						<p class="bus_name">
							<span>
								...
								<a href="<?= YLB_Registry::get('url') ?>?ctl=Shop&met=index&id=<?=($data['goods_base']['shop_id'])?>" class="cus_ser" ><?=($data['shop_base']['shop_name'])?></a>
