...
?>
<style>
    #search{margin: 0 ; height: 20px !important;}
</style>
<div class="wrapper fn-clear">
    ...
    <div class="search fn-clear">
        <form id="search_form" class="search_form_reset" method="get" action="#">
            <table class="search-form fr">
                <tbody><tr>
                    <td>&nbsp;</td>
                    <th class="pr8">订单状态</th>
                    <td class="w100 tl"><select  name="search_state_type">
                            <option  value="no">待自提</option>
                            <option  value="yes" <?php if($search_state_type == 'yes') echo 'selected=selected';?>>已自提</option>
                        </select></td>
                    <th class="w100 tl"> <select name="search_key_type">
                            <option value="order_sn">订单号</option>
                            <option value="buyer_phone" <?php if($search_key_type == 'buyer_phone') echo 'selected=selected';?>>手机号</option>
                        </select>
                    </th>
                    <td class="w160"><input type="text" class="text w150" name="keyword" id="search" value=""></td>
                    <td class="tc w70"><label class="submit-border">
                        <input class="submit" value="搜索" type="submit"></label>
                    </td>
                </tr>
                </tbody></table>
        </form>
    ...
    <table class="ncsc-default-table table-list-style">
        ...
        <tr>
            <th colspan="20" class="tl"><span class="ml10">订单编号：<?=$val['order_id']?></span><span class="ml20">下单时间：<?=$val['order_create_time']?></span></th>
        </tr>

            <tr>
                <td class="bdl"></td>
                <td class="w70">
                    <div class="goods-thumb"><a href="<?=YLB_Registry::get('url')?>?ctl=Goods_Goods&met=goods&type=goods&gid=<?=$val['goods_list'][0]['goods_id']?>" target="_blank"><img src="<?=$val['goods_list'][0]['goods_image']?>"></a></div></td>
                <td>
                    <dl class="goods-info">
                        <dt class="goods-name"><a href="<?=YLB_Registry::get('url')?>?ctl=Goods_Goods&met=goods&type=goods&gid=<?=$val['goods_list'][0]['goods_id']?>" target="_blank"><?=$val['goods_list'][0]['goods_name']?></a></dt>
                ...
                <td rowspan="1" class="bdl"><p>收货人：<?=$val['order_receiver_name']?></p><p>电话：<?=$val['order_receiver_contact']?></p></td>
                <?php if($val['order_status'] == Order_StateModel::ORDER_SELF_PICKUP) {?>
                    <?php if($val['payment_id'] == PaymentChannlModel::PAY_ONLINE){ ?>
                    <td rowspan="1" class="bdl bdd"><p><?=_('待自提')?></p></td>
                        <?php }elseif($val['payment_id'] == PaymentChannlModel::PAY_CHAINPYA){?>
                        <td rowspan="1" class="bdl bdd"><p><?=_('门店付款自提')?></p></td>
                    <?php }?>
                ...
                <?php }elseif($val['order_status'] == Order_StateModel::ORDER_RECEIVED){?>
                    <td rowspan="1" class="bdl bdd"><p><?=_('已自提')?></p></td>
                <?php }?>
...
<script>
    $(function(){
        $('a[nctype="process_order"]').click(function(){
            ...
            $.dialog({
                title: '',
                content: "url: <?= YLB_Registry::get('url') ?>?ctl=Chain_Order&met=orderManage&order_id="+order_id+"&typ=e",
    ...
    $(document).ready(function(){
        $('#search_form').validator({
            ...
            stopOnError: false,
            valid:function(form){
                window.location.href="<?= YLB_Registry::get('url') ?>?ctl=Chain_Order&met=index&"+$("#search_form").serialize()+"&typ=e";
            }
        });
    });
