    ...
    <table class="ncsc-default-table table-list-style">
        ...
        <?php if(!empty($goods)){
            foreach($goods as $key => $val){?>
                <tr>
                    <td></td>
                    <td><div class="pic-thumb"><a href="<?=YLB_Registry::get('url')?>?ctl=Goods_Goods&met=goods&type=goods&gid=<?=$val['goods_id']?>" target="_blank"><img src="<?=$val['goods_image']?>"></a></div></td>
                    <td class="tl"><dl class="goods-name">
                            <dt style="max-width: 450px !important;"> <a href="<?=YLB_Registry::get('url')?>?ctl=Goods_Goods&met=goods&type=goods&gid=<?=$val['goods_id']?>" target="_blank"><?=$val['goods_name']?></a></dt>
                        </dl></td>
                    <td><?=$val['SPU']?></td>
                    <td><?=$val['goods_code']?></td>
                    <td><span>￥<?=$val['goods_price']?></span></td>
                    <td><span><?=$val['goods_stock']?>件</span></td>
                    <td class="nscs-table-handle"><span><a href="javascript:void(0);" class="btn-bluejeans" nctype="set_stock" data-commonid="<?=$val['SPU']?>"><i class="iconfont icon-icowaitproduct"></i>
                                <p>设置库存</p>
                            </a></span></td>
                </tr>
            <?php }}else{ ?>
    ...
    <script>
        $(function(){
            $('a[nctype="set_stock"]').click(function(){
                ...
                $.dialog({
                    title: '设置库存',
                    content: "url: <?= YLB_Registry::get('url') ?>?ctl=Chain_Goods&met=goodsStock&common_id="+common_id+"&typ=e",
        ...
        $(document).ready(function(){
            $('#search_form').validator({
                ...
                stopOnError: false,
                valid:function(form){
                    window.location.href="<?= YLB_Registry::get('url') ?>?ctl=Chain_Goods&met=goods&"+$("#search_form").serialize()+"&typ=e";
                }
