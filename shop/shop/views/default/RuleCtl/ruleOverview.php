        ...
        <div class="l-main">
            ...
            <div class="l-content">
                <div class="b-subsearch-wrap">
                    <p class="h-words">在<span class="txt-black">所有规则</span>中精细搜索：
                    </p>
                    <div class="b-subsearch-box">
                        ...
                        <div class="re-count-box">
                            共搜索到<span class="txt-red"><?= count($search['items'])?></span>条规则：
                        </div>
...
//            alert('123');
    ...
    function getList() {
        $.post("?ctl=Rule&met=getList&typ=json",{group_id:group_id}, function (r) {
            for(var i in r.data.items){
           $('.search_list').append("<li class='li-item'><h3 class='h-title'><a href='index.php?ctl=Rule&met=ruleDetail&id="+r.data.items[i]['help_id']+"'class='link-dlk'>"+r.data.items[i]['help_title']+"</a></h3></li>");
            }
        });
    }
    $('.menu-item').click(function () {
