...
<script>
	$(function() {
		...
		categoryTree = Public.categoryTreeshop($('#cat_name'), opts);
        $('#cat_name').css('height','30px');
        $('#cat_name').css('width','230px');
...
{ 
 ...
 $(document).ready(function(){
         var ajax_url = './index.php?ctl=Seller_Shop_Info&met=new_class&typ=json';
        $('#form').validator({
            ignore: ':hidden',
            theme: 'yellow_right',
            timely: 1,
...
//                           parent.Public.tips.success('操作成功！');
                            window.open("./index.php?ctl=Seller_Shop_Info&met=new_class&act=authorization&cat_id="+a.data.product_class_id);
