...
<div class="exchange">
	<div class="alert">
        ...
        <?php  }else{ ?>
		<h4>
            <?php if($this->combo_flag){ ?><?=_('套餐过期时间')?>：<em class="red"></em><?=$combo['combo_end_time']?>。
            <?php }else{ ?>
	...
	<form id="search_form" method="get" action="<?=YLB_Registry::get('url')?>?ctl=Seller_Promotion_MeetConditionGift&met=index&typ=e">
        ...
        <a class="button refresh" href="<?=YLB_Registry::get('url')?>?ctl=Seller_Promotion_MeetConditionGift&met=index&typ=e"><i class="iconfont icon-huanyipi"></i></a>
	...
	<table class="table-list-style table-promotion-list" id="table_list" width="100%" cellpadding="0" cellspacing="0">
        ...
        <tr class="line_row">
            ...
            <td>
                <span class="edit"><a href="<?=YLB_Registry::get('url')?>?ctl=Seller_Promotion_MeetConditionGift&met=index&op=detail&typ=e&id=<?=@$value['mansong_id']?>"><i class="iconfont icon-btnclassify2"></i><?=_('详情')?></a></span>
