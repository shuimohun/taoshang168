...
<div class="form-style">
    <form method="post" id="form" action="<?/*=YLB_Registry::get('url')*/?>?ctl=Seller_Promotion_Price&met=add&typ=e">
        ...
        <dl class="goods_price" style="display:none;">
            <dt><?/*=_('店铺价格')*/?>：</dt>
            <dd><del><span></span><input type="hidden" id="goods_price" value=""></del></dd>
        </dl>
        <dl class="shared_price" style="display:none;">
            <dt><?/*=_('分享优惠')*/?>：</dt>
            <dd><span></span><input type="hidden" id="shared_price" value=""></dd>
        </dl>
        <dl>
            <dt><i>*</i><?/*=_('优惠价格')*/?>：</dt>
            <dd>
...
<script type="text/javascript">
   $(document).ready(function(){
        ...
        $('.search-goods-list-bd').on('click', '[data-type="btn_add_goods"]', function(){
            ...
            var data_sharedPrice = $(this).attr('data-shared-price');
            ...
            $(".shared_price").find("span").html(data_sharedPrice);
            $("#goods_price").val(goods_price);
            $("#shared_price").val(data_sharedPrice);
            $(".selected-goods").show();
            $(".shared_price").show();
            $(".goods_price").show();
            $('.search-goods-list').hide();
        ...
        $('#form').validator({
           ...
           rules:{
               noGreaterThanGoodsPrice:function(element) {
                   ...
                   if(Number(element.value) <= Number(shared_price)){
                       return '优惠价格必须大于商品分享价格！';
                   }else if(Number(element.value) >= Number(goods_price)){
                       return '优惠价格必须小于商品价格！';
                   }
               },
           ...
           valid: function(form){
               ...
               $.ajax({
                   url: "index.php?ctl=Seller_Promotion_Price&met=price_add&typ=json",
                   data: $(form).serialize(),
                   type: "POST",
                   success:function(e){
                       if(e.status == 200)
