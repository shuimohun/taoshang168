...
<div class="form-style">
    <form method="post" id="form" action="<?=YLB_Registry::get('url')?>?ctl=Seller_Promotion_Voucher&met=addVoucherTemp">
        <dl>
        ...
        <dl>
            ...
            <dd>
                ...
                <p class="hint"><?=_('“金蛋兑换”时会员可以在金蛋商城用金蛋进行兑换')?>；</p>
        ...
        <dl>
            ...
            <dd>
                ...
                <p class="pic image_review" style="width:200px;height:200px;" hidden="true">
                    <img id="image_review" src="" height="180" width="180" />
                </p>
                ...
                <p class="hint"><?=_('此处图片将在代金券模块使用，建议尺寸180*180px')?>。</p>
            </dd>
        </dl>
        ...
        <dl>
            ...
            <dd>
                ...
                <div class="mm"><a href="javascript:void(0)" data-increase-id='<?=@$data['increase_id']?>' class="button btn-ctl-select-goods bbc_seller_btns"><?=_('添加商品')?></a></div>
                ...
                <p class="hint"><?=_('点击上方输入框从商店商品中选择代金券上展示的商品')?></p>
                <p class="hint"><?=_('每张代金券最多只能推3个商品，若不添加则系统默认抓取3个商品')?>。</p>
            </dd>
        </dl>
    ...
    $(document).ready(function(){
        ...
        $('#image_upload').on('click', function () {
            $.dialog({
                ...
                data: {width:180,height:180 , callback: callback },    // 需要截取图片的宽高比例
                width: '800px',
                lock: true
            })
        });
        ...
        $('#form').validator({
			...
			rules: {
                ...
                voucherPrice:function (element, param, field) {
                    voucher_price = $('#voucher_t_price').find("option:selected").text();
                    voucher_limit = $('input[name=voucher_t_limit]').val();
                    if(voucher_price && voucher_limit){
                        if(Number(voucher_price) >= Number(voucher_limit)){
                            return '代金券面额必须小于消费金额';
                        }
                    }
                },
                voucherLimitPrice:function (element, param, field) {
                    voucher_price = $('#voucher_t_price').find("option:selected").text();
                    voucher_limit = $('input[name=voucher_t_limit]').val();
                    if(voucher_price && voucher_limit){
                        if(Number(voucher_price) >= Number(voucher_limit)){
                            return '消费金额必须大于代金券面额';
                        }
                    }
                }
            },
            fields: {
                ...
                'voucher_t_price': 'required;voucherPrice',
                ...
                'voucher_t_limit': 'required;integer[+0],voucherLimitPrice',
