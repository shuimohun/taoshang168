...
<div class="freight">
	<div class="tabmenu">
		<ul>
        	<li ><a href="<?=YLB_Registry::get('url')?>?ctl=Seller_Transport&met=transport"><?=_('运费设置')?></a></li>
                    ...
                    <?php foreach($val['city'] as $citykey => $cityval){?>
                        <li>
                            <label class="checkbox city">
                                <input <?php if($shop_transport && in_array($cityval['district_id'],$shop_transport)){?>disabled="disabled"<?php }else{ if($type_city && in_array($cityval['district_id'],$type_city)){?>checked="checked"<?php }}?> type="checkbox"  name="city[]" data-province="<?=($val['district_id'])?>" value="<?=($cityval['district_id'])?>" /><?=($cityval['district_name'])?>
                            </label>
                        </li>
                    <?php }?>
                    </ul>
...
<script>
    ...
    $(document).ready(function(){
        ...
        if(act == 'add')
        {
            var ajax_url = '<?= YLB_Registry::get('url') ?>?ctl=Seller_Transport&met=addTransport&typ=json';
        }
        if(act == 'edit')
        {
            var ajax_url = '<?= YLB_Registry::get('url') ?>?ctl=Seller_Transport&met=editTransport&typ=json';
        }
        ...
        $('.area input[name="city[]"]').each(function(){
            if ($('input[data-province="'+$(this).attr('data-province')+'"]').size() == $('input[data-province="'+$(this).attr('data-province')+'"]:checked').size()) {
                $('input[value="'+$(this).attr('data-province')+'"]').prop('checked', true);
            } else {
                $('input[value="'+$(this).attr('data-province')+'"]').prop('checked', false);
