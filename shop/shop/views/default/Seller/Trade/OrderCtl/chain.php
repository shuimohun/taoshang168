	...
	<tr>
		...
		<td class="bdl" rowspan="<?= $val['goods_cat_num']; ?>" style="width: 126px;">
            ...
            <?php if($val['payment_id']==1) {?>
                <p class="goods-pay" title="支付方式：在线支付">在线支付</p>
            <?php }else{?>
                <p class="goods-pay" title="支付方式：门店付款">门店付款</p>
            <?php }?>
		...
		<td class="bdl bdr" rowspan="<?= $val['goods_cat_num']; ?>">
			...
			<!-- 订单查看 -->
			<p><a href="<?php echo YLB_Registry::get('url') . '?ctl=Seller_Trade_Order&met=chainInfo&order_id=' . $val['order_id']; ?>" target="_blank">订单详情</a></p>
			<!-- 物流跟踪 -->
			<p></p>
		</td>
