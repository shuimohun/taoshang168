...
<div class="form-style">
    <form method="post" id="form">
        ...
        <dl id="temp">
            <dt><?=_('选择面单模板')?>：</dt>
            <dd>
                <select id="temp_id" name="waybill_temp_id" class="w300">
                    <option value="0"><?=_('未选择')?></option>
                </select>
                <span></span>
        ...
        <dl>
            ...
            <dd>
                ...
                <p class="hint"><?=_('顺丰、EMS(只支持广东省内)、快捷、宅急送、邮政快递包裹、中铁快运、邮政国内标快, 无需申请电子面单客户号即可正常下发订单')?></p>
                <p class="hint"><?=_('请输入正确的密码，否则会影响下单')?></p>
            </dd>
        </dl>
        <dl>
            <dt><?=_('月结账号')?>：</dt>
            <dd>
                <input type="text" name="month_code" class="text w300"/>
        ...
        <dl>
            <dt><i>*</i><?=_('打印机名称')?>：</dt>
            <dd>
                <input type="text"  id="printer" name="printer_name" class="text w300"/>
        ...
        <dl>
            <dt><?=_('代收货款卡号')?>：</dt>
            <dd>
                <input type="text" name="customerid" class="text w300"/>
    ...
    $(document).ready(function(){
        ...
        $('#form').validator({
            ...
            valid: function(form){
                ...
                $.ajax({
                    url: "index.php?ctl=Seller_Trade_PrintWaybill&met=addWaybillCont&typ=json",
                    data: $(form).serialize(),
                    type: "POST",
                    success:function(e){
                        if(e.status == 200){
