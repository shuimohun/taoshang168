...
<div class="eject_con">
    ...
    <form id="form" action="#" method="post">
        ...
        <dl>
            <dt><input id='is_receive' class="checkbox mr5" name="is_receive" type="checkbox" value="<?=$data['is_receive'];?>" <?php if($data['is_receive'] == 1){?> checked <?php }?> ><?=_('商家消息提醒：')?></dt>
            <dd>
            ...
            valid: function (form)
            {
                ...
                $.ajax({
                    ...
                    success: function (a)
                    {
                        if (a.status == 200)
                        {
                            window.parent.location.href="<?= YLB_Registry::get('url') ?>?ctl=Seller_Message&met=messageManage";
                        }
                        else
                        {
