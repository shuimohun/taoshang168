...
<div class="deliverSetting">
    <div class="alert">
        <h4><?=_('操作提示：')?></h4>
        <ul>
            <li><?=_('1、可以对消息进行查看和删除。')?></li>
            <li><?=_('2、删除消息，删除后其他账户的该条消息也将被删除。')?></li>
        </ul>
    </div>
  ...
  <tbody>
    <tr>
      <th class="tl"><label class="checkbox"><input class="checkall" type="checkbox"/></label> <?=_('消息内容')?></th>
      <th class="w200" width="150"><?=_('发送时间')?></th>
    </tr>
  <?php
	if($data['items'])
	{
  ...
  <tr>
      <td class="toolBar" colspan="2"><label  class="checkbox"><input class="checkall" type="checkbox"></label><?=_('全选')?>
	  <span>|</span>
		...
		<span>|</span>
         <label class="del" ><i class="iconfont icon-lajitong"></i><a data-param="{'ctl':'Seller_Message','met':'delAllMessage'}" ><?=_('删除')?></a></label>
       </td>
    </tr>
	 </tbody>
  <?php
    ...
    $('a[nc_type="batchbutton"]').click(function(){
	  ...
	  $.ajax({
			...
			success:function(a){
				...
				if(a.status == 200)
				{
					Public.tips.success("<?=_('标记成功！')?>");
					//obj.parents('tr:first').children('.tl').removeClass('dark');
					 location.reload();
				}
				else
				{
					Public.tips.error("<?=_('标记失败！')?>");
				}
			}
		});
		 }
