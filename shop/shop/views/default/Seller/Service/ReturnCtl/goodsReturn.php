...
<table class="table-list-style" width="100%" cellpadding="0" cellspacing="0">
    ...
    foreach ($data['items'] as $key => $value)
    {
        ...
        <tr>
            <td>
                <dl class="fn-clear">
                    <dd class="alr">
                        ...
                        <h3><a style="text-overflow: ellipsis;width:350px;height: 20px;display: inline-block;overflow: hidden;line-height: 26px;white-space: nowrap;" href="<?= YLB_Registry::get('url') ?>?ctl=Goods_Goods&met=goods&type=goods&gid=<?= $value['good']['goods_id'] ?>" target="_blank" title="<?= $value['order_goods_name'] ?>"><?= $value['order_goods_name'] ?></a></h3>
            ...
            <td><?= $value['return_state_con'] ?></td>
            <td>
            	<?php if(isset($dist_common_ids) && in_array($value['common_id'],$dist_common_ids)){?>
            		<span></span>
            	<?php }else{?>	
                    <span><a href="./index.php?ctl=Seller_Service_Return&met=goodsReturn&act=detail&id=<?= $value['order_return_id'] ?>"><i class="iconfont icon-chakan"></i><?=_('查看')?></a></span>
            	<?php }?>
            </td>
        </tr>
    <?php } ?>
