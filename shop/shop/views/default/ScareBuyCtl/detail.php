    ...
    <div class="t_goods_detail">
        <div class="crumbs clearfix">
            <p>
                ...
                <a href="<?= YLB_Registry::get('url') ?>?ctl=ScareBuy&met=index"><?=_('惠抢购中心')?></a><i class="iconfont icon-iconjiantouyou"></i>
                <?php if ($data['scarebuy_detail']['scarebuy_type'] == ScareBuy_BaseModel::ONLINEGBY){ ?>
                    <a href="<?=YLB_Registry::get('url')?>?ctl=ScareBuy&met=scareBuyList"><?=_('线上团')?></a>
                <?php }elseif ($data['scarebuy_detail']['scarebuy_type'] == ScareBuy_BaseModel::VIRGBY){ ?>
                    <a  href="<?=YLB_Registry::get('url')?>?ctl=ScareBuy&met=vrScareBuyList"><?=_('虚拟团')?></a>
                <?php } ?>
                <?php if(!empty($data['cat']))
                {
                    ...
                    {
                ?>
                <i class="iconfont icon-iconjiantouyou"></i><a href="<?=YLB_Registry::get('url')?>?ctl=ScareBuy&met=scareBuyList&cat_id=<?=$cat['id']?>"><?=$cat['name']?></a>
                <?php
                    }
                }
                ?>
                <i class="iconfont icon-iconjiantouyou"></i><a href="#"><?=$data['scarebuy_detail']['scarebuy_name']?></a> </p>
        </div>
            ...
            <div style="width: 935px;" class="ev_left2">
                <div class="ncg-main buy-now">
                    <div class="ncg-group sticky">
                        <h2><?=$data['scarebuy_detail']['scarebuy_name']?></h2>
                        <h3 class="bbc_color"><?=$data['scarebuy_detail']['scarebuy_remark']?></h3>
                        <div class="ncg-item">
                            <div class="pic"><img src="<?=image_thumb($data['scarebuy_detail']['goods_image'],400,400)?>" alt=""></div>
                            <div class="button bbc_bg">
                                <span class="bbc_color"><em><?=format_money($data['scarebuy_detail']['scarebuy_price'])?></em></span>
                                <a href="<?=YLB_Registry::get('url')?>?ctl=Goods_Goods&met=goods&type=goods&gid=<?=$data['scarebuy_detail']['goods_id']?>" target="_blank">
                                <?php if($data['scarebuy_detail']['scarebuy_state'] == ScareBuy_BaseModel::UNDERREVIEW){ ?><?=_('审核中')?>
                                <?php }elseif($data['scarebuy_detail']['scarebuy_state'] == ScareBuy_BaseModel::NORMAL){ ?><?=_('我要抢')?>
                                <?php }elseif($data['scarebuy_detail']['scarebuy_state'] == ScareBuy_BaseModel::FINISHED){ ?><?=_('已结束')?>
                                <?php }elseif($data['scarebuy_detail']['scarebuy_state'] == ScareBuy_BaseModel::AUDITFAILUER){ ?><?=_('审核失败')?>
                                <?php }elseif($data['scarebuy_detail']['scarebuy_state'] == ScareBuy_BaseModel::CLOSED){ ?><?=_('管理员关闭')?>
                                <?php }elseif($data['scarebuy_detail']['scarebuy_state'] == ScareBuy_BaseModel::WILLSTART){ ?><?=_('即将开始')?>
                                <?php } ?>
                                </a>
                            </div>
                            <div class="info" id="main-nav-holder">
                                <div style="width: 100%;" class="prices clearfix">
                                    <dl>
                                        <dt><?=_('原价')?></dt>
                                        <dd><del><?=format_money($data['scarebuy_detail']['goods_price'])?></del></dd>
                                    </dl>
                                    <dl>
                                        <dt><?=_('折扣')?></dt>
                                        <dd><em><?=_('立减')?><?=format_money($data['scarebuy_detail']['reduce'])?></em></dd>
                                    </dl>
                                    <dl>
                                        <dt><?=_('评分')?></dt>
                                        <dd>
                                ...
                                <div class="require clearfix">
                                    <h4><?=_('本商品已被抢购')?><em class="bbc_seller"><?=$data['scarebuy_detail']['scarebuy_virtual_quantity']?></em><?=_('件')?></h4>
                                    <p><?=_('数量有限，欲购从速!')?></p>
                                </div>
                                <div class="time time-remain clearfix fnTimeCountDown" data-end="<?=$data['scarebuy_detail']['scarebuy_endtime']?>">
                                    <!-- 倒计时 距离本期结束 -->
...
<div class="t_goods_bot clearfix wrap">
    <div class="t_goods-left">
        <div class="current_hot current_hots">
            ...
            <ul>
                <?php
                    if(!empty($data['hot_scarebuy']['items']))
                    {
                        foreach($data['hot_scarebuy']['items'] as $key=>$value)
                        {
                ?>
                <li>
                    <a href="<?=YLB_Registry::get('url')?>?ctl=ScareBuy&met=detail&id=<?=$value['scarebuy_id']?>">
                        <img src="<?=image_thumb($value['goods_image'],200,150)?>">
                    </a>
                    <h5><?=$value['scarebuy_name']?></h5>
                    <p class="current_hot_price"><span class="bbc_color"><?=format_money($value['scarebuy_price'])?></span></p>
                </li>
                <?php
                        }
                    }
            ...
            <div class="hot_all"><a class="bbc_btns" href="<?=YLB_Registry::get('url')?>?ctl=ScareBuy&met=index&typ=e"><?=_('全部热门惠抢购')?></a></div>
        </div>
    </div>
    ...
    <div class="t_goods_bot_right t_goods_ct_right">
        ...
        <ul class="goods_det_about_cont">
            ...
            <li class="xq_1" style="display:block">
                <?=$data['scarebuy_detail']['scarebuy_intro']?>
            </li>
        </ul>
...
var shop_id     = <?=($data['scarebuy_detail']['shop_id'])?>;
