            ...
            <div class="small-nav all-type">
                ...
                <ul style="overflow:hidden;">
                    <ul style="overflow:hidden;" class="priceNav">
                        <li class="lf <?php if(isset($state_id) && $state_id == 1){ ?> curr <?php }?>" data-state="1"><a href="<?=YLB_Registry::get('url')?>?ctl=Goods_SaveMoney&met=moreHZGoodList&state_id=1<?php if(isset($cat_id)){?>&cat_id=<?=$cat_id?><?php }?><?php if(isset($cat_sid)){?>&cat_sid=<?=$cat_sid?><?php }?>">50元以下</a></li>
                        <li class="lf <?php if(isset($state_id) && $state_id == 2){ ?> curr <?php }?>" data-state="2"><a href="<?=YLB_Registry::get('url')?>?ctl=Goods_SaveMoney&met=moreHZGoodList&state_id=2<?php if(isset($cat_id)){?>&cat_id=<?=$cat_id?><?php }?><?php if(isset($cat_sid)){?>&cat_sid=<?=$cat_sid?><?php }?>">50~100元</a></li>
                        <li class="lf <?php if(isset($state_id) && $state_id == 3){ ?> curr <?php }?>" data-state="3"><a href="<?=YLB_Registry::get('url')?>?ctl=Goods_SaveMoney&met=moreHZGoodList&state_id=3<?php if(isset($cat_id)){?>&cat_id=<?=$cat_id?><?php }?><?php if(isset($cat_sid)){?>&cat_sid=<?=$cat_sid?><?php }?>">100~300</a></li>
                        <li class="lf <?php if(isset($state_id) && $state_id == 4){ ?> curr <?php }?>" data-state="4"><a href="<?=YLB_Registry::get('url')?>?ctl=Goods_SaveMoney&met=moreHZGoodList&state_id=4<?php if(isset($cat_id)){?>&cat_id=<?=$cat_id?><?php }?><?php if(isset($cat_sid)){?>&cat_sid=<?=$cat_sid?><?php }?>">更多</a></li>
                    </ul>
                </ul>
                <!--价钱筛选开始-->
            ...
            <div class="small-nav all-type has-arrow">
                ...
                <ul class="orflow">
                    ...
                    <li class="ng-scope lf <?php if(isset($v['curr'])){ ?> curr <?php }?>">
                        <a href="<?=YLB_Registry::get('url')?>?ctl=Goods_SaveMoney&met=moreHZGoodList&cat_id=<?=($v['cat_id'])?><?php if(isset($state_id)){ ?>&state_id=<?=($state_id)?><?php }?>" class="ng-binding">
                            <?=($v['nav_name'])?>
                        </a>
                    </li>
                    <?php }}?>
            ...
            <div class="small-nav type-detail block">
                <ul>
                    ...
                    <?php if($data['sub_cat']){ foreach ($data['sub_cat'] as $k=>$v){?>
                        <li class="ng-scope lf <?php if(isset($v['curr'])){ ?> curr <?php }?>">
                            <a href="<?=YLB_Registry::get('url')?>?ctl=Goods_SaveMoney&met=moreHZGoodList&cat_sid=<?=($v['cat_id'])?>&cat_id=<?=($v['cat_parent_id'])?><?php if(isset($state_id)){ ?>&state_id=<?=($state_id)?><?php }?>"  class="ng-binding"><?=($v['cat_name'])?></a>
                        </li>
                    <?php }}?>
        ...
        <div class="savemoney-main savemoney-main-first">  
            <div class="save-item orflow moreGoodList">
                ...
                <div class="save-item-contain">
                    <?php if($data['promotion_list']['items']){?>
                    <ul class="clearfix">
