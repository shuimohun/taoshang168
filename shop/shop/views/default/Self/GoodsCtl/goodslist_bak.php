		 	...
		 	<div class="goods_list_intro clearfix">
		 		...
		 		<ul>
					<?php foreach($hot_sale as $hkey => $hval):?>
		 			<li>
		 				<a target="_blank" href="<?=YLB_Registry::get('url')?>?ctl=Supplier_Goods&met=goods&gid=<?=($hval['goods_id'])?>" class="goods_list_href">
		 					<div class="goodslist_img2"><img  src="<?=image_thumb($hval['common_image'],100,100)?>"/></div>
						</a>
		 				<p>
		 					<a target="_blank" href="<?=YLB_Registry::get('url')?>?ctl=Supplier_Goods&met=goods&gid=<?=($hval['goods_id'])?>" class="goods_list_name"><?=($hval['common_name'])?></a>
		 					<span><?=_('价格：')?><strong><?=format_money($hval['common_price'])?></strong></span>
		 					<a class="list_hot_intro bbc_btns"  target="_blank" href="<?=YLB_Registry::get('url')?>?ctl=Supplier_Goods&met=goods&gid=<?=($hval['goods_id'])?>" ><?=_('立即抢购')?></a>
		 				</p>
		 			</li>
					<?php endforeach;?>
		 		</ul>
			...
			<div class="cright clearfix">
				...
				<ul class="cr_xie clearfix">
					...
					<li>
						...
						<a target="_blank" href="<?=YLB_Registry::get('url')?>?ctl=Supplier_Goods&met=goods&<?=$id?>" class="cr_xie_lia1">
						...
						<div class="list_padd">
							<a href="<?=YLB_Registry::get('url')?>?ctl=Supplier_Goods&met=goods&<?=$id?>"  target="_blank" class="cr_xie_name" title="<?=($val['common_name'])?>">
							...
							<!--<?php if(!$val['common_is_virtual']){?>
								<a  class="cr_xie_lia2" onclick="addCart(<?=(@$val['goods_id'])?>,<?=(@$val['shop_owner'])?>)">
									...
									<span class="cr_xie_lia2_span2 cr_xie_lia2_span2-1 "><?=_('加入购物车')?></span>
								</a>
							<?php }else{?>
							...
							<?php if(isset($suppliers) && !in_array($val['shop_id'],$suppliers)){?>
                            	<a class="apply_dist"  data-id = <?=$val['shop_id']?> style="padding: 3.5px 50px;background-color: #e45050;color: white;"><?=_('申请分销商')?></a>
                            ...
                            <?php }elseif($val['product_is_behalf_delivery'] && isset($suppliers) && in_array($val['shop_id'],$suppliers)){?>
                            	...
                            	<a  style="padding: 3.5px 50px;background-color: #e45050;color: white;" > <?=_('已淘金')?> </a>
                            	<?php }else{?>
                            	<a class="add_distributor" data-id=<?=$val['common_id']?> style="padding: 3.5px 50px;background-color: #e45050;color: white;" > <?=_('一键上架')?> </a>
                            	<?php }?>	
                            <?php }?>	
