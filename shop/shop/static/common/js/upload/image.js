...
$(function() {
    var remoteImage, uploadImage, $confirmButton = $('.js-confirm');
    ...
    function initImageList(param) {
        if (typeof(param) === 'undefinded' || !param) {
            param = {
                album_id: $("li[class='js-category-item active']").data('album_id')
            };
        }
            ...
            success: function(data) {
                ...
                if (data && data['items'].length > 0) {
                    ...
                    for (i = 0; i < items.length; i++) {
                        $imageUl.append('<li class="image-item js-image-item" data-id="' + items[i].upload_id + '">' + '<div class="image-box" onload="alert(123);" style="background-image: url(' + items[i].upload_path + ')"></div>' + '<div class="image-meta">600*400</div>' + '<div class="image-title">' + items[i].upload_name + items[i].upload_mime_type + '</div>' + '</li>')
                    }
    ...
    function initButtons() {
        ...
        /!*准备需要的组件*!/
        var remote, parent = window.parent
        list = [];
        //dialog对象
        if (parent.$EDITORUI) {
        ...
        initUploader: function() {
            ...
            ratio = window.devicePixelRatio || 1,
                ...
                uploader = _this.uploader = WebUploader.create({
                    ...
                    fileSingleSizeLimit: bytes_to_size(uploadConfig.imageMaxSize),
                    // 1M
                    // 配置生成缩略图的选项
                    //compress:false,
                    //detail width = 790
                ...
                if (file.getStatus() == 'invalid') {
                    ...
                    uploader.makeThumb(file, function(error, src) {
                        if (error || !src) {
                            ...
                            var $li = $('<li class="upload-local-image-item" id="' + file.id + '">' + '<div class="image-box" style="background-image: url(' + src + ')"></div>' + '</li>'),
                                $img = $('<img src="' + src + '">');
    ...
    function RemoteImage() {
        this.imageList = [], this.$image = $('.js-network-image-preview');
        this.init();
    }
    ...
    if (typeof(uploadConfig) === 'undefined') {
        ...
        initButtons(), initEvent(), initImageList(), initCategory();

    }

    function initCategory() {
        //相册列表
        $.post(serverUrl + '&met=getAlbumList&typ=json&firstRow=' + firstRowA + '&totalRows=' + totalRowsA, {}, function(data) {
            var li = new String();
            if (data.status == 200) {
                ...
                $('#image-list').find('ul.category-list').html(li);

                if (items && items.length > 0) {
                    var page_nav = $(data.data.page_nav).each(function(index, element) {
                        var href = $(this).prop('href');
                        if (!(typeof href == 'undefined')) {
                            var firstRow = href.match(/firstRow=\d+/).join().replace('firstRow=', ''),
                                totalRows = href.match(/totalRows=\d+/).join().replace('totalRows=', '');
    ...
    $('.ui-paginationA').on('click', 'a', function() {
        var _thisPage;
        if ($(this).hasClass('nextPage') || $(this).hasClass('prePage')) {
            if ($(this).hasClass('nextPage')) {
                _thisPage = parseInt($('.ui-paginationA').find('b').html()) + 1;
            } else {
                _thisPage = parseInt($('.ui-paginationA').find('b').html()) - 1;
            }
        ...
        pageA = _thisPage, firstRowA = $(this).data('firstRow'), totalRowsA = $(this).data('totalRows');
