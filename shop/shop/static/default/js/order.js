...
$(document).ready(function() {
    ...
    window.hideOrder = function(e) {
        $.dialog({
            ...
            ok: function() {
                ...
                $.post(SITE_URL + '?ctl=Buyer_Order&met=hideOrder&typ=json', {
                    order_id: e,
                    user: 'buyer'
                }, function(data) {
    ...
    window.canOrder = function(e) {
        $.dialog({
            ...
            drag: false,
            ok: function() {
                $.post(SITE_URL + '?ctl=Buyer_Order&met=cancel&typ=json', {
                    order_id: e
                }, function(data) {
                    if (200 == data.status) {
    ...
    window.restoreOrder = function(e) {
        $.dialog({
            ...
            drag: false,
            ok: function() {
                $.post(SITE_URL + '?ctl=Buyer_Order&met=restoreOrder&typ=json', {
                    order_id: e,
                    user: 'buyer'
                }, function(data) {
                    if (data && 200 == data.status) {
