...
function change_district(value) {
    ...
    var url = SITE_URL + '?ctl=Base_District&met=district&typ=json&pid=' + val_array[0];
    $.getJSON(url, function(data) {
        data = data.data.items;
        if (data && data.length > 0) {
            var str = '<option value="">--请选择--</option>';
            var class_div_id = parseInt(val_array[1]) + 1;
            ...
            $('#id_' + val_array[1]).val(val_array[0]);
            ...
            for (var i = parseInt(val_array[1]); i < 4; i++) {
                $('#select_' + (i + 1)).empty();
