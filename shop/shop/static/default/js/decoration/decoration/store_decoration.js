$(document).ready(function() {
    ...
    // decoration.default_nav_style += '.ncs-nav li a span { font-size: 14px; font-weight: 100; line-height: 20px; text-overflow: ellipsis; white-space: nowrap; max-width:160px; color: #FFF; float: left; height: 20px; padding: 9px 15px; margin-left: 4px; overflow:hidden; cursor:pointer;}';
    //图片热点图片对象
    ...
    decoration.show_dialog_module_html = function($dialog, content) {
        $dialog.nc_show_dialog({width: 1020,height:765, title: '自定义模块'});
        if(!decoration.editor) {
           decoration.editor = UE.getEditor('module_html_editor', {
            toolbars: [
                [
                    'fullscreen','source','bold', 'italic', 'underline', 'forecolor', 'backcolor', 'justifyleft', 'justifycenter', 'justifyright', 'insertunorderedlist', 'insertorderedlist', 'blockquote',
...
//        decoration.editor.html(content);
    ...
    decoration.show_dialog_module_slide = function($dialog, content, full_width) {
        $dialog.nc_show_dialog({width: 1020,height:689, title: '图片和幻灯'});
        var html = '';
    ...
    decoration.show_dialog_module_goods = function($dialog, content) {
        ...
        $dialog.nc_show_dialog({width: 1020,height:660, title: '店铺商品模块'});
    };
    ...
    $('#btn_module_goods_search').on('click', function() {
        //var block_id = decoration.current_block_id;
        var common_id = [];
        $('#decorationGoods #div_module_goods_list li').each(function () {
            var c_id = $(this).data('common-id');
            common_id.push(c_id);
        })
        var param = '&' + $.param({keyword: $('#txt_goods_search_keyword').val(),common_id:common_id});
        $('#div_module_goods_search_list').load(URL_DECORATION_GOODS_SEARCH + param,function () {
            $('#div_module_goods_search_list .goods-list li').each(function () {
                var c_id = $(this).data('common-id');
                if($.inArray(c_id, common_id) >= 0){
                    $(this).children('[nctype="btn_module_goods_operate"]').html('已经添加');
                    $(this).children('[nctype="btn_module_goods_operate"]').addClass('added');
                }
            })
        });
    });
    ...
    $('#div_module_goods_search_list').on('click', '[nctype="btn_module_goods_operate"]', function() {
        if(!$(this).hasClass('added')){
            ...
            $(this).parents('[nctype="goods_item"]').hide();
    ...
    $('#btn_save_module_goods').on('click', function() {
        ...
        $('#div_module_goods_list').find('[nctype="goods_item"]').each(function() { 
            ...
            goods.common_id = $(this).attr('data-common-id');
    ...
    decoration.show_dialog_module_hot_area = function($dialog, content) {
        ...
        decoration.hot_area_image = $('#div_module_hot_area_image img');
        decoration.hot_area_image.imgAreaSelect({handles: true});

        if(decoration.hot_area_image.length > 0){
            proportion = decoration.hot_area_image.data('bili');
        }
    ...
    //保存添加的热点图片
    function btn_save_add_hot_area_image() {
        var data = {};
        $image = $('#input_hot_image');
        if($image.val()) {
            decoration.hot_area_index = 1;
            $('#module_hot_area_select_list').html('');

            data.image_url = $image.val();
            var html = template.render('template_module_hot_area_image', data);
            $('#div_module_hot_area_image').html(html);

            $('#div_module_hot_area_image img').imgAreaSelect({ instance: true });
        } else {
            alert('请上传图片');
        }
    }
    ...
    $('#btn_module_hot_area_add').on('click', function() {
        ...
        var ias = $('#div_module_hot_area_image img').imgAreaSelect({ instance: true });
    ...
    decoration.add_hot_area = function(position, link) {
        ...
        var display = {};
        display.width = (position_array[2] - position_array[0]) / proportion;
        display.height = (position_array[3] - position_array[1]) / proportion;
        display.left = position_array[0] / proportion;
        display.top = position_array[1] / proportion;
    ...
    $('#btn_save_module_hot_area').on('click', function() {
        ...
        var naturalWidth = $('#hot_area_image').naturalWidth();
        var width = $('#hot_area_image').width();
        var proportion = 1;
        if(naturalWidth > 0 && width > 0){
            proportion = naturalWidth / width;
        }

        var data = {};
        var i = 0;
        data.image = $('#div_module_hot_area_image img').attr('data-image-url');
        ...
        $('#module_hot_area_select_list li').each(function() {
            var area = {};
            var position = $(this).attr('data-hot-area-position').split(',');
            area.x1 = position[0] * proportion;
            area.y1 = position[1] * proportion;
            area.x2 = position[2] * proportion;
            area.y2 = position[3] * proportion;
            area.link= $(this).attr('data-hot-area-link');
            data.areas[i] = area;
            i++;
        });

        data.bili = proportion;
    ...
    //头部banner 添加热点 窗口显示
    $('#btn_banner_add_hot_area').on('click', function() {
    ...
    /*$('[nctype="btn_add_quicklink"]').on('click', function() {
     ...
     $.post("http://www.taoshang168.com/tesa/shop/index.php?act=seller_center&op=quicklink_del", { item: item }, function(data) {
     ...
     $.post("http://www.taoshang168.com/tesa/shop/index.php?act=seller_center&op=quicklink_add", { item: item }, function(data) {
