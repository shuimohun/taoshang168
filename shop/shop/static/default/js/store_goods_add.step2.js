...
$(function () {
    ...
    function checkSharePrice(type) {
        ...
        var share_price = price - share_weixin - share_weixin_timeline - share_sqq - share_qzone - share_tsina - promotion_total_price;
    ...
    $('#form').validator({
        ...
        rules: {
            ...
            promotionPrice: function ( element, params ) {
                var is_promotion = $("input[name='is_promotion']:checked").val();
                if ( is_promotion == 1 ) {
                    if(element.value != null && element.value >= 0.1){
                        return checkSharePrice();
                    }else{
                        return '不能小于0.1';
                    }
            ...
            sharePrice: function ( element, params ) {
                if(element.value != null && element.value >= 0.1){
                    return checkSharePrice();
                }else{
                    return '不能小于0.1';
            ...
            tj_link:function (element,params) {
        ...
        fields: {
            ...
            'share_limit':'required;integer[+];range[1~99999999];',
            'share_weixin':'required;range[0.1~];sharePrice;',
            'share_weixin_timeline':'required;range[0.1~];sharePrice;',
            'share_sqq':'required;range[0.1~];sharePrice;',
            'share_qzone':'required;range[0.1~];sharePrice;',
            'share_tsina':'required;range[0.1~];sharePrice;',
            'promotion_total_price':'promotionPrice;sharePrice;',
            'promotion_unit_price':'promotionPrice',
            'tj_link':'tj_link',
        },
