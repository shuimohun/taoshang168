$(function(){
    ...
    SpecialMessage += "<span error_type=\"defalut_num\" class=\"msg J_Message\" style=\"display:none\"><i class=\"icon-exclamation-sign\"><\/i>首重重量应输入0.0至999.9的数字<\/span>\n";
    ...
    RuleHead += "<th class=\"w110\">首重重量<\/th>\n";
    RuleHead += "<th class=\"w110\">首重费用<\/th>\n";
    RuleHead += "<th class=\"w110\">续重重量<\/th>\n";
    RuleHead += "<th class=\"w110\">续重费用<\/th>\n";
    ...
    RuleCell += "<\/td>\n";
    RuleCell += "<td>\n";
    RuleCell += "<input class=\"text w60\" type=\"text\" aria-label=\"首重重量\" maxlength=\"6\" autocomplete=\"off\" value=\"\" data-field=\"defalut_num\" name=\"special[TRANSTYPE][CurNum][default_num]\"><em class=\"add-on\">\n";
    RuleCell += "<i><strong>kg<\/strong><\/i>\n";
    RuleCell += "<\/em>\n";
    RuleCell += "<td>\n";
    RuleCell += "<input class=\"text w60\" type=\"text\" aria-label=\"首重费用\" maxlength=\"6\" autocomplete=\"off\" value=\"\" data-field=\"postage\" name=\"special[TRANSTYPE][CurNum][postage]\"><em class=\"add-on\">\n";
    RuleCell += "<i class=\"icon-renminbi\"><\/i>\n";
    RuleCell += "<\/em>\n";
    RuleCell += "<\/td>\n";
    RuleCell += "<\/td>\n";
    RuleCell += "<td>\n";
    RuleCell += "<input class=\"text w60\" type=\"text\" aria-label=\"续重重量\" maxlength=\"6\" autocomplete=\"off\" value=\"\" data-field=\"add_num\" name=\"special[TRANSTYPE][CurNum][add_num]\"><em class=\"add-on\">\n";
    RuleCell += "<i><strong>kg<\/strong><\/i>\n";
    RuleCell += "<\/em>\n";
    RuleCell += "<\/td>\n";
    RuleCell += "<td>\n";
    RuleCell += "<input class=\"text w60\" type=\"text\" aria-label=\"续重费用\" maxlength=\"6\" autocomplete=\"off\" value=\"\" data-field=\"add_price\" name=\"special[TRANSTYPE][CurNum][add_price]\"><em class=\"add-on\">\n";
    RuleCell += "<i class=\"icon-renminbi\"><\/i>\n";
    RuleCell += "<\/em>\n";
    RuleCell += "<\/td>\n";
...
//	                if(target.getAttribute("id")==$(objPanel).attr('id'))return true;
    ...
    $('.trans-line').on('blur','input[data-field="defalut_num"]',function (){
        var oNum = new Number($(this).val());
        oNum = oNum.toFixed(1);
        if (oNum > 999.9) oNum = 999.9;
        if (oNum=='NaN') oNum = '0.0';
        $(this).val(oNum);
        if($(this)[0].className=='w50 mr5 text input-error') $(this).removeClass('input-error');
        if($(this)[0].className=='w50 ml5 mr5 text input-error') $(this).removeClass('input-error');
        ...
        //如果是动态添加的首费,当所有首费输入框都不为空时，提示层span隐藏
        isRemove = true;
        $(this).parent().parent().parent().find('input[data-field="defalut_num"]').each(function(){
            if ($(this).val()==''){
                isRemove = false;return false;
            }
        });
        //提示层span隐藏
        if (isRemove == true){
            $(this).parent().parent().parent().parent().parent().parent().find('.tbl-attach').find('.J_SpecialMessage').find('span[error_type="defalut_num"]').css('display','none');
        }
    });
    $('.trans-line').on('blur','input[data-field="add_num"]',function (){
        var oNum = new Number($(this).val());
        oNum = oNum.toFixed(1);
        if (oNum > 999.9) oNum = 999.9;
        if (oNum=='NaN') oNum = '0.0';
        ...
        //如果是动态添加的首费,当所有首费输入框都不为空时，提示层span隐藏
        isRemove = true;
        $(this).parent().parent().parent().find('input[data-field="add_num"]').each(function(){
            if ($(this).val()==''){
                isRemove = false;return false;
            }
        });
        //提示层span隐藏
        if (isRemove == true){
            $(this).parent().parent().parent().parent().parent().parent().find('.tbl-attach').find('.J_SpecialMessage').find('span[error_type="add_num"]').css('display','none');
        }
    });
    /*	删除一行运费规则*/
    ...
    $('.trans-line').on('click','.J_BatchSet',function(){
        if($('.tbl-except').find('input:checked').size()==0){
            ...
            //$.dialog.alert('请选择要批量设置的地区');return false;
        }
    ...
    $('#dialog_batch').on('click','.J_SubmitPL',function(){
        ...
        $('.tbl-except>table>tbody>tr').each(function(){
            if ($(this).find('.J_BatchField').attr('checked')){
                $(this).find('input[data-field="defalut_num"]').val($(obj_this).find('input[data-field="defalut_num"]').val()).removeClass('input-error');
                $(this).find('input[data-field="postage"]').val($(obj_this).find('input[data-field="postage"]').val()).removeClass('input-error');
                $(this).find('input[data-field="add_num"]').val($(obj_this).find('input[data-field="add_num"]').val()).removeClass('input-error');
                $(this).find('input[data-field="add_price"]').val($(obj_this).find('input[data-field="add_price"]').val()).removeClass('input-error');
            }
        });
        $("#dialog_batch").css('display','none');
        $('.ks-ext-mask').css('display','none');
    ...
    $('#submit_tpl').on('click',function(){
        ...
        {
            isShowError = false;
            $('.tbl-except').find('input[data-field="defalut_num"]').each(function(){
                if ($(this).val()==''){
                    $(this).addClass('input-error');isShowError = true; isSubmit = false;return false;
                }
            });
            ...
            if (isShowError){
                $('.tbl-attach').find('span[error_type="defalut_num"]').show();
            }
        }
        //地区JS空判断-------------------------------
        if($('.tbl-except').find('.cell-area').html() != null){
