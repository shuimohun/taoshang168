...
<div class="wrapper page">
    <div class="fixed-bar">
        <div class="item-title">
            ...
            <ul class="tab-base nc-row">
                ...
                <li><a class="current" ><span>更新PayCenter</span></a></li>
            </ul>
        </div>
    </div>
	...
	<?php if ($client_version !== $version_row['current'] && version_compare( $version_row['current'], $client_version, 'gt')):?>
        <h1>有新的PayCenter版本(<?=$version_row['current']?>)可供升级。</h1><br />
	...
	<?php else: ?>
        <h1>您使用的PayCenter是最新版本。 将来的安全更新将被自动安装。</h1><br />
...
<script>
    ...
    $('#reinstall').on("click", function (e)
    {
        $.dialog.confirm('确认重新安装,是否继续？', function () {
            window.location.href = "<?= YLB_Registry::get('url') ?>?ctl=Config&met=upgradePayContainer&upgrade=1&force-upgrade=1";
        });
    });

    $('#force-check').on("click", function (e)
    {
        window.location.href = "<?= YLB_Registry::get('url') ?>?ctl=Config&met=updatePay&force-check=1";
    });

    $('#upgrade').on("click", function (e)
    {
        $.dialog.confirm('确认更新,是否继续？', function () {
            window.location.href = "<?= YLB_Registry::get('url') ?>?ctl=Config&met=upgradePayContainer&upgrade=1";
        });
    });
    ...
    $('#force-upgrade').on("click", function (e)
    {
        $.dialog.confirm('确认强制重新安装,是否继续？', function () {
            window.location.href = "<?= YLB_Registry::get('url') ?>?ctl=Config&met=upgradePayContainer&force-upgrade=1";
        });
    });
