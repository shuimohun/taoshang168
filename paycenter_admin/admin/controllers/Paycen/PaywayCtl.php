 ...
 * @author     WenQingTeng
 */
class Paycen_PaywayCtl extends YLB_AppController
{
	...
	public function alipay()
	{
		...
		fb($type);
		$key    = YLB_Registry::get('zend_payway_key');
                $url    = YLB_Registry::get('payway_api_url');
                        //fb($url);
                $app_id = YLB_Registry::get('app_id');
                $data              = array();
                $data['app_id']    = $app_id;
                $data['ctl'] = 'Api';
                $data['met'] = 'getPayWays';
	...
	public function editPayStatus()
	{
		...
		if($flag == 200)
        {
            $msg = 'success';
            $status = 200;
			location_to(YLB_Registry::get('pay_status_url'));
        }
        else
        {
            $msg = 'failure';
