...
YLB_Log::log("r=" . json_encode($_REQUEST), YLB_Log::INFO, 'pay_alipay_return');
YLB_Log::log("g=" . json_encode($_GET), YLB_Log::INFO, 'pay_alipay_return');
YLB_Log::log("p=" . json_encode($_POST), YLB_Log::INFO, 'pay_alipay_return');

$Payment_AlipayModel = PaymentModel::create('alipay');
$verify_result          = $Payment_AlipayModel->verifyReturn();
...
if ($verify_result)
{
	...
	if (isset($_GET['trade_status']))
	{
		if($_GET['trade_status'] == 'TRADE_FINISHED' || $_GET['trade_status'] == 'TRADE_SUCCESS')
		{
			...
			//TRADE_SUCCESS（开通了高级即时到账或机票淘金产品后的交易成功状态）
		...
		{
			echo "fail";
			YLB_Log::log('trade_status 失败:' . $_GET['trade_status'], YLB_Log::ERROR, 'pay_alipay_return_error');
			YLB_Log::log('trade_status 失败:' . $_GET['trade_status'], YLB_Log::ERROR, 'pay_alipay_return');
		}
	}
	else
	{
		//验证失败
		echo "fail";
		YLB_Log::log('trade_status 失败', YLB_Log::ERROR, 'pay_alipay_return_error');
		YLB_Log::log('trade_status 失败', YLB_Log::ERROR, 'pay_alipay_return');
	}
}
...
else
{
	//验证失败
	echo "fail";
	YLB_Log::log('签名验证失败', YLB_Log::ERROR, 'pay_alipay_return_error');
	YLB_Log::log('签名验证失败', YLB_Log::ERROR, 'pay_alipay_return');
