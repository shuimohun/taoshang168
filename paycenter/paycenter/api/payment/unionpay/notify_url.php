...
YLB_Log::log("r=" . json_encode($_REQUEST), YLB_Log::INFO, 'pay_unionpay_return');
YLB_Log::log("g=" . json_encode($_GET), YLB_Log::INFO, 'pay_unionpay_return');
YLB_Log::log("p=" . json_encode($_POST), YLB_Log::INFO, 'pay_unionpay_return');
    ...
    { 
		...
		if ($rs)
		{
			...
			if($trade_type == 'shopping')
			{
				...
				{
					$return_app_url = YLB_Registry::get('shop_wap_api_url') . 'tmpl/member/order_list.html';
				}
				else
				{
		...
		echo '验签失败';die;
		YLB_Log::log('签名验证失败', YLB_Log::ERROR, 'pay_uinonpay_return_error');
		YLB_Log::log('签名验证失败', YLB_Log::ERROR, 'pay_unionpay_return');
    }
}else 
{
    echo '签名为空';die;
	YLB_Log::log('签名为空', YLB_Log::ERROR, 'pay_uinonpay_return_error');
	YLB_Log::log('签名为空', YLB_Log::ERROR, 'pay_unionpay_return');
}
?>