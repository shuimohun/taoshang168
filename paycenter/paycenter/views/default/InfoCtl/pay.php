        ...
        function checkPassword() {
            $.post(SITE_URL + "?ctl=Info&met=checkPassword&typ=json",{password:$("input[name='password']").val()},
                function(data){
                    console.info(data);
                    if(data.status == 250) {
                        $(".msg-box").html(data.msg + "，<a style='color:red;' href=' " + SITE_URL + "?ctl=Info&met=passwd'>忘记支付密码</a>");
                ...
                $.post(SITE_URL + "?ctl=Info&met=checkPayWay&typ=json" ,data,
                    function(data){
                        if(data.status == 200){
                            //如果选择了在线支付方式
                            if(online_payway){
                                var url = SITE_URL + "?ctl=Pay&met=" + online_payway + "&trade_id=" + uorder_id;
                                if(browserRedirect() == "phone" && !is_weixin()){
                                    url+="&typ=json&type=wap";
                                }
