...
<?php }else{?>
    <div class="security-tips">充值前必须先进行邮箱绑定或手机绑定，点击这里进行<a href="<?=YLB_Registry::get('ucenter_api_url')?>?ctl=User&met=security&op=mobile">手机绑定</a>或<a href="<?=YLB_Registry::get('ucenter_api_url')?>?ctl=User&met=security&op=email">邮箱绑定</a></div>
<?php }?>	
	<div class="recharge2-content-bottom content-public wrap" style="width:95%;">
		...
		<div class="content">
			<div class="one">
				...
				<p class="texts">答：您在带有信用卡小标识的店铺购物，可以直接使用信用卡快捷（含卡通）、网银进行信用卡支付，支付限额为您的卡面额度。在没有信用卡标识的店铺购物 时，您可以使用信用卡快捷（含卡通）、网银进行信用卡支付，月累计支付限额不超过XXX元。
                </p>
			</div>
			<div class="one">
				<h3>2.没有网上银行，怎么用银行卡充值？</h3>
				<p class="texts">答：储蓄卡用户，请使用储蓄卡快捷支付充值，开通后只需输入财付宝支付密码，即可完成充值。</p>
			</div>
			<div class="one">
				<h3>3.怎样在网上开通储蓄卡快捷支付(含卡通)？ </h3>
				<p class="texts">答：已支持国内大部分主流银行在线开通。在财付宝填写信息后，根据页面引导在网上银行完成开通。</p>
			</div>
		</div>
	</div>
