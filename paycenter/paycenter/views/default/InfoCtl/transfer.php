	...
	function checkyzm(){
		...
		$('#form').validator({
			...
			valid:function(form){
				...
				var ajax_url = '<?= YLB_Registry::get('url');?>?ctl=Info&met=addTransfer&typ=json';
				//表单验证通过，提交表单
				$.ajax({
					url: ajax_url,
	...
	<div class="transferaccounts-bottom fr">
		...
		<div class="content-center ">
			...
			<div class="pulibc-divcenter ">
				...
				<p><?=_('答：不会，淘尚168商城卖家请注意店铺交易安全，谨防假冒客服，以审核消保、二次审核为由诱导转账。')?></p>
			</div>
			<div class="pulibc-divcenter ">
				<h4><?=_('3.（提醒卖家知识）网购时，谨防卖家诱导加QQ要求转账付款')?></h4>
				<p><?=_('答：不会，请使用财付宝担保交易付款，谨防卖家诱导加QQ、使用转账付款；卖家请注意店铺交易安全，谨防假冒客服，以审核消保、二次审核为由诱导转账。')?></p>
			</div>
			<div class="pulibc-divcenter ">
				<h4><?=_('4.（提醒买家知识）淘宝购物，卖家加QQ要求转账付款，安全吗？')?></h4>
				<p><?=_('答：购物时，请使用财付宝担保交易付款，谨防卖家诱导加QQ、使用转账付款。')?></p>
			</div>
		</div>
	</div>
</div>
...
<?php }else{?>
    <div class="security-tips">转账前必须先进行手机绑定，点击这里进行<a href="<?=YLB_Registry::get('ucenter_api_url')?>?ctl=User&met=security&op=mobile">手机绑定</a></div>
	<?php }?>
<?php
