...
<div class="form-style-layout wrap" style="border:none;">
    ...
    <?php if($data['user_email'] || $data['user_mobile']){?>
        ...
        <?php }else{?>
            <div class="security-tips">修改密码前必须进行实名认证，点击这里进行<a href="<?=YLB_Registry::get('paycenter_api_url')?>?ctl=Info&met=certification">实名认证</a></div>
        <?php }?>
    <?php }else{?>
        <div class="security-tips">修改密码前必须先进行邮箱绑定或手机绑定，点击这里进行<a href="<?=YLB_Registry::get('ucenter_api_url')?>?ctl=User&met=security&op=mobile">手机绑定</a>或<a href="<?=YLB_Registry::get('ucenter_api_url')?>?ctl=User&met=security&op=email">邮箱绑定</a></div>
    <?php }?>
</div>
<?php
