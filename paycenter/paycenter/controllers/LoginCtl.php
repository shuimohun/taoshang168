...
class LoginCtl extends YLB_AppController
{
	...
	public function login()
	{
	    ...
	    if (!Perm::checkUserPerm())
		{
			...
			$findpwd_url = YLB_Registry::get('ucenter_api_url') . '?ctl=Login&met=findpwd&typ=e';
		...
		{
			header('location:' . YLB_Registry::get('base_url') . '/index.php?ctl=Info&met=account');
	...
	public function check()
	{
		...
		if (200 == $init_rs['status'])
		{
			...
			if ($user_row)
			{
				...
				{
                    $redirect = YLB_Registry::get('base_url') . '/index.php?' . request_string('redirect');
				}
				else
				{
					$redirect = YLB_Registry::get('base_url');
				}

				if ('e' == $this->typ)
				{
                   if(request_string('wt') == 'skip')
                   {
                        location_to($redirect);
                   }
                   else if($redirect)
                   {
	...
	public function loginout()
	{
		if ($_REQUEST['met'] == 'loginout')
		{
			...
			$callback  = YLB_Registry::get('shop_api_url') . '?redirect=' . urlencode(YLB_Registry::get('url')) . '&type=ucenter';
