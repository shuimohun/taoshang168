...
//        $data = $User_ResourceModel->getResourceLists();
//        $sql    = request_string('sql');
        $sql   = $_REQUEST['sql'];
        $page  = $_REQUEST['page'];
        $rows  = $_REQUEST['rows'];
        $data = $this->userInfoModel->select_sql($sql,$page,$rows);
        if ($data)
        {
            $msg    = 'success';
            $status = 200;
    ...
    //获取所有用户TOP资源信息
    public function  getUserInfo_top($sql){
//        $User_ResourceModel = new User_ResourceModel();
//        $data = $User_ResourceModel->getResourceLists();
//        $sql    = request_string('sql');
        $data = $this->userInfoModel->sql($sql);
        if ($data) {
            $msg    = 'success';
            $status = 200;
        } else {
    ...
    public function checkUser()
    {
        ...
        $this->data->addBody(-140, array(), '', $status);
    }
